...
Resolve the PR number, use `<worktree>/scripts/pr-state --summary <PR>` for state, use `scripts/pr-resolve list <PR>` for actionable threads, and use `scripts/pr-resolve reply <PR> <comment_id> <thread_id> <body>` for replies.
