...
2 is only as tight as the literal text around a message's placeholders.
`"Delete {{name}}"` compiles to `/^Delete [\s\S]+?$/`, which accepts *any*
sentence beginning "Delete " — so a rewrite of one such description reads as a
value extraction and is not reported. Measured by simulating a rewrite of every
removed string on two real migrations, a rewrite was caught for 79 of 96 strings
on one and 24 of 43 on the other. That is worth having and is not worth
trusting absolutely: it is why the mutation probe above is a separate step, and
why "the check was clean" is not on its own an answer to "did the English
change?".

Tightening this is not a one-liner, and the trade should be made deliberately
rather than rediscovered: `"{{a}} of {{b}}"` (too permissive to anchor anything)
and `"{{count}} of {{total}}"` (a legitimate value extraction that should stay
quiet) normalize to the same two-character remainder and compile to the *same*
pattern, so no rule that counts words or letters can separate them. Whatever
does — a literal-to-placeholder ratio, a minimum literal run — buys coverage by
spending false positives, which is the right direction for an advisory check but
should be chosen with the numbers in front of you.
