...
checks scan `components`, `app`, `hooks`, `lib`, and published Markdown in
`docs/public` repo-wide, and it runs in
CI (Frontend Tests → _Run i18n checks_). The one exception is
`check-nonjsx-copy.mjs`, which is scoped to `i18nGuardFiles` for the same reason
the eslint rule is (see [Non-JSX copy](#and-the-blind-spots-are-now-a-gate)). That is deliberate: each one pins a bug
...
It is **scoped to `i18nGuardFiles`**, the same allowlist the eslint rule uses and
for the same reason: judging an un-migrated file's existing config tables would
recreate the treadmill described above. The repo-wide half is
`check-new-code-i18n.mjs`, which runs this same detector over added files and
changed lines everywhere — so the new-code ratchet no longer inherits the hole.
...
const DEFAULT_PR_PRESETS = [{ label: "Review", hint: "Read the diff, flag issues" }];
```
...
marker with no reason after the colon is *reported*, not honoured.
