...
Taiwan (`zh-tw`), Traditional Chinese Hong Kong (`zh-hk`), and Japanese (`ja`,
endonym `日本語`) are the shipped human locales, and a generated **pseudo-locale**
is used for QA. Requirements live in
...
- **The five real locales gate the build.** `pt-pt`, `zh-cn`, `zh-hk`, `zh-tw`
  and `ja` are complete, and adding an `en` key without translating it fails
...
Use a canonical lowercase locale id such as `zh-cn` or `ja`. Locale negotiation
accepts case variants such as `zh-CN` and unambiguous region suffixes such as
`ja-JP` (collapsed onto `ja`); cookies and `<html lang>` use the canonical id.
...
None of it visible in a green build. The four catalogs were completed first, then
`ja` joined them, so the failure mode has flipped: the risk is no longer "a PR is
blocked on a translator" but "a PR silently re-opens the gap". So the pass
**gates**, and the cost is that adding user-facing copy means adding it in six
languages (`en` plus five real locales). That is the real cost of shipping those
languages; it was previously being deferred rather than paid.
...
For a namespace-scoped change, use the converter's `--namespace <name>` option,
for example `node scripts/convert-zh-cn-to-zh-hant.mjs --locale all
--namespace system --write` from `apps/web`. If using the `pnpm run` wrapper,
first confirm that its extra arguments reach the converter. Review the diff for
the selected keys and preserve unrelated manual translations; restore only
generated edits belonging to the current change.
