...
individual users. Promotion intentionally makes the surface on by default for
both existing and new users; users may then opt out portably.

## Consequences

- The stable status surface is available in every shipped profile without a
  release toggle or restart.
