...
Selected OpenCode evidence can end polling without unrelated bots once CI and all blocker gates are clean, while generic Actions activity cannot satisfy the gate. COMMENT deliberately avoids representing automated review as human approval. The App introduces credentials, installation, and operational setup; unavailable setup deliberately falls back to local review.
...
- **Local-only frontier review:** safest fallback but consumes more local review capacity for routine changes.
