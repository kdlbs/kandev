...
Each managed agent runner supplies the narrowest adapter needed to perform that
procedure. The OpenCode runner exposes a base-commit-controlled
`render_pr_walkthrough` custom tool. The tool accepts walkthrough JSON, binds
authoritative pull-request identity from the GitHub event, removes model-owned
link overrides, and runs only the trusted renderer against fixed output paths.
It does not grant generic edit or shell access. A future Codex or other runner
may use its native file and command tools, but it must follow the same skill and
