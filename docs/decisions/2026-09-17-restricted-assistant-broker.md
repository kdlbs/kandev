...
only the broker's permission allowlist. The pinned CLI's `--restricted` and
`--disable-slash-commands` apply as well. ACP host file/terminal methods and
...
The pinned SDK binary's help verifies the two extra CLI switches. A provider
