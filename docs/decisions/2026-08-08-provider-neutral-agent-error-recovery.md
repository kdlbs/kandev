...
| `agent_transport_lost` | Short same-profile retry when high-confidence (the ACP wire-level transport-death signature) | Retry the same route for a few seconds; the resume token is provider-specific, so fallback is not allowed — park for manual recovery after the short budget is exhausted |
