...
Kandev already has an agentctl development-server and Browser-panel path. It
routes executor-local HTTP ports through a session-authorized proxy and renders
them in an iframe that permits scripts and same-origin behavior. Reusing this
path gives static HTML the same fidelity and executor coverage as an explicitly
...
trusted Browser-panel model for native HTML preview instead of a dedicated
preview origin.

## Decision

Treat native HTML preview as execution of trusted workspace code. Use the
existing Browser-panel sandbox, origin behavior, and session port proxy. Do not
claim that preview isolates users from hostile HTML or JavaScript.
...
requests to agentctl. Desktop opens the returned session proxy URL in the
existing Browser panel. Mobile uses the same URL in its focused full-height file
viewer.
