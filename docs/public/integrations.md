...
The repository picker in **New Task** keeps **Local** and each eligible provider in named tabs above the search field. A provider tab appears only when its integration is configured, enabled, and successfully tested. Search results remain scoped to the active provider. Supported repository and pull-request URLs can still be entered manually when their provider has no eligible browse tab. The picker can add local and remote rows to the same task. For a private Azure repository, the backend uses the workspace PAT only while initially cloning or fetching the managed checkout. The PAT is not written into the remote URL, task metadata, command arguments, or agent environment. Configure executor Git credentials independently for pushes and for repository access outside that backend materialization path.
