...
| host.taskFilters | getSelection(id)/setSelection(id, values)/subscribe(listener) for this plugin's own registerTaskFilter registrations — the same selection the built-in dropdown reads | Active ui.bundle | Namespaced internally to `${pluginId}:${id}`; pair with `hidden: true` on the registration to drive a plugin-owned filter UI instead of the built-in section | host.taskFilters.setSelection("tags", ["tag-1"]) |
