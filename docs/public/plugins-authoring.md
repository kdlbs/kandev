...
| host.api.fetch / baseUrl        | fetch(path, init?) is scoped to /api/plugins/<id>/...; baseUrl is the backend origin for split-origin deployments                                                                                                                                                                                      | Active ui.bundle; backend path must be a declared webhook when relayed | Use authenticated access for UI-only or billable operations. It receives no Authorization or Cookie header. A generation change waits for the response.                                                         | host.api.fetch("webhooks/inbound", { method: "POST" })                                                              |
