...
| GitLab merge-request review and task MR lifecycle notifications | Dependency-bound | Requires GitLab credentials and project permissions. GitLab has no auto-fix or auto-merge automation; task-level controls notify agents when their review is requested (assignment as a reviewer, since GitLab has no distinct review-request event), or when a linked MR merges or closes. Lifecycle messages are factual, immutable server-owned templates with only a canonical MR URL as dynamic content, delivered through the same durable queue as GitHub's. |
...
| Language servers | Limited | TypeScript/JavaScript, Go, Rust, Python, and experimental Kotlin use one task-owned server per language, shared across that task's sessions, editors, and browser reconnects. The task aggregate remains actionable away from a matching file; each language has an independent collapsible, and **Settings > General > Editors** can hide languages from aggregate status without changing lifecycle state. The task top bar and phone/tablet Status drawer remain discoverable fallbacks, including when all languages are hidden, while a matching editor shortcut still exposes its current language and the phone file viewer does not attach or auto-start. Per-task Inherit default, Keep warm, and Disabled policies drive Start/Stop/Restart; global auto-start/installation defaults remain off and Kotlin is manual-only. Status distinguishes process start, `initialize`, server-reported project work, ready/idle, stopping, and errors without inventing an indexing ETA. Local PC and Local Docker are supported; SSH, Sprites, and remote Docker fail safely without launching resources. |
