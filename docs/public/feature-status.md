...
| Add workspace sources | Supported | An idle, non-archived repository-backed task can use **Files → Add sources** for an atomic batch of saved repositories, local/remote Git repositories, and, on Local/Local PC or Worktree only, arbitrary local folders. Repository sources also work on Local Docker, SSH, and Sprites. |
