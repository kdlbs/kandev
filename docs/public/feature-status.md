...
| Add workspace sources | Supported | An idle, non-archived repository-backed task can use **Files → Workspace actions → Add sources** for an atomic batch. **Local** shares task creation's saved/discovered local-repository selector and offers local folders only on Local/Local PC or Worktree; **Remote** uses provider-backed or pasted-URL repository selection. Repository sources also work on Local Docker, SSH, and Sprites. |
