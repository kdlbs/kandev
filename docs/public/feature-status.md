...
| GitHub integration | Dependency-bound | Workspace repository scope, issue/PR browsing, actions, watches, presets, reviews, checks, and Gists depend on GitHub auth, scopes, rate limits, and repository permissions. An administrator can grant a coordinator one exact linked-PR fresh-CI recovery operation; it requires a repository-scoped GitHub App installation with Actions write, reruns first, denies fork dispatch, and exposes no credential or arbitrary workflow/ref/input selection. Supported Local Docker, Sprites, and SSH launch paths may receive the globally stored token as `GITHUB_TOKEN`/`GH_TOKEN`; workspace repository scope does not constrain its privileges. |
