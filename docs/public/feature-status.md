...
| Configuration Chat | Limited | Uses an ephemeral repository-less task plus configuration MCP. Closing its browser tab does not delete the backing task, and config-mode tasks are excluded from Quick Chat expiry. |
...
| GitHub integration | Dependency-bound | Workspace repository scope, issue/PR browsing, actions, watches, presets, reviews, checks, and Gists depend on GitHub auth, scopes, rate limits, and repository permissions. Local Docker, Sprites, and SSH launches may receive the globally stored token as `GITHUB_TOKEN`/`GH_TOKEN` (as does the unimplemented Remote Docker path); workspace repository scope does not constrain its privileges. |
