...
| GitHub integration | Dependency-bound | Each workspace selects one PAT, named `gh` account, or App installation for automation and may add a personal App identity. Managed task access uses repository-bound broker leases; App tokens are minted for that repository, while redeemed PAT/CLI bearer tokens retain their provider scopes. Explicit profile `GITHUB_TOKEN`/`GH_TOKEN` values remain unmanaged overrides. |
