...
| Multi-repository tasks | Limited | New Task supports multiple local or remote rows on Worktree, Local Docker, SSH, and Sprites; Local/Local PC creation remains gated and Remote Docker is not implemented. Changes/Review/PR surfaces group by repository. |
| Add workspace sources | Supported | An idle, non-archived repository-backed task can use **Files → Workspace actions → Add Repositories to workspace** for an atomic batch. **Local** shares task creation's saved/discovered local-repository selector, refresh, and create-repository actions, and offers local folders only on Local/Local PC or Worktree; **Remote** uses provider-backed or pasted-URL repository selection. Repository sources also work on Local Docker, SSH, and Sprites. |
...
| Task and work-item references in chat | Dependency-bound | Structured Task Chat and Quick Chat search workspace tasks plus connected Jira, Linear, GitHub, GitLab, Azure DevOps, and Sentry sources with `#`. Passthrough chat remains literal, and provider scope, credentials, availability, and rate limits determine external results. |
...
| SSH executor | Limited | Remote sessions support trusted Linux or macOS hosts on amd64 or arm64, with public-key authentication, SFTP, TCP forwarding, and repository-source materialization. It ignores profile prepare/cleanup scripts and retains the remote task directory for manual cleanup. |
...
| Capability | Status | Current boundary |
|---|---|---|
| Task-scoped Kandev MCP | Dependency-bound | Injected into compatible task sessions with scoped task/session, plan, walkthrough, workspace-source, workflow-step, and coordination tools. `add_workspace_sources_kandev` adds an atomic source batch to an idle task; the legacy `add_branch_to_task_kandev` remains available. |
