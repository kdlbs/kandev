...
| Language servers | Limited | TypeScript/JavaScript, Go, Rust, Python, and experimental Kotlin use one task-owned server per language, shared across that task's sessions, editors, and browser reconnects. Tasks that share a physical environment retain separate processes, policy, progress, and roots. With **Show status bar** enabled, the task aggregate remains actionable away from a matching file; each language has an independent collapsible, and **Settings > General > Editors** can hide languages from aggregate status without changing lifecycle state. The task top bar and phone/tablet Status drawer remain discoverable fallbacks, including when the status bar is off or all languages are hidden, while a matching editor shortcut still exposes its current language and the phone file viewer does not attach or auto-start. Per-task Inherit default, Keep warm, and Disabled policies drive Start/Stop/Restart; global auto-start/installation defaults remain off and Kotlin is manual-only. Status distinguishes process start, `initialize`, server-reported project work, ready/idle, stopping, and errors without inventing an indexing ETA. Physical workspace-root changes require explicit Restart, and browser document traffic is contained to authorized task roots. Local PC and Local Docker are supported; SSH, Sprites, and remote Docker fail safely without launching resources. |
