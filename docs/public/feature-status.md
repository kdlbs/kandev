...
| GitHub pull-request review, checks, merge, and task PR automation | Dependency-bound | Requires GitHub credentials and repository permissions. GitHub-only task-level controls apply to every linked PR and can auto-fix feedback, auto-merge, and prompt agents when your review is requested, or when a PR merges or closes. Lifecycle messages use immutable server-owned templates with only a canonical PR URL as dynamic content. Review-request matching uses the installation-wide connected GitHub account and a quiet baseline; delivery uses an existing task session or queue and never creates a new session. GitLab lifecycle parity is not shipped. Azure has a create-PR path only; other Git hosts do not get the full task PR panel. |
