...
For an idle, non-archived repository-backed task, **Files → Workspace actions → Add Repositories to workspace** opens shared task-create selectors: **Local** includes saved workspace and discovered local Git repositories with refresh and create-repository actions, plus arbitrary local folders on Local/Local PC or Worktree; **Remote** includes provider-backed repositories and pasted repository URLs. Every repository chooses one base branch, and Local/Local PC uses the current checkout without switching it. Desktop uses a dialog; phones use a full-height drawer. A mixed submission is atomic, and repository additions refresh repository-aware tools while folders remain Files-only. See [Tasks and workflows](tasks-and-workflows.md#add-sources-to-an-existing-task).
