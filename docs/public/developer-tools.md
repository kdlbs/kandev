...
For an idle, non-archived repository-backed task, **Files → Workspace actions → Add sources** opens shared task-create selectors: **Local** includes saved workspace and discovered local Git repositories, plus arbitrary local folders on Local/Local PC or Worktree; **Remote** includes provider-backed repositories and pasted repository URLs. Every repository has a base branch; Worktree, Docker, SSH, and Sprites can use an optional existing checkout branch for repository rows, while Local/Local PC uses the current checkout without switching it. Desktop uses a dialog; phones use a full-height drawer. A mixed submission is atomic, and repository additions refresh repository-aware tools while folders remain Files-only. See [Tasks and workflows](tasks-and-workflows.md#add-sources-to-an-existing-task).
