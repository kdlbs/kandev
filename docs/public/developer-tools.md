...
Open **Task tools** in the workbench top bar to choose a layout, open the selected session worktree in an editor, or open its folder. The split-editor action opens your default editor; its menu lets you choose another configured editor. The folder shortcut sits beside it and works independently of editor configuration. A file's **Open with** menu can also open a specific editor, copy the path, or ask the operating system to show the folder.

The task title, workflow progress, linked review status, and panel visibility remain directly accessible in the top bar. Human assignment uses an avatar or unassigned icon; hover or focus to see the current name, or open it to assign the task. When host metrics are enabled and the status bar is off, **System metrics** opens the readings without filling the header with meters. Touch devices use a drawer for these disclosures. Phones retain their native task menu, Files actions, and Status drawer.
