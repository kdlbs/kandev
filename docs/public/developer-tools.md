...
Inline scripts run in a restricted worker runtime with a virtual document. The preview can update virtual nodes and respond to supported pointer and form-control events, but source scripts and inline handlers never execute in the Kandev page. Select **Show code** if the runtime stops because the document uses an unsupported capability or exceeds a safety limit.

The preview uses a controlled renderer. Embedded `data:` resources and preview-owned `blob:` resources are allowed; workspace-relative and remote resources are not. Links, forms, refresh metadata, popups, downloads, browser navigation, storage, workers, and network APIs are unavailable. For a multi-file site, start a development server and open it in the Browser panel instead.
