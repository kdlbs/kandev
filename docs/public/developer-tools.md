...
Auto-start and auto-install are off for every language by default. These global settings are defaults; each task and language has its own **Inherit default**, **Keep warm**, or **Disabled** policy. Bounded project discovery can activate an inherited default without requiring you to open a matching file, but discovery only inspects names and extensions—it does not install or start a server or evaluate project files.
...
With the Application status bar enabled, its compact task item remains visible while a relevant language is detected, configured, running, importing, queued, or failed—even when an unsupported file or another panel is active. Open it to see every relevant language, policy, lifecycle phase, reported work, elapsed time, generation, last action evidence, errors, and controls. The task top bar provides the same always-discoverable control when the status bar is disabled or no supported file is active, and a supported Monaco editor keeps a current-language shortcut.

Phone and touch-oriented tablet layouts expose the same task-level state and controls in the existing **Status** drawer with touch-sized actions. The phone file viewer itself does not attach to or silently start a language server.
