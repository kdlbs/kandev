...
Kubernetes detection makes the default log format JSON. Kandev writes daily files under `/data/logs/` and emits warn-and-above to stdout by default; ensure the home path is persistent if the three-day file history must survive pod replacement.
