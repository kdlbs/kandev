...
2. Select the workspace and workflow when Kandev cannot infer them. A regular non-ephemeral task must belong to a workflow. When the workflow selector shows a selected workflow, **Starts in** identifies the step that an immediate **Start task** action will enter. Kandev checks the first positional step with **Auto-start agent**, then the configured **Start step**, then the first step.
3. Select a source:
