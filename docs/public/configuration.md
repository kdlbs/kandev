...
| `KANDEV_LSP_MAX_SERVERS` | `8` | Positive integer process-wide cap for actual task/language servers. Browser attachments and editor mounts do not consume slots; invalid/non-positive uses default. |
| `KANDEV_LSP_MAX_CONNECTIONS` | unset | Deprecated fallback for `KANDEV_LSP_MAX_SERVERS`, read only when the new variable is unset. Positive values retain the old deployment's numeric cap; invalid/non-positive uses `8`. |
