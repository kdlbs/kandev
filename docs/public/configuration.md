...
| `features.app_status_bar` | `KANDEV_FEATURES_APP_STATUS_BAR` | `true` in production | Global connection/metrics/plugin status surface: bottom bar on tablet/desktop and Status drawer on phones. |
...
| `features.appStatusBar` | `KANDEV_FEATURES_APP_STATUS_BAR` | on | Global status bar on tablet/desktop and Status drawer entry on phones. Disabling changes visibility only. |
