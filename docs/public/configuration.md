...
| `limits.lspMaxConnections`                 | `KANDEV_LSP_MAX_CONNECTIONS`                      | positive integer, `8`             | Maximum active and detached task-host language-server leases when browser continuity is enabled. At capacity, Kandev evicts the least-recently detached lease; if all leases are attached, it rejects the new request.                                                                                                                                                                                                                                                                                                                     |
...
| `limits.lspMaxConnections`           | `KANDEV_LSP_MAX_CONNECTIONS`                 |      `8` | Positive integer cap for active and detached task-host language-server leases when continuity is enabled; Kandev evicts the least-recently detached lease or rejects the request if all leases are attached. Invalid/non-positive uses default.                                                      |
