...
Use `get_message_delivery_kandev(delivery_id)` to inspect a receipt's state, attempts, queue entry, timestamps, and safe failure code. The stored prompt is never returned. If bounded delivery ends as `recoverable`, its exact source or target session can call `retry_message_delivery_kandev(delivery_id)` to restart server-side admission. Delivered, cancelled, and terminal failures cannot be revived.
