...
Use `get_message_delivery_kandev(delivery_id)` to inspect a receipt's state, attempts, queue entry, timestamps, and safe failure code. The stored prompt is never returned. If bounded delivery ends as `recoverable`, its exact source or target session can call `retry_message_delivery_kandev(delivery_id)` to restart server-side admission. An `ambiguous` receipt means Kandev cannot prove whether the executor already accepted the prompt; it stays inspectable but retry is rejected to avoid a duplicate. Delivered, cancelled, and terminal failures cannot be revived.
...
| Stop the current approach and give replacement work now | `message_task_kandev` with `delivery_mode: "interrupt"`          | The direct parent requests immediate cancellation and redispatch. Scheduling reports the message as queued; only executor acceptance changes its receipt to delivered. |
