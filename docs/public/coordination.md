...
Use `get_task_conversation_kandev` to read a primary or explicit session conversation. It supports limits, before/after cursors, ascending or descending order, and message-type filters. When a task has more than one session, use `list_task_sessions_kandev` to list them (newest first, flagging the primary and your own) and pass the `session_id` you want. Use `list_related_tasks_kandev` to list a task's parent, direct children, siblings, stored blocker relationships, and associated GitHub pull requests. Tasks normally inspect only their own related tree. The persisted Office Coordinator can also inspect compact trees for unrelated tasks in its workspace, but cannot use that access to read descriptions or task documents.
