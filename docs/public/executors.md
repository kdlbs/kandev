...
Saved executor connection settings are different from the recorded workload snapshot. Current kubeconfig/in-cluster credentials, context, namespace, and timeout are used to reach an existing session; changing them can restore or break reconnect and cleanup, and the namespace must still match the recorded resource. Current Pod template, image, platform, main container, and storage settings affect new sessions only. If Kandev must replace a missing Pod, it uses the recorded workload snapshot rather than the edited profile.
...
- **Kubernetes session cannot resume or clean up:** restore the current executor's cluster credentials and namespace, then compare the recorded Pod/PVC UID and complete ownership labels. Do not delete a same-name object.
