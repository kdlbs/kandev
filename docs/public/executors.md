...
The profile editor exposes remote shell and agent-readiness checks. Backend/API configuration also recognizes `ssh_workdir_root` (default `~/.kandev`), `ssh_workdir_policy`, and `ssh_shell`; the current profile UI exposes `ssh_shell` but not the workdir-root fields.

> **Coder deployments:** configure `ssh_workdir_root` on a durable writable mount and set `ssh_workdir_policy=durable`. Do not rely on a Coder home, `/tmp`, or a template checkout that is rebuilt between workspace starts. Suitable mounted roots include `/work/.kandev` and `/opt/jumprope-fullstack/.kandev`. Before creating the task directory or starting the agent, Kandev detects Coder, rejects missing or non-writable roots, and performs an isolated write probe. Kandev then creates each task checkout below this root and requires the task directory itself, not an ancestor checkout, to be the Git repository root. `ssh_workdir_policy=allow_ephemeral` explicitly accepts the data-loss risk for a home or temporary root, but cannot bypass availability and write checks.
