...
> **Coder deployments:** configure `ssh_workdir_root` on a durable writable mount. Do not rely on a Coder home or template checkout that is rebuilt between workspace starts. Suitable mounted roots include `/work/.kandev` and `/opt/jumprope-fullstack/.kandev`. Kandev creates each task checkout below this root and requires the task directory itself, not an ancestor checkout, to be the Git repository root.
