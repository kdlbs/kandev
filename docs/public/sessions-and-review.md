...
The default pending-message limit is 10 per session. An admin can change it live under **Settings > General > Message Queue**; `0` removes the cap. `KANDEV_QUEUE_MAX_PER_SESSION` takes precedence and makes the UI field read-only; changing the environment still requires a restart. Lowering the saved limit does not delete entries already waiting. Interrupt delivery is restricted to a direct parent task messaging its child. Other senders must queue.
