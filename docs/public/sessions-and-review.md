...
**PR Details** is a reusable Layouts panel. The built-in Default omits it until the active task has a linked GitHub pull request or GitLab merge request; then Kandev adds it beside **Agent** as an inactive tab. You can add PR Details explicitly in the Layouts editor, move it to another group, or remove it with the same controls. A panel supplied by the selected profile or a saved task layout remains as an empty panel when no review is linked, while closing a conditionally added tab prevents it from reappearing in that session. Changing the default applies to task environments without a saved task-specific layout and **Reset Layout**, not a layout already saved for a task. Removing Terminal from the Default layout also prevents Kandev from creating its initial user shell.
