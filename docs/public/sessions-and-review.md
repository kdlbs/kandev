...
Open **Settings > General > Appearance > Pull Request Tabs** to choose where GitHub PR tabs open on desktop. **Agent pane** places them beside the active agent; **Right pane** uses the designated right content group, which contains Files and Changes in the default layout. If that group is unavailable, Kandev uses the visually furthest-right non-Agent group and does not create a new split. Saving a different choice does not move open PR tabs, but explicitly opening an existing PR again moves and focuses that exact tab. The setting is editable on mobile, but the mobile task **Review** surface is unchanged.
