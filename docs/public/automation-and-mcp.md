...
`get_message_delivery_kandev` returns a receipt's delivery state and safe operational fields, never its prompt. `retry_message_delivery_kandev` reopens only a `recoverable` receipt for bounded server-side retry. Both tools require the exact source or target session recorded on the receipt.
