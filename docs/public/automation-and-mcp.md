...
A same-task message requires the sibling's session ID. Normal messages can cross workspaces when the sender knows the full task ID. Every task-mode call with an active source turn receives a durable `delivery_id`, including queued busy delivery, WAITING/CREATED starts, parent-question replies, and authorized interrupts. Send an `idempotency_key` to make a repeated call in the same source turn return that receipt instead of creating another delivery. A full target queue reports `delivery_status: "pending_capacity"` on the accepted receipt and retries server-side, so the sender should not poll or resend the report. When a direct child must abandon its current approach and receive replacement work now, its parent should use `message_task_kandev` with `delivery_mode: "interrupt"`; another sender receives a hard error rather than a silent downgrade, and a request that cannot dispatch safely remains queued.

`get_message_delivery_kandev` returns a receipt's delivery state and safe operational fields, never its prompt. Interrupt scheduling remains `queued`; it becomes `delivered` only after the executor accepts the FIFO prompt. `retry_message_delivery_kandev` reopens only a `recoverable` receipt for bounded server-side retry. An `ambiguous` receipt means an executor may already have accepted the prompt while its terminal acknowledgement could not be confirmed; it remains available for inspection but cannot be retried automatically or through this tool. Both tools require the exact source or target session recorded on the receipt.
...
`settle_stale_session_kandev` is an exact-turn recovery tool. It accepts `session_id` and `turn_id`, and succeeds only when the server can prove an eligible completion intent has become quiet. It is available only to another session on the same task, the target task's direct parent, or the verified task that spawned the session, all within one workspace. A live, recent, reserved, successor, or otherwise ambiguous turn returns `active_turn/not_stale` without changing it.
