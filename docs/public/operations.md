...
- **Office session identity**: experimental, high risk, and on in every profile by default. It gives each Office participant a separate task conversation and requires a restart. Pre-existing duplicate `(task_id, agent_profile_id)` rows stay safe by selection, not migration; disabling the toggle restores the pre-graduation runner-seat binding and task-active-session decision re-evaluation.
