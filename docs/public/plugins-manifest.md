...
| `webhooks[].public` | no | bool | Defaults to `false`. When `false` (the default), kandev requires a real caller identity (a session cookie or PAT, or the synthetic identity while authentication is disabled) before relaying to this webhook — an anonymous request gets `401`. Set to `true` only when the plugin itself authenticates the caller (an external provider's HMAC/signing secret, an IdP redirect flow) and the webhook must be reachable with no kandev credential at all — for example a third-party callback (Slack, GitHub, Jira) or an `auth_providers[].initiate`/callback pair. kandev can only enforce that *a* caller identity is present; it cannot verify that a `public: true` webhook actually authenticates its callers, so this is the plugin author's assertion, not a kandev-verified guarantee. |
| `auth_providers[]` | no | object[] | Login buttons this plugin contributes to the pre-auth login screen (needs `capabilities.auth`). Each is `{ id, display_name, initiate }`, where `initiate` names one of the plugin's `webhooks[].key` values — the button navigates to that webhook, which 302-redirects to the IdP. **That webhook (and the IdP callback key it redirects back to) must declare `public: true`**: the visitor clicking the button has no session or PAT yet, so a non-public initiate webhook would `401` immediately and kandev silently omits the provider from `auth.ssoProviders` rather than surface a login button that 401s. |
