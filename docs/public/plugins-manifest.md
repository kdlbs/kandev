...
webhooks:
  - key: "slack-events"
    ...
    access: public # opt-in anonymous access; authenticated is the default
...
| `webhooks[].access` | no | string | `authenticated` (default) requires a Kandev identity; `public` allows anonymous callers only for plugin handlers that validate their own third-party caller. Cookie-authenticated browser calls remain subject to Kandev's same-origin policy. |
