...
- Managed GitHub task access may remain the saved policy for historical workspaces. Upgrades preserve whichever task Git policy is saved. Managed mode uses task/repository-bound broker leases from the workspace automation connection. New workspaces default to **Inherit executor Git credentials**. GitHub App tokens are minted for the redeemed repository. PAT and named-CLI bearer tokens retain all provider-granted scopes after delivery to the trusted agent subprocess. Personal tokens and the App private key never enter executors. Select **Inherit executor Git credentials** in the workspace GitHub settings for host-visible Local/Worktree credentials or credentials configured in remote executors.
