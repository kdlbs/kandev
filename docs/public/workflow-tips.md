...
| Prompt | Step-specific agent prompt. When you add content to an empty prompt, the editor enables **Auto-start agent**. You can then change **Auto-start agent** independently. `{{task_prompt}}` inserts the task description. Without this placeholder, the step prompt replaces the description. Type `@` to reference a saved prompt by name. |
