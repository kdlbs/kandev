# Office Wave 6: Routines, Org Chart & Notifications

**Date:** 2026-04-26
**Status:** proposed
**Specs:** `office-routines`, `office-overview` (org chart)
**UI Reference:** `docs/plans/2026-04-office-ui-reference.md` (routines page, org chart, routine rows)
...
**Repository** (`internal/office/repository/sqlite/routines.go`):
- Full CRUD for `office_routines`, `office_routine_triggers`, `office_routine_runs`
...
**Service** (`internal/office/service/routines.go`):
...
**Webhook endpoint** (`internal/office/handlers/webhook.go`):
- `POST /api/v1/office/routine-triggers/:publicId/fire`
...
**Variable interpolation** (`internal/office/service/variables.go`):
...
- Click a node to navigate to `/office/agents/[id]`
...
**Wire `office.inbox_item` to notification service:**
- In `cmd/kandev/gateway.go`: subscribe to `office.inbox.item` events
- Call existing notification service `HandleEvent()` with event type `office.inbox_item`
- Build notification message: type, summary, deep link to `/office/inbox`
- Default: auto-subscribe Local and System providers to `office.inbox_item`
- Add `office.inbox_item` to notification events constant list
