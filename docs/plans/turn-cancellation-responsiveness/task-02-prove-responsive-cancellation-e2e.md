...
- The existing desktop delayed-turn scenario proves the real cancel control and input settle within two seconds of activation while enabled and disabled workflow completion policies still produce their existing single outcomes.
- The existing `mobile-chrome` scenario invokes the shared control by touch and proves the same two-second outcome without changing mobile composition, touch geometry, scroll ownership, or overflow behavior.
...
cd apps/web && pnpm e2e:run --no-build --project mobile-chrome tests/workflow/mobile-workflow-cancel-completion.spec.ts -- --repeat-each=3 --workers=1
```
