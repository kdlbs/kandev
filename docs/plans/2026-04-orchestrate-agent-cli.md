# Orchestrate Agent CLI & MCP Mode Implementation Plan

**Date:** 2026-04-27
**Status:** proposed
**Spec:** `docs/specs/orchestrate-agent-cli/spec.md`
...
- `task get [--id ID]` → `GET /api/v1/orchestrate/tasks/:id` (defaults to `$KANDEV_TASK_ID`)
- `task update [--id ID] --status S [--comment C]` → `PATCH /api/v1/orchestrate/tasks/:id`
...
- `comment add --task ID --body BODY` → `POST /api/v1/orchestrate/tasks/:id/comments`
- `comment list --task ID [--limit N] [--after CID]` → `GET /api/v1/orchestrate/tasks/:id/comments`
...
- `agents list [--role R] [--status S]` → `GET /api/v1/orchestrate/workspaces/:wsId/agents`

### 1.6 Memory commands

**File:** `cmd/agentctl/kandev_memory.go` (NEW)

- `memory get [--layer L] [--key K]` → `GET /api/v1/orchestrate/agents/:id/memory` or `GET .../memory/:layer/:key`
- `memory set --layer L --key K --content C` → `PUT /api/v1/orchestrate/agents/:id/memory`
- `memory summary` → `GET /api/v1/orchestrate/agents/:id/memory/summary`
...
## Phase 2: ModeOrchestrate for MCP
...
### 2.2 Register orchestrate tools
...
### 2.3 Set mode for orchestrate sessions

**File:** `internal/orchestrator/executor/executor_execute.go`

When launching an orchestrate agent session, set `McpMode = ModeOrchestrate`:

```go
if isOrchestrateSession(session) {
    req.McpMode = mcp.ModeOrchestrate
}
```

The `isOrchestrateSession()` check looks at the task's workflow -- orchestrate tasks use the system "Orchestrate" workflow. Or add a flag to the session/execution metadata.
...
Add test:
```go
func TestModeOrchestrate(t *testing.T) {
    s := New(newTestLogger(t), nil, "orchestrate", false)
...
### 3.2 Pass path through to orchestrate service

**File:** `internal/orchestrate/service/service.go`
...
**File:** `internal/orchestrate/service/env_builder.go`
...
**File:** `internal/orchestrate/configloader/skills/kandev-protocol/SKILL.md`
...
## Phase 5: Executor-aware skill & instruction delivery

### 5.1 Skill manifest model

**File:** `internal/orchestrate/service/skill_manifest.go` (NEW)
...
Built by `buildSkillManifest()` during session prep, before executor selection.

### 5.2 Refactor scheduler_integration.go

**File:** `internal/orchestrate/service/scheduler_integration.go`

Currently `prepareRuntime()` and `InjectSkillsForAgent()` are called unconditionally. Refactor:
...
func (si *SchedulerIntegration) processWakeup(ctx context.Context, wakeup *models.WakeupRequest) {
    ...
    // Deliver skills based on executor type
...
### 5.3 Executor-specific delivery

**File:** `internal/orchestrate/service/skill_delivery.go` (NEW)
...
func (si *SchedulerIntegration) deliverSkills(
    ...
    default:
        return si.deliverSkillsLocal(ctx, manifest) // fallback
    }
}
```

**Local delivery** (`deliverSkillsLocal`):
- Same as current: write to `~/.kandev/runtime/`, create symlinks on host
- Returns host path: `~/.kandev/runtime/<ws>/instructions/<agentId>/`

**Docker delivery** (`deliverSkillsDocker`):
- Write to `~/.kandev/runtime/` on host (same as local)
- The runtime dir will be mounted into the container (see 5.4)
- Return the container-side path: `~/.kandev/runtime/<ws>/instructions/<agentId>/`
- Symlinks created inside container via prepare script snippet (see 5.5)

**Sprites delivery** (`deliverSkillsSprites`):
- Write files to a local staging area
- The executor's `CreateInstance` step uploads them (see 5.6)
- Return the sprite-side path: `/root/.kandev/runtime/<ws>/instructions/<agentId>/`

### 5.4 Docker: mount runtime dir (orchestrate sessions only)

**File:** `internal/agent/lifecycle/container.go`

Add the kandev runtime directory as a volume mount, only when the orchestrate scheduler passes it via metadata:

```go
// In container config, when orchestrate metadata is present:
if runtimeDir := req.Metadata["kandev_runtime_dir"]; runtimeDir != "" {
...
Normal kanban tasks never set `kandev_runtime_dir` -- the metadata key is only populated by the orchestrate scheduler. No behavior change for existing sessions.

### 5.5 Docker & Sprites: symlink creation script

**File:** `internal/orchestrate/service/skill_delivery.go`

Generate a shell snippet that creates skill symlinks inside the execution environment:

```go
func buildSymlinkScript(manifest *SkillManifest, runtimeDir string) string {
    var sb strings.Builder
    // Create target dirs
    for _, targetDir := range agentSkillDirs(manifest.AgentTypeID) {
        sb.WriteString(fmt.Sprintf("mkdir -p %s\n", targetDir))
        for _, skill := range manifest.Skills {
            skillPath := filepath.Join(runtimeDir, manifest.WorkspaceSlug, "skills", skill.Slug)
            sb.WriteString(fmt.Sprintf("ln -sf %s %s/%s\n", skillPath, targetDir, skill.Slug))
        }
    }
    return sb.String()
}
...
For Docker: append to the prepare script via `KANDEV_SKILL_SETUP` env var.
For Sprites: run as a command after file upload.

### 5.6 Sprites: upload skills in setup flow (orchestrate sessions only)

**File:** `internal/agent/lifecycle/executor_sprites_operations.go`

Add a new step between credential upload and prepare script. The step is a no-op when no manifest is present (i.e. normal kanban tasks):

```go
func (r *SpritesExecutor) uploadSkillFiles(
    ctx context.Context, sprite *sprites.Sprite, req ExecutorCreateRequest,
) error {
    manifest := req.Metadata["skill_manifest"] // deserialized SkillManifest
    if manifest == nil { return nil } // No-op for normal kanban tasks

    for _, skill := range manifest.Skills {
        path := fmt.Sprintf("/root/.kandev/runtime/%s/skills/%s/SKILL.md",
            manifest.WorkspaceSlug, skill.Slug)
        r.writeFileWithRetry(ctx, sprite, path, []byte(skill.Content), 0o644)
    }
    for _, instr := range manifest.Instructions {
        ...
        r.writeFileWithRetry(ctx, sprite, path, []byte(instr.Content), 0o644)
    }

    // Create symlinks
    script := buildSymlinkScript(manifest, "/root/.kandev/runtime")
    sprite.CommandContext(ctx, "bash", "-c", script).Output()

    return nil
}
```

Progress step changes from 7 to 8 (new "Upload skills" step between credentials and prepare script).
...
**File:** `internal/orchestrate/handlers/middleware.go` (NEW)
...
Apply to the orchestrate route group in `handlers.go`:
...
**File:** `internal/orchestrate/service/permissions.go` (NEW)
...
**File:** `internal/orchestrate/service/agents.go` (MODIFY)
...
**File:** `internal/orchestrate/handlers/middleware.go`
...
**File:** `internal/orchestrate/handlers/meta.go` + `internal/orchestrate/dto/meta.go`
...
**File:** `apps/web/app/orchestrate/agents/[id]/components/agent-permissions-tab.tsx` (NEW)
...
**File:** `apps/web/app/orchestrate/agents/[id]/page.tsx` (MODIFY)
...
| `TestModeOrchestrate` | 5 tools registered, kanban tools excluded |
| `TestModeOrchestrate_ExcludesKanban` | create_task, update_task, list_tasks NOT present |
| `TestEnvBuilder_KandevCLI` | KANDEV_CLI set in orchestrate env vars |
| `TestSkill_CLIExamples` | Skill references $KANDEV_CLI, no curl commands |
| `TestDeliverSkillsLocal` | Writes to runtime dir, creates host symlinks |
| `TestDeliverSkillsDocker` | Writes to runtime dir, returns container-side path |
| `TestDeliverSkillsSprites` | Builds manifest, returns sprite-side path |
| `TestBuildSymlinkScript` | Generates correct symlink commands per agent type |
...
| `internal/agentctl/server/mcp/server.go` | MODIFY: add ModeOrchestrate constant + case |
| `internal/agentctl/server/mcp/server_test.go` | MODIFY: add orchestrate mode tests |
| `internal/orchestrator/executor/executor_execute.go` | MODIFY: set ModeOrchestrate for orchestrate sessions |
| `internal/agentctl/client/launcher/launcher.go` | MODIFY: expose BinaryPath() |
| `internal/orchestrate/service/service.go` | MODIFY: add agentctlBinaryPath field |
| `internal/orchestrate/service/env_builder.go` | MODIFY: inject KANDEV_CLI per executor type |
| `internal/orchestrate/service/skill_manifest.go` | NEW: SkillManifest model + builder |
| `internal/orchestrate/service/skill_delivery.go` | NEW: executor-aware delivery (local/docker/sprites) |
| `internal/orchestrate/service/scheduler_integration.go` | MODIFY: use manifest + delivery instead of unconditional export |
| `internal/agent/lifecycle/container.go` | MODIFY: add kandev_runtime_dir mount for orchestrate |
| `internal/agent/lifecycle/executor_sprites_operations.go` | MODIFY: add uploadSkillFiles step |
| `internal/orchestrate/configloader/skills/kandev-protocol/SKILL.md` | REWRITE: curl → CLI examples |
| `cmd/kandev/main.go` | MODIFY: pass agentctl binary path to orchestrate service |
| `internal/orchestrate/handlers/middleware.go` | NEW: JWT auth middleware + task scope check |
| `internal/orchestrate/service/permissions.go` | NEW: ResolvePermissions, HasPermission, validateNoEscalation |
| `internal/orchestrate/service/agents.go` | MODIFY: add permission checks to create/update/delete |
| `internal/orchestrate/handlers/handlers.go` | MODIFY: apply auth middleware to route group |
| `internal/orchestrate/handlers/meta.go` | MODIFY: add permissions + defaults to meta response |
| `internal/orchestrate/dto/meta.go` | MODIFY: add PermissionMeta type |
