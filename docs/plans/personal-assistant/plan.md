...
Delivery task 01 adds an independent assistant flag before coordinator-only dogfooding. Preserve retained-owner/context guards even with that flag off. Continue tasks 06–11 in dependency order; these work orders now include canonical requirement/criterion references and detailed remaining checks. Tasks 01/02 below are historical completed work, not work to redo.
