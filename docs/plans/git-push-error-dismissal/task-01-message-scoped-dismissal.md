...
acceptance_criteria:
  ...
  - AC-TASKS-GIT-PUSH-ERROR-DISMISSAL-001.8
system_design:
...
- Add `git_operation_error_dismissed_at` to the existing message metadata through a session-serialized first-write transaction, preserve all diagnostic fields, and publish the existing broadcast only for the winning write; return the existing timestamp without another write/event for concurrent or repeated requests.
- Render Dismiss only on rows with `type: error`, `git_operation_error: true`, and `operation: push`, including existing legacy rows; keep it alongside recovery controls when both apply, and hide the full row only after a persisted timestamp arrives through the broadcast or success response.
...
- A successful dismissal survives reload and task switching, while a later push failure stays visible; concurrent requests share one stored timestamp and one update event; a missed update broadcast is recovered from the success response; and a failed save shows the localized toast, leaves the original card and both controls visible/enabled after pending state clears, and permits a successful retry.
...
The desktop and mobile browser specs each seed a legacy-shaped failure, dismiss it through the UI, assert the live card disappears, open a fresh page to force server-backed hydration, switch away and back, then seed a later message and assert that the later diagnostics and Fix action remain visible. The mobile spec also asserts Dismiss is visible without hover and has a touch target of at least 44 pixels. Backend tests assert the exact persisted timestamp and preserved metadata, sequential and concurrent idempotence, write failure, strict message eligibility, forbidden-session and cross-workspace denial, sessionless and unrelated-message rejection, and zero persistence calls/events for every rejected request. The focused action-message test makes the dismissal request fail, asserts the translated toast and both enabled controls, then retries successfully and asserts the success response hides the full card without a broadcast; it also switches sessions while a request is in flight and proves the acknowledgment updates only the original session's message. The session-store regression proves a stale message update cannot erase the acknowledged timestamp, and the action row is covered with a combined recovery action. The existing `task:dismiss` and `common:requestFailed` keys cover all six supported locales, so the i18n check is the locale gate and no locale files should change. The public Git operations page receives one manual-dismissal sentence and is validated with both public-doc commands above.
...
- Message update publication is best effort after persistence, so the active client must apply the persisted success response and browser verification must still cover the authoritative state after reload.
...
PR feedback follow-up: `(cd apps/web && pnpm exec vitest run components/task/chat/messages/action-message.test.tsx lib/state/slices/session/session-slice.update-messages.test.ts)` passes 42 tests across two files. The new store regression first failed because an older message update erased the dismissal marker, then passed after the existing store merge began retaining the acknowledged timestamp. The action tests prove response-only dismissal after retry, failed-request visibility, and original-session targeting when the user switches sessions while a request is pending. Targeted ESLint, `pnpm run typecheck`, `pnpm run i18n:ratchet`, `pnpm run i18n:check`, `python3 scripts/list-docs.py validate`, and `python3 scripts/lint-spec-files.py --all` pass. The focused browser specs were not rerun because this fix changes response and store handling without changing layout or interaction controls; the new component/store regressions cover the affected paths.
