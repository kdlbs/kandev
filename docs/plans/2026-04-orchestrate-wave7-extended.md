# Orchestrate Wave 7: Assistant, Channels, Memory, Config Sync

**Date:** 2026-04-26
**Status:** proposed
**Specs:** `orchestrate-assistant`, `orchestrate-config`
**UI Reference:** `docs/plans/2026-04-orchestrate-ui-reference.md` (settings page for config sync)
**Depends on:** Wave 5 (inbox, approvals), Wave 6 (routines for proactive work)

## Problem

The core orchestrate system is functional after Waves 1-6. Wave 7 adds the extended features: personal assistant with external chat channels, persistent agent memory, agent self-improvement, and configuration portability.
...
**Repository** (`internal/orchestrate/repository/sqlite/channels.go`):
- Full CRUD for `orchestrate_channels`
...
**Channel task creation** (`internal/orchestrate/service/channels.go`):
...
**Webhook ingress** (`internal/orchestrate/handlers/channel_webhook.go`):
...
**Outbound relay** (`internal/orchestrate/service/channel_relay.go`):
...
**Frontend:**
- Channels tab on agent detail page (`/orchestrate/agents/[id]`):
...
**Repository** (`internal/orchestrate/repository/sqlite/memory.go`):
- Full CRUD for `orchestrate_agent_memory`
...
- `GET /api/v1/orchestrate/agents/:id/memory?layer=&key=`
- `PUT /api/v1/orchestrate/agents/:id/memory` -- create or update (upsert by agent+layer+key)
- `DELETE /api/v1/orchestrate/agents/:id/memory/:entryId`
- `GET /api/v1/orchestrate/agents/:id/memory/summary` -- for session bootstrap
...
**Backend** (`internal/orchestrate/service/self_improvement.go`):
...
**Export** (`internal/orchestrate/service/config_export.go`):
...
**Import** (`internal/orchestrate/service/config_import.go`):
...
**Frontend** (`/orchestrate/company/settings`):
