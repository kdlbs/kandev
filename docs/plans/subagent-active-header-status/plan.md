...
- Do not change `isSubagentEffectivelyActive`, `SubagentMetaRow`, or when UUID/model/duration chips appear (`!isActive`).
...
- **What:** active card with children still shows Working + Loading spinner; complete card shows neither and no success check; failed/cancelled show the labelled red mark.
...
- **Scenario:** GIVEN `/e2e:subagent` completes, WHEN the card is visible, THEN there is no Loading status, no Working copy, no success check, and the metadata row is visible.
...
- `pnpm --filter @kandev/web test -- components/task/chat/messages/tool-subagent-message.test.tsx` — 31 passed.
