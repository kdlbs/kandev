...
Implemented lifecycle executor-root forwarding through agentctl to ACP `session/new`. Lifecycle derives roots only from materialized task checkouts and durable task-owned attachments, never the source repository used to create a worktree. Agentctl revalidates that server-owned canonical set before ACP capability negotiation; the adapter emits sibling directories only when the initialized provider advertises `additionalDirectories`. A changed or malformed root, or an unsupported required sibling scope, returns `git_metadata_projection_unsupported` before `session/new`. Clone-based Docker, SSH, and Sprites create and persist executor-visible roots rather than host sources. Focused/package Go tests and race verification pass.
