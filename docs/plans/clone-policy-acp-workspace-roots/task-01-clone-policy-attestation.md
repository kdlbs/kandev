...
Implemented a typed, path-free clone-policy requirement and executor-side attestation for Docker, SSH, and Sprites. Docker bootstrap, SSH, and Sprites validate the actual regular clone after preparation; the lifecycle then asks agentctl to attest its own canonical primary checkout and renders `CODEX_CONFIG` only before `ConfigureAgent`/`Start`. Multi-repository materialization requires the same server-side regular-checkout attestation before a writable Git metadata rule is installed. Docker, SSH, and Sprites reuse is bypassed when fresh attestation is required. Focused/package Go tests, race verification, and backend lint pass; Docker/SSH E2E remains environment-gated in task 03.
