...
Extend the existing owner-only `set_task_title_kandev` transaction with a best-effort, per-repository
...
- For each remaining worktree, call `worktree.RenderTaskBranchName` with the accepted title, task ID,
  ticket/issue metadata, repository template, and a suffix generated by the same executor-specific rule
  used at branch creation: a fresh three-character worktree suffix for host worktrees and the task-ID
  prefix for container/Sprites/SSH branches. Do not special case templates without `{suffix}`;
  collision behavior remains the Git layer's responsibility.
...
- Return per-repository renamed, preserved, and failed outcomes. Continue after an individual Git
  failure; never rename or delete a remote branch and never roll back a previously successful rename.
...
- Update `docs/public/tasks-and-workflows.md` to explain that the default-on prompt-first option also
