...
- Use a hover card for fine pointers and an inset drawer for coarse pointers.
...
Use `useTouchDrawer` to select the surface. Fine pointers use `HoverCard`. Coarse pointers use `Drawer` with internal overflow and safe-area padding.
...
The desktop scenario hovers the compact trigger, shows every step, selects a target, and polls the task API for the new step.

The tablet scenario uses `tabletTestPage`. It opens the touch drawer, checks viewport containment, selects a target, and polls the same API result.
...
| `AC-UI-COMPACT-WORKFLOW-STEP-NAVIGATION-001.2` | Fine-pointer component test and desktop hover E2E             |
...
- Component tests: 8 passed in `components/task/workflow-stepper.test.tsx`.
- Focused ESLint: passed for the changed component, test, and E2E files.
- i18n checks and ratchet: passed with no new violations.
...
- A nested hover card can close its parent before a movement button receives the pointer. The design uses one disclosure surface and no nested hover roots.
