# Orchestrate Wave 1: Shell, Data Model & Routing

**Date:** 2026-04-26
**Status:** proposed
**Spec:** `docs/specs/orchestrate-overview/spec.md`
**UI Reference:** `docs/plans/2026-04-orchestrate-ui-reference.md` (sidebar, layout, component patterns)
**Depends on:** nothing (foundation wave)

## Problem

No orchestrate infrastructure exists. We need the database tables, the `/orchestrate` page shell with sidebar, routing for all sub-pages, and the backend API stubs before any feature work can begin.

## Scope

### Backend: Database tables (SQLite, code-based migration)

All tables created in a new `internal/orchestrate/repository/sqlite/base.go` using `CREATE TABLE IF NOT EXISTS` pattern (matching `internal/task/repository/sqlite/base.go`).

**Tables to create:**

1. **`orchestrate_agent_instances`**
...
8. **`orchestrate_routine_triggers`**
...
15. **`task_comments`** (orchestrate async comments, separate from session messages)
    - `id TEXT PRIMARY KEY`
...
├── models/          # All orchestrate data types
├── repository/
...
Register routes under `/api/v1/orchestrate/` namespace:

```
# Agent instances
GET    /api/v1/orchestrate/workspaces/:wsId/agents
POST   /api/v1/orchestrate/workspaces/:wsId/agents
GET    /api/v1/orchestrate/agents/:id
PATCH  /api/v1/orchestrate/agents/:id
DELETE /api/v1/orchestrate/agents/:id

# Skills
GET    /api/v1/orchestrate/workspaces/:wsId/skills
POST   /api/v1/orchestrate/workspaces/:wsId/skills
GET    /api/v1/orchestrate/skills/:id
PATCH  /api/v1/orchestrate/skills/:id
DELETE /api/v1/orchestrate/skills/:id

# Projects
GET    /api/v1/orchestrate/workspaces/:wsId/projects
POST   /api/v1/orchestrate/workspaces/:wsId/projects
GET    /api/v1/orchestrate/projects/:id
PATCH  /api/v1/orchestrate/projects/:id
DELETE /api/v1/orchestrate/projects/:id

# Cost events
GET    /api/v1/orchestrate/workspaces/:wsId/costs
GET    /api/v1/orchestrate/workspaces/:wsId/costs/by-agent
GET    /api/v1/orchestrate/workspaces/:wsId/costs/by-project
GET    /api/v1/orchestrate/workspaces/:wsId/costs/by-model

# Budget policies
GET    /api/v1/orchestrate/workspaces/:wsId/budgets
POST   /api/v1/orchestrate/workspaces/:wsId/budgets
PATCH  /api/v1/orchestrate/budgets/:id
DELETE /api/v1/orchestrate/budgets/:id

# Routines
GET    /api/v1/orchestrate/workspaces/:wsId/routines
POST   /api/v1/orchestrate/workspaces/:wsId/routines
GET    /api/v1/orchestrate/routines/:id
PATCH  /api/v1/orchestrate/routines/:id
DELETE /api/v1/orchestrate/routines/:id
POST   /api/v1/orchestrate/routines/:id/run

# Approvals
GET    /api/v1/orchestrate/workspaces/:wsId/approvals
POST   /api/v1/orchestrate/approvals/:id/decide

# Activity log
GET    /api/v1/orchestrate/workspaces/:wsId/activity

# Inbox (computed view)
GET    /api/v1/orchestrate/workspaces/:wsId/inbox

# Agent memory
GET    /api/v1/orchestrate/agents/:id/memory
PUT    /api/v1/orchestrate/agents/:id/memory
DELETE /api/v1/orchestrate/agents/:id/memory/:entryId
GET    /api/v1/orchestrate/agents/:id/memory/summary

# Dashboard
GET    /api/v1/orchestrate/workspaces/:wsId/dashboard

# Wakeup queue (internal, for dashboard status)
GET    /api/v1/orchestrate/workspaces/:wsId/wakeups
```

Wire into `cmd/kandev/gateway.go` and `cmd/kandev/services.go` via Provider pattern.

### Frontend: `/orchestrate` shell

**New directory structure:**
```
apps/web/app/orchestrate/
├── layout.tsx                    # Orchestrate layout (sidebar + topbar)
...
- Replaces default sidebar with orchestrate sidebar
- Shows workspace switcher at top
- Renders orchestrate topbar
- Children render in main content area

**Orchestrate sidebar** sections:
...
**Zustand slice stub** (`lib/state/slices/orchestrate/`):
```
orchestrate-slice.ts     # Slice creator with default state
types.ts                 # All orchestrate types
```

**API client stubs** (`lib/api/domains/orchestrate-api.ts`):
- Functions for all endpoints above
- Typed with response types

### Frontend: Homepage link

Add "Orchestrate" link to the kandev homepage top navigation bar. Navigates to `/orchestrate`.
...
4. `/orchestrate` page renders with sidebar
