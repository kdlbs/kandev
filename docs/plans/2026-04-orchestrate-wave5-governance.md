# Orchestrate Wave 5: Cost Tracking, Budgets, Inbox, Activity & Dashboard

**Date:** 2026-04-26
**Status:** proposed
**Specs:** `orchestrate-costs`, `orchestrate-inbox`
**UI Reference:** `docs/plans/2026-04-orchestrate-ui-reference.md` (dashboard, costs page, budget cards, inbox, activity page, approval rows)
...
**models.dev pricing integration** (`internal/orchestrate/service/pricing.go`):
...
**Cost event handler** (`internal/orchestrate/service/cost_handler.go`):
...
- Write `orchestrate_cost_events` row
- Increment `TaskSession.cost_cents`, `TaskSession.tokens_in`, `TaskSession.tokens_out`
- Check budget policies (see 5B)

**Cost aggregation queries** (`internal/orchestrate/repository/sqlite/costs.go`):
...
**Frontend** (`/orchestrate/company/costs`):
...
**Repository** (`internal/orchestrate/repository/sqlite/budgets.go`):
- Full CRUD for `orchestrate_budget_policies`
...
**Service** (`internal/orchestrate/service/budgets.go`):
...
**Repository** (`internal/orchestrate/repository/sqlite/activity.go`):
...
**Service** (`internal/orchestrate/service/activity.go`):
- `LogActivity(ctx, actorType, actorID, action, targetType, targetID, details)` -- helper
- Called from all orchestrate services on mutations (agent CRUD, skill CRUD, approval resolution, budget events, etc.)
- Emit `orchestrate.inbox.item` event for notification-worthy entries

**Frontend** (`/orchestrate/company/activity`):
...
**Inbox service** (`internal/orchestrate/service/inbox.go`):
- Computed view, no dedicated table. Queries:
  - Pending approvals from `orchestrate_approvals` where status=pending
...
**Approval service** (`internal/orchestrate/service/approvals.go`):
...
**Execution policy flow** (`internal/orchestrate/service/execution_policy.go`):
...
- Add event type `orchestrate.inbox_item` to notification service
- Subscribe Local and System providers by default

**Frontend** (`/orchestrate/inbox`):
...
**API:**
- `GET /api/v1/orchestrate/workspaces/:wsId/dashboard` returns:
