---
id: "07-migrate-session-cost"
title: "Migrate the session-cost plugin and publish PR artifacts"
status: pending
wave: 2
...
# Task 07: Migrate the session-cost plugin and publish PR artifacts

- **Acceptance:** `kdlbs/kandev-plugin-session-cost` pins the Task 04 SDK commit,
  removes its local Kandev replacement, and passes repository tests with
  `GOWORK=off`.
...
- **Files likely touched:** external repository
  `kdlbs/kandev-plugin-session-cost`: `go.mod`, `go.sum`, Go imports,
  `Makefile`, `README.md`, `.github/workflows/**`.
- **Dependencies:** Tasks 04 and 06.
- **Parallelism:** parallel-safe with Tasks 08–09 after the template pattern is merged.
