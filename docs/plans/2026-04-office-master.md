# Office: Master Implementation Plan

**Date:** 2026-04-26
**Status:** proposed
**Specs:** all `docs/specs/office-*` specs (10 specs)
**Plans:** Waves 1-7 in `docs/plans/2026-04-office-wave*.md`

## Overview

Office is an autonomous agent management layer built on top of kandev's existing task/session/workflow system. Implementation is split into 7 waves with clear dependency ordering. Each wave contains parallelizable work units that can be assigned to subagents.
...
| `apps/web/lib/state/slices/office/` | 1, 2, 3, 5 | Store slice (additive) |
| `apps/web/lib/api/domains/office-api.ts` | 1, 2, 3, 5, 6, 7 | API client (additive) |
| `apps/web/app/office/layout.tsx` | 1 | Layout (set once in Wave 1) |
| `apps/web/app/office/components/office-sidebar.tsx` | 1, 2, 5, 6 | Sidebar (additive) |
...
- **Feature flag**: Office UI is behind the `/office` route. Users who don't navigate there see no changes.
- **Shared models**: Task model extensions are additive columns with defaults. Office tasks use a system workflow (workflow_id stays NOT NULL). Existing tasks are unaffected.
- **Isolated package**: `internal/office/` is a new package. No modifications to existing service/repository code except additive task model fields and event type constants.
