# Orchestrate Wave 3: Task System & Issues UI

**Date:** 2026-04-26
**Status:** proposed
**Specs:** `orchestrate-overview` (issues list, task detail, new issue dialog)
**UI Reference:** `docs/plans/2026-04-orchestrate-ui-reference.md` (issues list, task detail simple/advanced, new issue dialog, properties panel, chat thread)
**Depends on:** Wave 2 (agents, skills, projects exist for assignment)

## Problem

The issues list, task detail pages (simple + advanced mode), and new issue dialog are the core user-facing UI for orchestrate. This wave extends the existing task model and builds the issue tracker interface.
...
- Status transitions: when orchestrate task reaches `done` with execution_policy, enter review/approval flow
...
- Non-orchestrate tasks (kanban) continue to require workflow_id
- Existing task creation, move, workflow transitions unaffected
- Kanban board only shows tasks with workflow_id (orchestrate tasks excluded from kanban view)
...
- `OrchestrateIssueBoard`: board view
  - Columns by status, drag-drop between columns
  - Reuse shadcn components
- `OrchestrateIssueToolbar`: filter/sort/group controls
...
- `OrchestrateIssueFilters`: filter state management
  - Persist to localStorage per workspace

**API calls:**
- `GET /api/v1/orchestrate/workspaces/:wsId/issues` (extends existing task list with orchestrate filters)

**Store:**
- `orchestrate.issues`: `{ items: Task[], filters: FilterState, viewMode, sortField, sortDir, groupBy, nestingEnabled }`
...
**Simple mode** (`/orchestrate/issues/[id]/page.tsx`):
...
**Components:**
- `OrchestrateTaskDetail`: main layout, simple/advanced toggle
- `OrchestrateTaskProperties`: right sidebar panel
...
- `OrchestrateTaskChat`: comment thread
  - Agent run transcripts with collapsible tool call details
  - User/agent comments
  - Input box for posting comments
- `OrchestrateTaskActivity`: timeline of status changes, assignments, approvals

**Advanced mode** (`?mode=advanced`):
- Dockview layout within orchestrate chrome (sidebar + topbar stay)
...
**Components:**
- `OrchestrateNewIssueDialog`: modal
