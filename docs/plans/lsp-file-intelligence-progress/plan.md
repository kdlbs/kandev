...
- **Task-host transport safety:** agentctl tests observe the bounded write deadline on the pre-bridge ready frame, prove stdout-forwarder exit releases an active stdin write, and use virtual time to verify the 30-second stdin cutoff; existing bridge tests retain process-cleanup coverage for failed and blocked WebSocket writes.
