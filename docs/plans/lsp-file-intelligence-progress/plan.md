...
- Register signature help only when the initialized server advertises it, using its exact trigger and retrigger characters.
...
- **Completion, signature help, and configuration:** focused provider and manager tests prove completion range fallback/override behavior, incomplete-list preservation, advertised completion and signature-help trigger registration, signature-help capability gating, all 25 standard completion-kind mappings, connection reuse, live configuration notification, and updated request responses.
