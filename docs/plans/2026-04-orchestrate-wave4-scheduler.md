# Orchestrate Wave 4: Wakeup Queue & Heartbeat Scheduler

**Date:** 2026-04-26
**Status:** proposed
**Spec:** `orchestrate-scheduler`
**Depends on:** Wave 3 (tasks can be assigned to agent instances)

## Problem

The existing scheduler is reactive -- tasks only execute when a user explicitly starts them. Orchestrate needs agents to wake autonomously when events happen (task assigned, comment posted, blockers resolved, etc.). This wave adds the wakeup queue and extends the scheduler to process it.

## Scope

### 4A: Wakeup Queue Service (backend)

**Repository** (`internal/orchestrate/repository/sqlite/wakeup.go`):
...
- `QueueWakeup(ctx, agentInstanceID, reason, payload, idempotencyKey)`:
  ...
  5. Emit `orchestrate.wakeup.queued` event
...
5. **Resolve executor** (`internal/orchestrate/service/executor_resolver.go`):
...
7. **On session complete:**
   ...
   - Emit `orchestrate.wakeup.processed` event
   - Log to activity log

**Agent API authentication** (`internal/orchestrate/service/agent_auth.go`):
...
- API middleware on orchestrate endpoints validates the JWT and scopes access:
...
**Skill injection at session start** (`internal/orchestrate/service/skill_injection.go`):
- Before launching agent process, resolve agent instance's `desired_skills`
- Call `MaterializeSkills()` from Wave 2B
- Call `SymlinkSkills()` to create symlinks in agent's home dir
- After session ends: call `CleanupSymlinks()`
- Hook into existing lifecycle manager's session start/end events
...
**`internal/orchestrate/service/prompt_builder.go`**:
