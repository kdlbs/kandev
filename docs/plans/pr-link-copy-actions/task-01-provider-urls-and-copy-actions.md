...
- REFACTOR: the copy control is shared by the header and comment rows; comment
  actions reveal on fine-pointer hover/focus and remain visible at coarse
  pointers with 44px targets. The public PR URL fallback avoids API URLs.
- Verification: backend unit and race tests, `make -C apps/backend lint`, 41
  focused frontend tests, desktop and Pixel 5 E2E scenarios, web typecheck,
