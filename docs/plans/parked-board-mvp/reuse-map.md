...
| Action | Files / work |
|---|---|
| **HARVEST** | All probe files (both Unix + Windows stub) + tests; transport (`client_probe.go`, `manager_probe.go`, `agent_probe.go`); `dto/parked.go`; the Claude recogniser inline + `Kind==shell` filter (D-7: no `background_launch_recognizer.go` public registry in V1); `parked_architecture_test.go` (AC-35); board frontend (`kanban-card-content.tsx`, `kanban-card.tsx`, `state-icons.tsx` board bits, `kanban-parked` WS handler, `hydrator-kanban-parked*`, `use-all-workflow-snapshots-parked`, board e2e spec) — **not** `lib/kanban/parked-projection.ts` (D-2: V1 ships the boolean-only form, not the harvested three-value projection) |
| **EXTRACT** | `parked_state.go` / `parked_projection.go` → take **only** the three-term formula + settle hook + clear-on-`RUNNING`-or-`STARTING` (D-3). **Leave out** the sampler loop, eviction, tombstone, epoch/revision. *This is V1's one real engineering task.* |
| **NEW** | Runtime flag (`runtimeflags/registry.go` + `profiles.yaml`, off everywhere) + disabled-path tests; clear-on-observed-`RUNNING`-or-`STARTING` (D-3, substitutes for the deferred `turn_started`); resolve the turn-stamp bypass (D-1: stamp on all dispatch paths); test debt — zombie-*descendant* (AC-27a), §L-shaped trees (AC-70/70a), fix scrambled AC labels in `probe_unix_test.go` |
