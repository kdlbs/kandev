...
- **Artifacts**: Files or data produced (e.g., plan.md)
...
Task artifacts (plans, specs, etc.) are stored in the `task_artifacts` table, not just as files in the worktree.

**Rationale**:
- **Accessible without worktree**: View plans even if worktree is deleted or task moves repos
- **Versioning**: Easy to track plan revisions
- **Search/query**: Search across plans, show "all plans this week", etc.
- **Multi-repo tasks**: Plan isn't tied to a specific repo's worktree
- **UI display**: Direct DB query, no filesystem access needed

The agent can still write to `.kandev/plans/{task_id}.md` in the worktree for its own reference, and we sync that content to the DB on step completion.
...
interface WorkflowStep {
  ...
  // Artifacts
  artifacts?: {
    type: 'plan' | 'spec' | 'review' | 'custom';
    filename: string;              // e.g., "plan.md", "spec.md"
...
steps:
  ...
  - id: planning
    ...
    artifacts:
      - type: plan
        filename: .kandev/plans/{task_id}.md
    transitions:
...
-- Session artifacts (plans, specs, etc.) - tied to session, not task
CREATE TABLE session_artifacts (
  id TEXT PRIMARY KEY,
  session_id TEXT NOT NULL REFERENCES task_sessions(id) ON DELETE CASCADE,
  artifact_type TEXT NOT NULL,    -- plan, spec, design, review, custom
...
| No artifacts | `session_artifacts` table (per-session) |
...
│  └── artifacts: [plan.md, implementation_notes.md]                          │
│                                                                             │
│  Session 2 (abandoned)                                                      │
│  ├── is_primary: FALSE                                                      │
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ 📋 Session Artifact: Implementation Plan                 [Open ↗]  │   │
│  ├─────────────────────────────────────────────────────────────────────┤   │
│  │                                                                      │   │
│  │  ## Overview                                                         │   │
...
│  │  1. Create `auth` package with JWT utilities                         │   │
│  │  2. Add login/logout endpoints to API                                │   │
...
1. The session's artifact to review (plan, code changes, etc.)
2. Previous feedback (if any)
3. Approval/rejection actions
4. Feedback input for requesting changes
...
function SessionReviewPanel({ session, task, step }: {
  ...
  // Artifacts are session-level, not task-level
  const artifact = useSessionArtifact(session.id, step.artifacts?.[0]?.type);
  // Feedback is stored on the session, not the task
  const existingFeedback = session.review_feedback ?? [];
  const unresolvedFeedback = existingFeedback.filter(f => !f.resolved);
        ...
        <div className="border rounded-lg">
          <div className="flex items-center justify-between p-3 border-b bg-muted">
            <span className="font-medium">
              {step.artifacts?.[0]?.type === 'plan' ? '📋 Implementation Plan' : '📄 Artifact'}
            </span>
            <Button variant="ghost" size="sm" onClick={() => openInEditor(artifact)}>
              Open in Editor
            </Button>
          </div>
...
# Session Artifacts (artifacts are session-level)
GET    /api/v1/sessions/:id/artifacts          # List session artifacts
GET    /api/v1/sessions/:id/artifacts/:type    # Get specific artifact (e.g., plan)
POST   /api/v1/sessions/:id/artifacts          # Create/update artifact
```
...
| **Plan Storage** | Database (`session_artifacts` table) | Accessible without worktree, versioning, searchable, multi-repo support |
...
1. **Multi-Repo Artifacts**: How do session artifacts work with multi-repository tasks?
   - Option A: Artifacts are session-level (not repo-specific) ✓ (current design)
   - Option B: Each repo can have its own artifacts per session
   - **Decided: Option A** - artifacts belong to the session, not the repo

2. **Step Skipping**: Should users be able to skip steps when starting a new session?
...
5. [ ] Create `session_artifacts` table (session-level, not task-level)
6. [ ] Create `session_step_history` table

### Phase 2: Backend Services
7. [ ] Implement `WorkflowService` (CRUD for templates and steps)
8. [ ] Implement `SessionArtifactService` (store/retrieve session artifacts)
9. [ ] Update `SessionService` with workflow step operations (`MoveSessionToStep`, etc.)
