...
- Every Git snapshot write is retained. Explicit maintenance may remove redundant live/status snapshots only within the same session and content-digest group; snapshots from different sessions remain distinct history. Configurable retention also selects superseded tool payloads and obsolete plan revisions without silently deleting them.
