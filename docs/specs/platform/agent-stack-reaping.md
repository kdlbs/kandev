...
The 2026-08-23 incident is the reference case: 11 ACP stacks launched between Aug 20 and Aug 22, every one of them belonging to a task sitting in REVIEW, still alive two to three days later and holding ~3-3.5 GB of mostly swapped-out RSS. A single task had accumulated five stacks of its own across agent switches. A frontend build inside a task workspace then took 2.1 GB, filled the 2.9 GB swap, and drove a 6-vCPU / 9.8 GB host to a load average of 70 for 75 minutes with the CPU 95% idle. The two facts that shape this design: the stacks were idle for *days* (so a bounded idle window catches all of them with room to spare) and the damage came from how many were alive at once (so a ceiling, not just an age test, is what actually bounds the failure).
...
- A safety net reaps any remaining idle agent stack after a bounded idle window (default 10 minutes without session activity), regardless of how it was left behind (office tasks, uncovered paths, races). Idle age MUST be measured from the session row, not from `executors_running.updated_at` — the latter is refreshed by execution persistence and status writes, so a long-lived stack that just finished a turn would read as ancient.
- A ceiling on concurrently live stacks (default 6) bounds memory regardless of timing, which is the shape the incident actually took. When the number of non-stopped `executors_running` rows exceeds the ceiling, the least recently active reapable sessions are stopped until the count is back at the ceiling. Working sessions count toward the ceiling but are never evicted.
...
- A runtime feature toggle (`features.agentStackReaping`, env `KANDEV_FEATURES_AGENT_STACK_REAPING`) acts as a kill switch. It defaults to ON in every shipped profile because the guarded stop is the fix for the incident class, not a new capability; operators can disable it via Settings > System > Feature Toggles or the environment variable (restart required).

## API surface
- Runtime flag registry entry `features.agentStackReaping` (see `docs/specs/platform/feature-toggles.md` for the registry contract). Metadata: feature kind, stable, medium risk, restart required, mutable.
- No new HTTP/WS/CLI surface. Teardown uses the existing `StopAgentWithReason` lifecycle path; observable as the existing `agent.stopped` event and `stopping agent` log lines carrying reason strings `agent stack reaping: task completed`, `agent stack reaping: idle ttl`, and `agent stack reaping: live stack cap`.
...
Idle net and cap (periodic, every 30 s alongside the idle-row reaper):

```text
session idle ≥ 10 min (session row clock)
...
non-stopped executors_running rows > 6
  ──> stop reapable sessions, least recently active first,
      until the count is back at the ceiling
```

## Failure modes
- Stop failure (agentctl unreachable, process already gone): logged at warn, skipped. The idle net retries on later ticks; the existing row-repair/reclaim machinery converges the `executors_running` row once the process is dead.
...
- **GIVEN** a session whose session row has been settled for over 10 minutes with a live execution and no active turn, **WHEN** the idle tick evaluates it, **THEN** the stack is stopped with reason `agent stack reaping: idle ttl`.
...
- **GIVEN** more live stacks than the ceiling, **WHEN** the cap pass runs, **THEN** the least recently active reapable sessions are stopped with reason `agent stack reaping: live stack cap` until the count is back at the ceiling, and working sessions are never chosen.
...
- Making the idle window or the live-stack ceiling operator-configurable. Both are code constants alongside the existing idle-reaper knobs; promote them only if a real deployment needs a different value.
- Office-specific lifecycle hooks (office tasks skip the REVIEW write today; the idle net and the cap cover their stacks).
