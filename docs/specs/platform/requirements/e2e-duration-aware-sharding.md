...
- **AC-PLATFORM-E2E-DURATION-AWARE-SHARDING-002.1:** Container-backed E2E jobs shall reuse browser binaries from a matching cache when available. A miss, stale entry, cache-service failure, or incompatible entry shall use the existing pinned-image extraction path and shall not change test selection or make the cache a correctness dependency.
