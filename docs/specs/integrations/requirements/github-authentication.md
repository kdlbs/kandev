...
- **AC-INTEGRATIONS-GITHUB-AUTHENTICATION-001.13:** When a new managed session starts for a task attached to canonical `kdlbs/kandev`, the system may bind an already-existing writable fork only after the workspace automation connection verifies the fork owner, source, target, parent, stable provider identities, and credential generation. The system shall not trust a caller-added Git remote or create a fork through this compatibility path.
- **AC-INTEGRATIONS-GITHUB-AUTHENTICATION-001.14:** A verified contributor-fork binding shall route the managed push action for the exact current task branch and head to the same branch on that exact fork while preserving canonical `origin`, and the managed action shall reject a mismatched branch, an invalid or different refspec, force, or any history-rewriting update before the fork credential can be redeemed. The fork credential scope shall be withheld from the agent, task shell, tracker, and arbitrary command environment and supplied only to the trusted managed-push subprocess after validation. The broker shall continue to deny arbitrary repositories and forks, cross-workspace tasks, executor-owned credentials, and unrelated credentials. PAT and named-CLI bearer credentials retain their provider-granted authority after delivery to an admitted trusted subprocess.
- **AC-INTEGRATIONS-GITHUB-AUTHENTICATION-001.15:** Existing running sessions shall retain their immutable launch-time credential scopes. A task that gains a verified contributor-fork binding after deployment shall start a fresh session before attempting publication.
- **AC-INTEGRATIONS-GITHUB-AUTHENTICATION-001.16:** Launch-time contributor-fork preparation shall add the destination with an atomic or optimistic metadata-only mutation that rechecks the exact task/repository attachment and existing contribution bindings. Concurrent base-branch, checkout-branch, policy, position, and unrelated metadata changes shall be preserved.
