...
- **AC-INTEGRATIONS-GITHUB-AUTHENTICATION-001.14:** A verified contributor-fork binding shall route the managed push action for the current task branch and head to the same branch on that exact fork while preserving canonical `origin`, and the managed action shall reject force or history-rewriting updates. The broker shall continue to deny arbitrary repositories and forks, cross-workspace tasks, executor-owned credentials, and unrelated credentials. PAT and named-CLI bearer credentials retain their provider-granted authority after delivery to a trusted raw Git subprocess.
