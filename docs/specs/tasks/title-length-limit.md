...
- Other automatic title-generating paths are covered where implemented: the automation-trigger path already routes its generated title through the shared truncation helper. The Azure DevOps work-item watcher (`[<project>#<n>] <title>`) does **not** yet shorten its generated title and is the one remaining unshortened watcher path; bringing it (and any future watcher) through the shared `TruncateTaskTitle` helper is tracked as follow-up. New watcher paths that auto-generate a title from a remote item must route it through the same helper.
