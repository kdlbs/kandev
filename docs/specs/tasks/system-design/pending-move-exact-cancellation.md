...
- `internal/mcp/server` advertises `cancel_pending_move_kandev` only in the
  automation Coordinator catalog and redacts its complete payload from backend
  request logs.
- `internal/mcp/handlers` derives the caller from the scoped principal, maps
  stable errors, and bypasses predicate-light generic preflight so the repository
...
and PostgreSQL. Existing pending rows receive identities during the compatible
pending-move migration owned with the TTL/reaper work.
...
- Migration tests cover replay and same-workspace foreign-key enforcement.
