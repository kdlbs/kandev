...
number. The fetch can force-update this internal ref because users do not own
it. The fetch never writes directly to a user-named local branch.
...
error for its matching session and stamp. The task Chat surface renders one
persistent card from this record.
...
An unrelated historical error keeps its normal transcript presentation. A
runtime error that occurs after agent startup keeps the existing session
recovery surface.
