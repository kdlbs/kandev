...
- `orchestrator.Service.switchSessionForStep` applies the destination step's profile-session policy when it reuses a nonterminal destination session or prepares a new session in the task's existing environment.
...
`workflow_steps.profile_session_policy` governs whether the replaced session is
completed, parked for profile re-entry, or parked while later entries start a
fresh session. The full lifecycle contract is in
...
2. After the destination step is selected and before any entry action runs, resolve the effective profile and normalized session policy from that step.
3. If the effective profile is empty or matches the active session, keep that session and make it primary when needed.
4. If the profile differs, select reuse or new-session behavior from the destination step's normalized profile-session policy.
5. Promote the destination session, transfer queued messages and pending move state, then complete or park and stop the replaced session according to that policy.
...
stores the profile-session policy.
