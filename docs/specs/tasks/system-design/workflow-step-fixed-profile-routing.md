...
- `orchestrator.Service.switchSessionForStep` applies the workflow's profile-session policy when it reuses a nonterminal destination session or prepares a new session in the task's existing environment.
...
`workflows.profile_session_policy` governs whether the replaced session is
...
4. If the profile differs, select reuse or new-session behavior from the workflow's normalized profile-session policy.
...
session-metadata records. The workflow row stores the profile-session policy.
The repositories retain profile identity, primary ownership, terminal or parked
state, and workflow-switch provenance after a restart.
