...
- `apps/backend/internal/task/service/service_messages.go` owns the message-specific dismissal operation, validates that the addressed row is a Git push error, and delegates the single durable update and event publication to `UpdateMessage`.
...
- `apps/web/lib/ws/handlers/messages.ts` already applies `session.message.updated` by message ID, so the existing broadcast updates active chat views without a separate client state store.
