...
- The preview workflow treats an author in `CLAUDE_REVIEW_ALLOWLIST` as trusted for fork preview deployment on every non-closed pull-request-target event it already handles (`opened`, `synchronize`, `reopened`, and `labeled`). A maintainer-applied `safe-to-test` label remains visible across later `synchronize` events, but the label path does not authorize privileged preview execution for a new fork head until the maintainer provides fresh label approval.
...
- Approval labels remain until a maintainer removes them. A contributor push does not revoke either label; `safe-to-test` requires fresh approval for privileged preview execution, while `safe-to-review` remains an approval for follow-up review runs.
...
- A non-allowlisted fork author still needs the existing maintainer-applied labels. The labels remain visible until a maintainer removes them; `safe-to-test` must be freshly applied for a new privileged preview head, while `safe-to-review` authorizes current and subsequent constrained OpenCode review heads. Maintainers are responsible for revoking approval when the contributor or proposed change is no longer trusted.
...
- **GIVEN** an unallowlisted fork pull request has `safe-to-test`, **WHEN** its contributor pushes a follow-up commit, **THEN** `safe-to-test` remains present but the preview deploy job is not eligible through that label on `synchronize` until fresh maintainer approval is applied.
