...
| AC-EO-13 | **lock SPAN, the case "exactly one commit" cannot see.** Two concurrent calls, same operation id; the winner commits and then leaves the operation unmarked (drive that from the test, as in the AC-EO-17 row). Assert the loser's evaluation loaded the **target** step, not the source. A lock taken only around the engine call passes the one-commit row above and fails this one, because the loser still holds a `PreloadedState` built before the winner's commit — that is exactly the AC-EO-13 / AC-EO-17 conflict this row exists to pin. The fix under test is that the lock is acquired before `resolveAgentErrorDispatchTarget` / `buildMachineState`, so the loser reloads |
...
| AC-EO-16 | `processOnChildrenCompleted` still brackets its own marker; `on_turn_start` / `on_turn_complete` still pass no `OperationID` |
