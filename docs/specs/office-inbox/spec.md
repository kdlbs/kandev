...
- The inbox at `/office/inbox` is the user's single view of everything that needs their attention.
...
- A new event type `office.inbox_item` is added alongside the existing `session.waiting_for_input` event.
- Users subscribe to this event per provider in the notification settings (the same UI at `/settings/general`).
- Default behavior: Local (browser) and System (OS) providers are auto-subscribed to `office.inbox_item` when Office is enabled.
- Notification content includes: item type, summary, and a deep link to `/office/inbox`.
...
- Users set reviewers/approvers when creating a task (via the properties panel or new task dialog).
...
- Workspace setting `require_approval_for_task_completion` (default `false`): if true, all office tasks default to having the user as an approver. Can be overridden per task.
...
- Workspace-level settings control defaults:
  ...
  - `require_approval_for_task_completion`: default `false`. If true, all office tasks default to `requires_approval=true`. Users and agents can override per task.
...
- The activity log records every significant action in the Office system.
...
- `/office/company/activity` page shows:
...
- The Office dashboard at `/office` shows a "Recent Activity" section with the last ~10 entries.
