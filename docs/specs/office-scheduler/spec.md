...
# Office: Wakeup Queue & Heartbeat Scheduler
...
Office adds a wakeup queue that sits alongside the existing task queue. Events in the system generate wakeup requests for agent instances. The scheduler claims and processes wakeups by spinning up the executor, running a single agent turn, and tearing everything down — a fire-and-forget heartbeat model integrated with kandev's existing orchestrator pipeline. Sessions persist across turns per-(task, agent); see [office-task-session-lifecycle](../office-task-session-lifecycle/spec.md) for the IDLE↔RUNNING state machine and `EnsureSessionForAgent` lookup.
...
| `task_comment` | Comment posted on a task assigned to this agent (non-self). This is also how channels work: inbound Telegram/Slack messages become comments on a channel task, triggering this wakeup. See [office-assistant](../office-assistant/spec.md). | `{task_id, comment_id}` |
...
- Office tasks live on the system office workflow and appear on the kanban board. Users can drag them between columns (steps), which emits a `task.moved` event.
- The office event subscribers handle `task.moved` events for office tasks (identified by `assignee_agent_instance_id != null`) and fire the appropriate wakeups:
...
- Non-office tasks (those without `assignee_agent_instance_id`) are ignored by office subscribers, even if moved on any workflow.
...
- Session resume is the default for office tasks: each subsequent wakeup for a `(task, agent)` pair reloads the prior ACP session via `session/load`, with a clean fall-back to `session/new` on error. See [office-task-session-lifecycle](../office-task-session-lifecycle/spec.md) for the full per-pair model and `EnsureSessionForAgent` semantics.
...
- Office does not have a separate workflow/template engine for subtask ordering. The agent's instructions (via skills) define how to decompose work and which subtasks to create.
