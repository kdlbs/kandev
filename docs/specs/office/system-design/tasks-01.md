...
- Agents can list related tasks (parent, children, siblings, blockers, blocked) and can read/write allowed task documents via MCP or CLI tools. Related-task reads are relation-scoped by default. The persisted Office CEO/Coordinator alone can inspect a compact unrelated task tree in its workspace; this capability never grants descriptions, document keys, document bodies, or write access. `verbose=true` requires ordinary document-read authorization for the requested target, and each returned node filters descriptions and document keys independently. Unknown and cross-workspace targets share the same non-leaking denial. Decision: ADR-2026-08-20-mcp-related-task-read-authorization.
