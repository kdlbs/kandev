...
- **GIVEN** an agent below its failure threshold with `cooldown_sec > 0`, **WHEN** a run fails and `HandleAgentFailure` marks it terminal, **THEN** `office_agent_runtime.last_run_finished_at` is stamped the same as after a successful run, and the heartbeat gate blocks the agent's next fire (for any task) until `cooldown_sec` has elapsed.
...
- **GIVEN** an `agent_paused_after_failures` inbox entry, **WHEN** the user clicks Mark fixed and the agent's `pause_reason` still matches the value the entry was raised for, **THEN** the compare-and-set clear succeeds, `consecutive_failures` resets to zero, the entry is dismissed, and a wakeup with reason `manual_resume_after_failure` is queued for every (task, agent) listed on the entry (`AC-OFFICE-RUNTIME-001.9`, `AC-OFFICE-RUNTIME-001.10`).
- **GIVEN** an `agent_paused_after_failures` inbox entry, **WHEN** the user clicks Mark fixed but a second auto-pause has already overwritten the agent's `pause_reason` with a different value, **THEN** the compare-and-set clear is refused, the call aborts instead of adopting the newer reason, `consecutive_failures` is not reset, no wakeup is queued, and the inbox entry is not dismissed so it stays actionable for the still-paused agent (`AC-OFFICE-RUNTIME-001.9`, `AC-OFFICE-RUNTIME-001.10`).
