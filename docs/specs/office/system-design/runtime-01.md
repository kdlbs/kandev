...
This retry re-drives the whole run from the top. It carries no effect-safety evidence about the failed attempt — the Kanban ladder's `promptAttemptPreResultSafe` gate excludes Office entirely, and the routing tier's own post-start requeue already re-drives without that evidence, so this is a pre-existing Office property this retry inherits rather than introduces. A retried run may therefore repeat side effects the failed attempt already committed.
