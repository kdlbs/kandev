...
The retry additionally checks `time.Since(run.RequestedAt) > staleRunThreshold` (2h, `scheduler_staleness.go`) and refuses to schedule when it would already be true — falling through to terminal accounting instead. This is a second, tighter gate than the 24h `retryMaxAge` above: `evaluateRunStaleness` cancels any claimed run with `RetryCount > 0` once it is older than `staleRunThreshold`, and scheduling this retry always leaves `RetryCount > 0`. Without the extra gate, a transient failure on a run between 2h and 24h old would schedule a retry the claim path immediately cancels on relaunch — losing the failure silently (no `consecutive_failures` increment, no `agent_run_failed` inbox row) instead of counting it. An Office agent session running past 2h is routine, not an edge case, so this band is reachable in practice.
