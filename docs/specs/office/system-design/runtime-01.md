...
Mechanism: `HandleAgentFailure` classifies the failure (preferring the structured `ProviderError.Message` over the bare error string, since a bare adapter string like `"Overloaded"` or a raw 429 rarely classifies on its own) after its `MarkRunFailed` guarded write succeeds and before terminal accounting runs. Classification passes the underlying agent id (for example `"claude-acp"`) as the provider id, substituting `ProviderError.ProviderID` only when the agent id itself has no provider rules and the provider id does — the same resolution `classifyKanbanFailure` uses, mirrored rather than reimplemented, because provider rules are keyed by agent id and a raw stderr rate-limit string only matches its provider's rule set when the right id is threaded through. `ProviderError.ResetAt` is not used on this path: unlike the routing tier's short-retry delay, the fixed 5s/10s backoff below does not shorten for an imminent reset window. On a classified-transient hit, bounded by `run.RetryCount < 2` and the same 24h `retryMaxAge` staleness rule the pre-launch tier uses, it releases the task checkout, clears the agent's working flag, and calls `ScheduleRetry` at a fixed 5s then 10s delay — returning early so `recordTerminalShape`, `stampRunFinished`, `IncrementAgentConsecutiveFailures`, `publishRunFailed`, and `autoPauseAgent` never run for that attempt. `RetryCount` is shared with the pre-launch tier rather than a new column, so a run that already burned pre-launch retries gets no post-start retry — under-retrying rather than ever exceeding the combined budget. `ScheduleRetry` also clears `session_id` and `error_message`: the row may already carry the session id and error text of the failed attempt, and a relaunch must resolve the session the new attempt actually gets and not still be flagged with an error once it finishes successfully.
