...
- **Agent CLI integration**: an authorized CEO creates and lists projects in its current workspace through `$KANDEV_CLI kandev projects ...`; follow-up task creation accepts a project ID. Office agents do not create additional workspaces from inside a run.
