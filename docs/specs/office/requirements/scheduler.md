...
- **AC-OFFICE-SCHEDULER-001.10:** A cron routine trigger's wall-clock slot fires at most once across a DST transition: a slot that does not exist (spring-forward) is skipped, and a slot that occurs twice (fall-back) fires only on its first occurrence. This holds for every IANA zone whose DST shift is a whole number of hours. `Australia/Lord_Howe`, the only zone with a 30-minute shift, is a known exception: a returned fire is still guaranteed to match the expression's wall-clock fields, but every fire scheduled anywhere on the transition day is silently skipped, in both directions (spring-forward and fall-back) — not only fires inside the 30-minute transition window itself. For example `0 12 * * *` loses noon, 9.5 hours from the window (the underlying `robfig/cron/v3` day-loop DST correction only nudges by whole hours, so in this single 30-minute-shift zone it lands on the wrong day entirely).
