...
`cost_subcents`, `tokens_in`, `tokens_out`, and `tokens_cached_in` are maintained by the production task cost ledger writer, not by this Office subscriber ([task-cost-ledger](../task-cost-ledger/spec.md) AC-10, AC-21 - that spec amends this passage). They provide quick per-session totals without a scan of `task_usage_events`. An unknown ledger `tokens_out` value contributes zero to the numeric rollup. Therefore, the session output total is a lower bound when at least one ledger row has `tokens_out=NULL`. Consumers that need completeness must read the raw ledger. `tokens_cached_in` is DB-only today (no model/DTO/API/web reader). It exists so the rollup reconciles column-for-column against that ledger. The rollup consumes only the merged `tokens_cached_in` sum, not the `tokens_cached_read` / `tokens_cached_write` split. The split exists solely on the ledger row for cost-per-token-type analysis (contract v2). `office_cost_events` still records its own copy of these fields per row, independent of and no longer reconciled against this rollup.
...
- **`recordCostEventAndRollup` only inserts `office_cost_events`** ([task-cost-ledger](../task-cost-ledger/spec.md) AC-10, AC-21 - that spec amends this passage): it no longer increments the `task_sessions` rollup or shares a transaction with that increment. The task cost ledger writer's own insert (`task_usage_events`) and its `task_sessions` rollup increment are wrapped in one transaction on that writer's side, independent of whether or how this `CreateCostEvent` call succeeds, so a redelivery of the same event is idempotent on each ledger's own unique index without the two ledgers coordinating a shared transaction.
...
- `TaskSession.cost_subcents` / `tokens_in` / `tokens_out` / `tokens_cached_in` running totals. These are maintained by the task cost ledger writer, not by this Office subscriber ([task-cost-ledger](../task-cost-ledger/spec.md) AC-10, AC-21 - that spec amends this passage); that writer's own ledger insert and rollup increment are atomic, so they cannot diverge on a partial write. The numeric `tokens_out` rollup is a lower bound when a source row has an unknown output count.
...
- **GIVEN** the same underlying prompt-usage completion is delivered twice (e.g. a reconnecting WS client replaying a buffered stream event), **WHEN** the second copy is processed, **THEN** it derives the same `usage_event_id` and the `office_cost_events` insert is rejected by the unique partial index; the `task_sessions` rollup is unaffected by either copy, since it is maintained by the task cost ledger writer's own idempotent insert, not by this subscriber ([task-cost-ledger](../task-cost-ledger/spec.md) AC-10, AC-21 - that spec amends this passage).
