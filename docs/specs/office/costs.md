...
| `tokens_out` | int64, nullable | output token count. NULL means "never observed" - `estimated=true` with no output count reported (e.g. adapter_prompt.go's context-occupancy synthesis, which infers `tokens_in` but never sets output tokens). Never `0` for that shape: a plain `0` would assert a measurement that was never taken, so a downstream per-output-token measure would divide by a fake zero-output turn instead of seeing "unknown" (contract v3) |
...
| `cost_contract_version` | int64, nullable | in-band activation marker for this row's column semantics, since the point-in-time Rill extract has no schema versioning of its own. Current value `3` (v2 -> v3: `tokens_out` became nullable, NULL when `estimated=true` and no output count was observed - see the `tokens_out` row above; v1 -> v2: the unpriced path stopped forcing `estimated=true`, see the `estimated` row above). NULL for legacy pre-contract rows - never backfilled |
...
- **GIVEN** a synthesised-usage turn (`estimated=true`, no output token count observed, a provider-reported cost sample present), **WHEN** the cost event is recorded, **THEN** `tokens_out` is NULL (never `0`) even though `cost_subcents` is positive - a row must never present real money against an unmeasured output count as if it were a measured zero. A turn that is `estimated=true` but still reports a real output count (a possible future adapter fallback shape) keeps that measured value; the NULL only applies to the conjunction of `estimated=true` and no output observed.
