...
- **Task / session subscriptions** — no per-key authorization; any client can subscribe to any `task_id` or `session_id`. Single-user model bounds the risk; a future iteration MUST gate on workspace membership. **Run subscriptions** — `run.subscribe` checks the caller against the run's workspace via `AuthorizeWorkspaceAccess`, denying when the run has none. See `apps/backend/AGENTS.md`.
