...
- **Right panels:** The right workbench column, normally Files and Changes above Terminal.
...
- **AC-UI-RIGHT-PANEL-VISIBILITY-001.3:** The control shall work in both tablet compositions and compact desktop mode.
  An explicit show action shall work even when the responsive default has no separate right column.
...
Showing Dockview panels retains its existing standard-right-column reconstruction behavior.
This feature does not promise restoration of every custom right-side tab arrangement.
