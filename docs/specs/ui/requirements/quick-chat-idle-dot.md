...
owners:
  - kandev
---
# Quick Chat Idle Dot Requirements

## Overview

Quick chat sessions keep running after the user closes the dialog. A session's turn can finish while the chat window is out of view, and nothing tells the user the agent has answered. Replies are missed until the user happens to reopen the dialog.

## Requirements

### REQ-UI-QUICK-CHAT-IDLE-DOT-001: Quick Chat Idle Dot

**Intent:** Quick chat sessions keep running after the user closes the dialog. A session's turn can finish while the chat window is out of view, and nothing tells the user the agent has answered. Replies are missed until the user happens to reopen the dialog.

#### Acceptance criteria

- **AC-UI-QUICK-CHAT-IDLE-DOT-001.1:** Each Quick Chat entry icon in the active workspace — the sidebar rail Quick Chat item, the sidebar Quick Chat shortcut, the tablet kanban header button, the mobile kanban header button, and the mobile task-switcher sheet button — shows a small red dot in the icon's corner while the Quick Chat dialog is closed and at least one quick chat session of that workspace has completed a turn (or settled to idle) since the dialog was last open.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.2:** A turn that completes while the dialog is open does NOT raise a dot; the user is looking at the chat window.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.3:** Opening the Quick Chat dialog clears every dot. Closing it re-arms tracking: a later turn completion while closed raises the dot again.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.4:** The dot is per-workspace: only sessions belonging to the active workspace contribute to that workspace's entry icons.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.5:** A session whose tab is closed or whose backing task is deleted stops contributing immediately.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.6:** The dot is decorative (`aria-hidden`). Button labels, tooltips, and the dialog itself are unchanged.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.7:** **GIVEN** no quick chat session in the active workspace, **WHEN** the app renders, **THEN** no Quick Chat entry icon shows a dot.
- **AC-UI-QUICK-CHAT-IDLE-DOT-001.8:** **GIVEN** a quick chat session with a running turn and the dialog closed, **WHEN** the turn completes (the session settles to an idle state), **THEN** a red dot appears on every Quick Chat entry icon of that workspace.
