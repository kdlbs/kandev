...
- **AC-UI-COMMAND-PANEL-TASK-ACTIVITY-001.2:** When a task is not active, each result shall show the same non-spinning task-state icon as the task sidebar.
...
- **AC-UI-COMMAND-PANEL-TASK-ACTIVITY-001.4:** When live task data changes, an open command panel shall update the affected icon without a new search.
