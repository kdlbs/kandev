...
- **AC-UI-QUICK-TERMINAL-001.11:** When an agent passthrough terminal owns keyboard focus inside the shared dialog, unmodified Escape shall reach the agent TUI and the dialog shall remain open. Escape from non-terminal focus targets retains the existing dialog and nested-widget behavior.
