...
- **AC-UI-MOBILE-TASK-CHROME-001.7:** When one or more plugins contribute session top-bar status or actions, the phone task header shall keep those contributions out of its persistent row and expose them in the shared phone menu's Plugins section. The menu shall preserve current task, workspace, active-session, and task-session context, identify its presentation as mobile, keep multiple or long contributions inside the menu viewport, and give controls created with `host.ui.Button` at least a 44 CSS-pixel active target. Tablet and desktop shall retain the inline top-bar presentation.
