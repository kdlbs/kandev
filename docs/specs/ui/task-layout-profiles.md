...
- When the active task gains a linked GitHub pull request or GitLab merge request and the selected layout does not contain PR Details, Kandev adds the canonical panel as an inactive tab beside the live Agent panel. The user's current tab remains selected.
- A conditionally added PR Details panel is removed when the active task has no linked review. A panel supplied by the selected profile or restored task layout remains in its configured location and keeps its normal empty state.
- Closing a conditionally added PR Details panel suppresses automatic re-creation for that session. The user can still reopen a specific review explicitly or add PR Details through the layout editor.
...
- Existing saved profiles and task-specific layouts are not rewritten to add or remove PR Details. The code-defined templates without PR Details affect fresh environments and explicit Reset Layout actions; users opt custom profiles into an always-available panel through the editor.
...
- **GIVEN** Kandev conditionally added PR Details for one task, **WHEN** the active task changes to one without a linked review, **THEN** the conditional panel is removed while any panel supplied by the incoming task's saved layout remains intact.
