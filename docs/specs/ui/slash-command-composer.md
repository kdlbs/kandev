...
- Selecting a slash command with Enter, Tab, or pointer/touch replaces only the active slash trigger range with the selected command text in the composer. It MUST NOT submit, queue, or otherwise send a chat message.
- The inserted command text is a normal editable draft. The user can add text before or after it, delete it, or leave the composer without starting an agent turn.
...
- **GIVEN** a task chat session with advertised commands, **WHEN** the user types `/s` and presses Enter to choose `/slow`, **THEN** the composer contains `/slow` as draft text, focus remains in the composer, and no user message or agent turn is created.
