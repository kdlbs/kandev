...
- Built-ins are limited to Kandev-owned state:
  ...
  - Phone metrics form one compact card in the Status drawer and the Menu fallback. The detailed Host marker shares the section heading line. Three equal columns keep the default CPU, memory, and disk readings together; additional metrics continue in enabled order on later rows. Detailed cells show translated labels, values, and percentage meters beneath the values. Labels wrap within their column. Simplified cells retain only their icons and values. Every indicator remains inside the metrics item without clipping or horizontal scrolling.
...
- **GIVEN** all host metrics are enabled on a phone, **WHEN** the user opens Status or the Menu fallback, **THEN** every host metric is visible in enabled order inside a three-column card; additional metrics continue on later rows without widening the metrics item, drawer, or document.
