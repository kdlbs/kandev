...
viewport retain their current geometry.
...
saved-prompt result from trigger through touch selection and focus retention;
existing mobile `#` and `/` scenarios provide sibling-consumer regressions.
