...
- Provider payloads normalize as follows:
  ...
  - OMP: keep the reported command visible once in the command row, omit the matching leading `$ <command>` presentation content from the output body, and preserve real output that immediately follows it without a separator. A structured final `rawOutput` that has no recognized shell-output field does not bypass this normalization.
