...
- Desktop and phone layouts expose the same capabilities and launch result. On phone, the existing mobile sessions entry point opens the dialog; the prompt menu follows the shared [composer suggestion overlay requirements](requirements/composer-suggestion-overlays.md), and the controls remain touch reachable without horizontal document overflow.
