...
A plugin system decouples integrations from core. Plugins run as separate processes, communicate with kandev over HTTP, and extend office through well-defined capabilities: receiving events, registering agent tools, exposing webhook endpoints, and reading/writing kandev data. The core stays small; the ecosystem grows independently.
...
All events from `internal/events/types.go` are available, plus office-specific events:
...
**Wildcard subscriptions:** Plugins can subscribe to patterns like `task.*`, `office.*`, or `agent.*`.
...
Plugins authenticate with their API key and can call existing office REST endpoints:

- `GET /api/office/tasks` -- list/search tasks
- `GET /api/office/tasks/{id}` -- get task detail
- `GET /api/office/tasks/{id}/comments` -- list comments
- `GET /api/office/agents` -- list agent instances
- `GET /api/office/agents/{id}` -- get agent detail
- `GET /api/office/projects` -- list projects
- `GET /api/office/costs` -- list cost events
- `GET /api/office/activity` -- list activity log
- `GET /api/office/approvals` -- list approvals

Access is gated by the plugin's declared `capabilities.api_read` list.

#### 6. Write kandev data via REST API

Plugins can create and modify office data:

- `POST /api/office/tasks` -- create a task
- `PATCH /api/office/tasks/{id}` -- update task fields (status, title, labels, assignee, etc.)
- `POST /api/office/tasks/{id}/comments` -- post a comment (with `source: "plugin:<plugin_id>"`)
- `POST /api/office/wakeups` -- queue a wakeup for an agent
- `POST /api/office/activity` -- log an activity entry
...
capabilities:
  events: ["task.created", "task.state_changed", "office.comment.created"]
...
- A plugin with `api_read: ["tasks"]` can call `GET /api/office/tasks` but not `GET /api/office/agents`.
...
The simplest useful plugin. Receives `task.created`, `task.state_changed`, `office.approval.created` events. Posts formatted messages to Slack channels. Config: bot token (secret ref), default channel, per-event toggles.
...
Migrates the existing `internal/jira/` integration to a plugin. Bidirectional sync between Jira tickets and office tasks: status mapping, comment bridging, webhook ingestion from Jira, periodic reconciliation via polling. Uses plugin state for link tracking (Jira issue ID to kandev task ID mappings).

#### 3. Custom metrics exporter (analytics plugin)

Subscribes to `office.cost.recorded`, `office.wakeup.processed`, `agent.completed`, `agent.failed`. Aggregates metrics and exposes a Prometheus `/metrics` endpoint. No agent tools, no webhooks, no state -- pure event consumer.
...
- Plugin registry: in-memory registry loaded from `~/.kandev/plugins/*.yml` on startup, with `fsnotify` for external changes (same pattern as office config).
...
- [office-overview](../office-overview/spec.md) -- Office feature overview, page structure
- [office-scheduler](../office-scheduler/spec.md) -- Wakeup queue drives event generation for plugins
- [office-skills](../office-skills/spec.md) -- Skills are the preferred extension mechanism for agent behavior; plugins extend the system, skills extend agents
- [office-assistant](../office-assistant/spec.md) -- Channels (Telegram, Slack) are the first candidates for plugin extraction
- [office-config](../office-config/spec.md) -- Filesystem-first config pattern reused for plugin registration storage
