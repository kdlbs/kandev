...
An SSH executor points at a host Kandev does not own. That host can move, sleep,
or change address at any time, and Kandev learns only when a launch fails.
`SSHExecutor.HealthCheck` returns `nil` unconditionally and the runtime
registry's health sweep runs once at backend startup, so no configured host is
re-checked while Kandev runs. The manual "Test connection" action is the only
reachability signal, and it sits on a settings page nobody has open when a host
goes away.
...
while the real cause was a stale address; two cards died mid-task and the cause
was found only by reading backend logs.
...
to the host we trusted? It puts that answer where a user already looks: the SSH
executor's settings page.
...
[executor system boundary](../README.md) already claims. The UI owns no
contract here: it renders executor state it does not define.
...
- **Reachability record:** the persisted outcome of the most recent probe for
  ...
  probed host, that probe's completion timestamp, and the timestamp of the most
  recent probe that succeeded.
...
- **Effective interval:** the interval in force after the normalization required
  by AC-EXECUTORS-SSH-REACHABILITY-001.24. Every rule below that refers to "the
  interval" means this value.
- **Sticky failure reason:** `config` or `host_key`. Both repeat on every later
  probe until a human changes something, so neither can be a transient blip.
...
- **AC-EXECUTORS-SSH-REACHABILITY-001.6:** When an executor's configuration cannot be resolved into a connection target, including a missing host and a missing pinned fingerprint, the system shall record reason `config` without attempting any network connection.
...
- **AC-EXECUTORS-SSH-REACHABILITY-001.24:** When the configured interval is absent, empty, or not a non-negative whole number, the system shall use the documented default. When it is `0` the system shall disable the poller. When it is above `0` but outside the documented supported range, the system shall clamp it to the nearer bound and log the clamp once at startup. Every case shall start the backend normally rather than refuse to boot, and every rule that depends on the interval shall use the resulting effective value.
...
**Intent:** Put the answer where the user already is. A probe result nobody sees
fixes nothing; the 2026-09-06 outage was diagnosed from backend logs. A user
whose agent is failing should be able to see that the runner is unreachable
without opening a log, and stop debugging the agent.
...
stand between a user and their own machine: the launch is attempted anyway.
...
- **AC-EXECUTORS-SSH-REACHABILITY-003.2:** When a launch targets an SSH executor whose state is `unreachable` and periodic probing is enabled, the system shall warn before the attempt, naming the host and the age of the last successful probe or stating that none has been recorded, and shall proceed without confirmation. When no interactive user is present, the warning shall be recorded against the launched session rather than shown as a prompt. When periodic probing is disabled, the system shall raise no warning, because a retained record is not a current measurement.
...
Two external prior-art legs were required and neither was available: the
`wiki-query` skill and its vault are absent here, and the `saas-kb` /
`search_fsm_docs` MCP server is not exposed to this session. Both are recorded
as unavailable, not as empty results.

In-repository prior art was available and did inform this specification:
`internal/integrations/healthpoll`, the poller behind the Jira and Linear
auth-health checks, and the existing SSH test-connection endpoint. This contract
departs from it in four places, because an SSH host is not an HTTP API; each is
recorded in the system design's `## Prior art and departures`.

## Out of scope

The first two entries are *deferred*, not rejected: each was cut for size and
seeds its own card. The rest are permanent.

- **Deferred: task-card reachability indicator.** Cards bound to an
  `unreachable` executor name that host, clear on return to `reachable`, and
  show nothing for `unknown`, `reachable`, an ineligible executor, or while
  probing is off, including a retained `unreachable` record. No polling and no
  new join: cards carry the primary executor's id and type, and every entry into
  and exit from `unreachable` is already pushed. Retires
  AC-EXECUTORS-SSH-REACHABILITY-002.8 and 002.9; neither is reused.
- **Deferred: launch path as a probe producer.** A launch's own dial is stronger
  evidence than a probe: success writes `reachable` with a zero counter and a
  dial-step failure writes a classified failure, both under the scheduled
  probe's reason, threshold, sticky-reason and last-write-wins rules, while a
  launch that never dials and any post-connection failure write nothing. Until
  it lands the poller is the only writer, so a host dying between passes lags
  the settings page by up to the failure threshold times the effective interval.
  AC-EXECUTORS-SSH-REACHABILITY-003.3 still names the host and reason on the
  failure itself. Retires AC-EXECUTORS-SSH-REACHABILITY-003.4 through 003.7;
  none is reused.
...
- **Probing remote capability on the cadence.** Platform detection, `git`
  presence, agent binary readiness, shell discovery, and the agentctl cache
  check stay on the manual test-connection endpoint.
- **Changing the existing manual test-connection endpoint.** It keeps dialing
  ...
  against a saved executor and its pinned fingerprint.
- **Rewriting error text produced by an agent.** The 2026-09-06 firewall message
  ...
  and does not intercept or edit agent output.
- **Per-session or per-forward liveness.** Whether a running session's port
  ...
  accepts a new connection, and is not answered here.
