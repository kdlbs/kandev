...
# Office: Recurring Scheduled Tasks (Routines)
...
- **DB** (`office_routines`): routine definition -- name, description, task template, assignee, concurrency policy, variables.
- **DB** (`office_routine_triggers`): operational trigger state -- `next_run_at`, `last_fired_at`, `public_id` (for webhooks), `enabled`. Needs atomic claims for cron scheduling.
- **DB** (`office_routine_runs`): run history -- status, linked task, trigger payload, timestamps.
- **Filesystem** (`routines/<name>.yml`): export format for git versioning. Written/read via Sync UI. See [office-config](../office-config/spec.md).
