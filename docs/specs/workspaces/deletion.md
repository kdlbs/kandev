...
- All workspace-owned data is removed: agents (+ memory, instructions, runtime, runs, continuation summaries), skills, projects, routines (+ triggers, runs), run events, run route attempts, run skill materializations, approvals, channels, labels, cost events, budget policies, wakeup queue entries, routing settings, provider health, activity logs, governance settings, workspace groups, tree holds, workspace settings, and the onboarding record.
