...
- Successful Monaco file saves first synchronize the persisted snapshot to every matching open language-server document, then notify servers that requested `textDocument/didSave`. Failed saves emit no save notification, and the persisted text is included only when the server advertises `includeText`.
...
- **GIVEN** an open Monaco document has a debounced content change and its server requests save synchronization, **WHEN** Kandev successfully persists the file, **THEN** the server receives the final `textDocument/didChange` before `textDocument/didSave` for its canonical task-host URI and receives the persisted snapshot on the save notification only when it requests `includeText`; a rejected write sends no save notification.
