...
- The assistant's skill defines which commands it understands and how to route them to the office API. This is instruction-based (skill content), not a hardcoded command parser.
...
- These are standard routines (see [office-routines](../office-routines/spec.md)) with the assistant as the assignee. The routine creates a task, the assistant processes it, and posts the result as a comment on the channel task (which relays to the platform).

### Agent memory

Agents need persistent knowledge that survives across sessions -- who the user is, what they prefer, what happened last week, lessons learned from past mistakes. Memory is stored in the **database** (`office_agent_memory` table) and can be exported to the filesystem via the Sync UI for sharing and backup.
...
- The skill instructs the agent to use a dedicated CLI tool (`kandev-memory` or `curl` against the office API) to read and write memory entries. This avoids using MCP tools, saving tokens -- the agent calls the CLI via shell commands as instructed by the skill.
- API endpoints exposed by the backend:
  ...
  - `GET /api/office/agents/<id>/memory/summary` -- return operating knowledge + recent knowledge entries (for session bootstrap).
...
- This pattern should be preferred going forward for office capabilities: teach the agent via skill instructions, expose simple API endpoints, let the agent call them with curl or a lightweight CLI.
...
- The agent creates a skill via the office API (same endpoint the UI uses) and adds it to its own `desired_skills` list.
- Use cases:
...
- Skill creation by agents goes through the same approval flow if `require_approval_for_skill_changes=true` (workspace setting, default true). The user reviews the proposed skill in the inbox before it takes effect. See [office-inbox](../office-inbox/spec.md) for the `skill_creation` approval type and workspace settings.
...
- `/office/agents/[id]` (agent detail page) is the central hub for an agent's configuration and state. It shows:
...
- [office-agents](../office-agents/spec.md) -- agent instances and permissions (`can_manage_own_skills`)
- [office-skills](../office-skills/spec.md) -- skill registry (agents create skills via the same API)
- [office-scheduler](../office-scheduler/spec.md) -- `task_comment` wakeup drives the chat flow
- [office-routines](../office-routines/spec.md) -- proactive work (digests, monitoring)
- [office-inbox](../office-inbox/spec.md) -- approval flow for skill changes and hire requests via chat
