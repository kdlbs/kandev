...
func NewMockTaskRepository(log *logger.Logger) *MockTaskRepository {
	return &MockTaskRepository{
		tasks:  make(map[string]*v1.Task),
		logger: log.WithFields(zap.String("component", "mock_task_repo")),
	}
}

// GetTask retrieves a task by ID
func (r *MockTaskRepository) GetTask(ctx context.Context, taskID string) (*v1.Task, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	task, exists := r.tasks[taskID]
	if !exists {
		r.logger.Debug("task not found", zap.String("task_id", taskID))
...
func (r *MockTaskRepository) UpdateTaskState(ctx context.Context, taskID string, state v1.TaskState) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	task, exists := r.tasks[taskID]
	if !exists {
		r.logger.Debug("task not found for state update", zap.String("task_id", taskID))
		return ErrTaskNotFound
	}

	task.State = state
	task.UpdatedAt = time.Now()
	r.logger.Info("task state updated",
		zap.String("task_id", taskID),
...
func (r *MockTaskRepository) AddTask(task *v1.Task) {
	r.mu.Lock()
	defer r.mu.Unlock()

	r.tasks[task.ID] = task
	r.logger.Debug("task added to mock repository", zap.String("task_id", task.ID))
}
