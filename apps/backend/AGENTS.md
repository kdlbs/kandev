...
- **Two session paths bypass the task service and must guard themselves.** (1) Handlers that resolve an execution by a bare in-memory lookup (`GetExecutionBySessionID`, `*BySessionID`) skip the `GetOrEnsure*` chokepoint where the lifecycle check runs — call `service.AuthorizeSessionAccess` (see `ProcessHandlers.denySessionAccess`) or `lifecycleMgr.CheckSessionAccess` first. (2) The orchestrator reads sessions through its own repo handle, so it inherits nothing from the task service; session-keyed entry points there must call `authorizeSession` (the hook is wired by `SetSessionAccessChecker`). **Coverage there is currently partial** — only `GetTaskSessionStatus` and `CheckSessionPR` consult it. The session-mutating WS actions (`session.stop`, `session.delete`, `session.set_primary`, `session.rename`, `session.set_plan_mode`, `session.reset_context`, `session.recover`, `session.launch`, `session.ensure`, `agent.cancel`, `permission.respond`) and `internal/agent/handlers` `agent.stdin` / `agent.resize` still lack it. Add `authorizeSession` when you touch one, and do not assume the orchestrator is scoped.
