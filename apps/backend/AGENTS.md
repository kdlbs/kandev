...
**Worktrees:** `internal/worktree/Manager` provides workspace isolation. Each session can have its own worktree (branch) to prevent conflicts between concurrent agents. `GitMetadataProjection` is the shared, revalidated permission boundary: never derive grants from `.git` pointers or reintroduce a broad `MainRepoGitDir` writable mount; unsupported native policies fail closed.
