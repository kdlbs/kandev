...
**Surviving agentctl adoption:** A compatible retained agentctl is adopted through its authenticated delivery descriptor. Restore the SQL generation, stream owner, projected cursor, and active submission before connecting intake or admitting prompts. Replay bounded pages through the normal inbox, canonical projection, lifecycle, and ACK path. Do not call ACP initialize, load, resume, or new during adoption; any identity, journal, cursor, or projection error fails closed and keeps Stop available. `features.agentSurvival` controls process lifetime only.
