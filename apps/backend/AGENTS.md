...
- **Port-proxy subtree capability:** with auth enforced, `/port-proxy/:sessionId/:port/*` needs a credential on _every_ request, but browser subresource fetches cannot always carry one (the `<link rel="manifest">` fetch never sends cookies; sandboxed iframes and `?token=`-opened previews drop them elsewhere). `PortProxyHandler` mints a short-lived (15 min, sliding), path-scoped, HMAC-signed capability after the document authenticates and propagates it as a cookie scoped to the proxy subtree plus a `kandev_cap` query parameter appended to rewritten asset URLs. `requireConnectionAuth` (`internal/gateway/websocket/access.go`) accepts either form only for the exact session:port it was minted for and restores the issuing identity, so `CheckSessionAccess` still gates on the real owner. Synthetic identities (auth disabled) skip minting — proxied bodies stay byte-identical to the pre-auth behavior.
- **Self-authenticating webhooks** (automation, office channels) and `/health`, `/api/v1/features`, `/api/v1/app-state` stay public — the allowlist lives in `auth/httpmw/middleware.go` with a pinning test. Plugin webhooks are NOT in that allowlist: `auth/httpmw` structurally defers GET/POST `/api/plugins/*/webhooks/*` (it has no access to the plugin registry to know policy), and `internal/plugins.Controller.webhook` enforces the actual gate. API v1 keeps omitted webhook access public for compatibility; API v2 defaults it to authenticated. Session-authenticated calls also require an accepted Origin. The plugin-generation lease covers the RPC and response side effects. See `docs/decisions` for the plugin-webhook-auth-gate ADR.
