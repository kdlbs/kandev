...
**Worktree file materialization:** Two overlapping mechanisms seed files into a new worktree. `copy_files` (legacy, `Repository.CopyFiles`) copies gitignored files by pattern. `worktree_files` (`Repository.WorktreeFiles`, list of `{path, mode}`) materializes each file with its own mode: `copy` (default) or `symlink` (links back to the main repo so shared files like `.env.local` stay centralized). Per-file `FileSpec` + logic live in `internal/worktree/materialization.go`; `Manager.materializeWorktreeFiles` (in `manager_lifecycle.go`) runs alongside `copyConfiguredFiles`. The launch path often lacks a `RepositoryID`, so config is resolved by ID when present and otherwise by local path (`GetRepositoryByLocalPath`). Non-fatal like copy-files; symlinks use relative targets (unix/macOS).
