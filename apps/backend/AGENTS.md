...
**Remote SSH lifecycle:** Resolve and run remote preparation, then verify the canonical checkout, origin, and `HEAD` before starting agentctl. Retain the remote task directory in session state for stop/resume. `SSHExecutor.StopInstance` never removes the task checkout at `<workdir_root>/tasks/<taskDirName>/` on any stop reason, including terminal (archive/delete/cascade) — it is left intact until a v2 housekeeper sweeps stale directories (out of scope per `docs/specs/ssh-executor/spec.md`). Terminal stop reasons additionally run the executor profile's optional `cleanup_script` over the live SSH connection (empty on every seeded profile, so a no-op by default). Every stop reason except graceful backend shutdown kills the remote agentctl process and removes its per-session runtime directory.
...
- **Self-authenticating webhooks** (automation and office channels) and `/health`, `/api/v1/features`, `/api/v1/app-state` stay public — the allowlist lives in `auth/httpmw/middleware.go` with a pinning test. Plugin webhooks are not allowlisted: `auth/httpmw` structurally defers only GET/POST relay paths because it cannot read manifests, and `internal/plugins.Controller.webhook` enforces each manifest's `access` policy.
