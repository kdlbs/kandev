...
- Production Git commands must use `subproc.NewGitCommand` with a classified
  `subproc.RunGit*` helper, or hold a classified admission slot across streaming
  `Start`/`Wait`. Do not construct raw Git commands outside
...
- **PR status sync coverage:** A `github_pr_watches` row is unique per (session, repository, branch) and gets re-pointed when the session's branch moves on, so watches are not a complete index of a task's PRs. Any code path that refreshes PR status must also cover `github_task_prs` rows no watch points at (`unwatchedTaskPRs` / `syncUnwatchedTaskPRs` in `service_pr_unwatched.go`) — a watch-only sweep leaves every PR from an earlier branch frozen at its last-observed state, which the UI renders as an open, mergeable PR long after it merged. Terminal (merged/closed) and detached rows are intentionally excluded so the reconciliation set stays bounded.
