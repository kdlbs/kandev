...
- **Self-authenticating webhooks** (automation, office channels, plugin webhooks) and `/health`, `/ready`, `/api/v1/features`, `/api/v1/app-state` stay public — the allowlist lives in `auth/httpmw/middleware.go` with a pinning test.
