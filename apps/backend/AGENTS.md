...
- **Self-authenticating webhooks** (automation, office channels) and `/health`, `/api/v1/features`, `/api/v1/app-state` stay public — the allowlist lives in `auth/httpmw/middleware.go` with a pinning test. Plugin webhooks are NOT in that allowlist: `auth/httpmw` structurally defers `/api/plugins/*/webhooks/*` (it has no access to the plugin registry to know policy), and `internal/plugins.Controller.webhook` enforces the actual gate — a webhook requires a real caller identity (session/PAT, or the synthetic identity while auth is disabled) unless its manifest declares `webhooks[].public: true`. See `docs/decisions` for the plugin-webhook-auth-gate ADR.
