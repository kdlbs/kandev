...
import (
	...
	"github.com/kandev/kandev/internal/orchestrate"
...
func provideRepositories(cfg *config.Config, log *logger.Logger) (*db.Pool, *Repositories, []func() error, error) {
	...
	orchestrateRepo, _, orchestrateCleanup, err := orchestrate.Provide(writer, reader, log)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("orchestrate repo: %w", err)
	}
	cleanups = append(cleanups, orchestrateCleanup)
