...
- list_related_tasks_kandev: List parent, child, sibling, blocker, and blocked tasks. By default it returns compact task data. An authorized Office Coordinator may inspect an unrelated task in this workspace, but descriptions and document keys still require normal task-document access. Optional: task_id (defaults to the current task), verbose (include authorized task descriptions; omitted by default).
