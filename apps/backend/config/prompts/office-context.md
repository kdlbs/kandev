...
- step_complete_kandev: Signal that every requirement for the current workflow step is satisfied. Call it as the last action, never before a question or during partial progress. On a step that requires it, the step does not advance until this call is accepted: ending your turn without it leaves the task parked. Required: summary.
