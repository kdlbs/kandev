...
- list_related_tasks_kandev: List parent, child, sibling, blocker, and blocked tasks. Access is relation-scoped by default. A persisted Office Coordinator may inspect compact unrelated task trees in its own workspace; this does not grant descriptions or document keys. verbose requires normal task-document access. Optional: task_id (defaults to the current task), verbose (include authorized task descriptions; omitted by default).
