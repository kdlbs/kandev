...
func TestRegisterWsSentTestRouteRejectsInvalidSinceSeq(t *testing.T) {
	gin.SetMode(gin.TestMode)
	tests := []string{"-1", "notanumber"}

	for _, sinceSeq := range tests {
		t.Run(sinceSeq, func(t *testing.T) {
			router := gin.New()
			registerWsSentTestRoute(router, &fakeWsSentSource{}, logger.Default())
			...
			req := httptest.NewRequest(
				http.MethodGet,
				"/api/v1/_test/ws-sent?connection_id=conn-1&since_seq="+sinceSeq,
				nil,
			)
			router.ServeHTTP(rec, req)
			...
			if body := rec.Body.String(); !strings.Contains(body, "since_seq must be a non-negative integer") {
				t.Fatalf("response body %s missing since_seq validation error", body)
			}
		})
	}
}
