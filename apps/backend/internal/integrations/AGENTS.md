...
- **If the watcher has a per-watch `MaxInflightTasks` cap**, register the integration's task-metadata watch-id key in `watcherMetadataKeyByIntegration` (`internal/task/repository/sqlite/task.go`). The orchestrator throttle gate calls `CountOpenWatcherCreatedTasks(integration, watchID)`, which uses that map to count open tasks; an unregistered integration returns 0 and the cap **silently never applies**. Sentry shipped with this gap (the cap was stored and validated but not enforced). The metadata key must match the one `BuildTaskRequest` writes in the source's `source_<name>.go` (e.g. `sentry_issue_watch_id`).
