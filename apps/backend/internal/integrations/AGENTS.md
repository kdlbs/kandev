...
## Where Sentry deliberately diverges

- **Multiple instances, not a singleton:** unlike Jira/Linear (one install-wide config), `sentry_configs` holds **N named instances** keyed by a generated UUID `id` + `name`. The HTTP surface is `/api/v1/sentry/instances` (CRUD) rather than `/config`; secrets are keyed `sentry:<instanceID>:token` (`secretKeyFor`); the service caches one client per instance (`clients map[string]Client` + per-id generation counter). Browse endpoints (`/organizations`, `/projects`, `/issues`) require an `instanceId` query param, and each `sentry_issue_watches` row carries a `sentry_instance_id` FK selecting which instance it polls. Deleting an instance is blocked with HTTP 409 (`SENTRY_INSTANCE_IN_USE`) while watches still reference it. A legacy single-tenant DB is migrated on boot: the `'singleton'` row is promoted to a generated id (named after its URL host) and its token re-keyed (`migrateLegacySecret`, crash-safe).
