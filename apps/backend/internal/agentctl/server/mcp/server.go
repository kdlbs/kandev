...
func (s *Server) registerKanbanTools() {
	...
	s.mcpServer.AddTool(
		mcp.NewTool("create_task",
			mcp.WithDescription(`Create a new task or subtask.
...
- Set start_agent=false to create without starting an agent`),
			...
			mcp.WithString("repository_id", mcp.Description("Repository ID for top-level tasks. Use list_repositories to find available IDs. Not needed for subtasks (inherited from parent).")),
