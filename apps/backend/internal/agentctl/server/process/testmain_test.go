...
func TestMain(m *testing.M) {
	...
	// An inherited KANDEV_GITLAB_HOST / GITLAB_TOKEN then leaks into GitLab
	// remote-host detection and PR-provider auth, failing tests that assume
	// these are unset. Scrub them here so the suite is hermetic regardless of
	// the parent shell's environment; tests that need a specific value set it
