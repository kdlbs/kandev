...
func (r *Repository) GetSkillBySlug(
	...
	err := r.ro.QueryRowxContext(ctx, r.ro.Rebind(
		`SELECT * FROM orchestrate_skills WHERE workspace_id = ? AND slug = ?`),
...
func (r *Repository) ListSkills(ctx context.Context, workspaceID string) ([]*models.Skill, error) {
	...
	if workspaceID == "" {
		err = r.ro.SelectContext(ctx, &skills,
			`SELECT * FROM orchestrate_skills ORDER BY name`)
	} else {
		err = r.ro.SelectContext(ctx, &skills, r.ro.Rebind(
			`SELECT * FROM orchestrate_skills WHERE workspace_id = ? ORDER BY name`), workspaceID)
	}
	if err != nil {
		return nil, err
	}
