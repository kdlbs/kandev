...
// publishCommentCreated emits an OrchestrateCommentCreated event.
...
func (s *Service) publishCommentCreated(ctx context.Context, comment *models.TaskComment) {
	...
	event := bus.NewEvent(events.OrchestrateCommentCreated, "orchestrate-service", data)
	if err := s.eb.Publish(ctx, events.OrchestrateCommentCreated, event); err != nil {
