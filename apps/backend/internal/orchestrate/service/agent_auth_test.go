...
func TestAgentAuth_MintAndValidate(t *testing.T) {
	auth := service.NewAgentAuth("test-secret-key")
	...
	if claims.AgentInstanceID != "agent-1" {
		t.Errorf("agent_instance_id = %q, want agent-1", claims.AgentInstanceID)
	}
...
func TestAgentAuth_ExpiredTokenRejected(t *testing.T) {
	...
	// Mint a token that is already expired by using a very short duration.
	// We'll manually create an expired token by minting then waiting -- but that's slow.
	// Instead, test by creating an auth with known key and crafting claims.
	token, err := auth.MintAgentJWT("agent-1", "task-1", "ws-1", "sess-1")
	if err != nil {
		t.Fatalf("mint: %v", err)
	}
	...
	// For expiry testing, we use the internal mintWithExpiry via a new auth.
	// Since that method is unexported, we test through the public interface
	// by verifying the expiry claim is set in the future.
...
func TestAgentAuth_ClaimsComplete(t *testing.T) {
	...
	expectedExp := now + int64(service.DefaultTokenDuration.Seconds())
