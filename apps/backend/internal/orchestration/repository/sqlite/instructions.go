...
func (r *Repository) ListInstructions(ctx context.Context, agentInstanceID string) ([]*models.InstructionFile, error) {
	...
	err := r.ro.SelectContext(ctx, &files, r.ro.Rebind(
		`SELECT * FROM orchestration_instructions WHERE agent_profile_id = ? ORDER BY is_entry DESC, filename`),
		agentInstanceID)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetInstruction(
	...
	err := r.ro.QueryRowxContext(ctx, r.ro.Rebind(
		`SELECT * FROM orchestration_instructions WHERE agent_profile_id = ? AND filename = ?`),
...
func (r *Repository) UpsertInstruction(
	...
	_, err := r.db.ExecContext(ctx, r.db.Rebind(`
		INSERT INTO orchestration_instructions (
...
func (r *Repository) DeleteInstruction(ctx context.Context, agentInstanceID, filename string) error {
	_, err := r.db.ExecContext(ctx, r.db.Rebind(
		`DELETE FROM orchestration_instructions WHERE agent_profile_id = ? AND filename = ?`),
