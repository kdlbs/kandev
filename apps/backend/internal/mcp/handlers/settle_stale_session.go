...
func staleSettlementAuthority(actor *models.Task, actorSession *models.TaskSession, target *models.Task, targetSession *models.TaskSession) (string, bool) {
	...
	if supervision, ok := models.LoadSessionSpawnSupervision(targetSession.Metadata); ok &&
		supervision.SupervisorTaskID == actor.ID && supervision.SupervisorSessionID == actorSession.ID {
