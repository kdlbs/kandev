...
func TestHandleCreateTask_UsesVerifiedCreatorSessionProfileAndRuntime(t *testing.T) {
	...
	principalCtx := streams.WithMCPExecutionContext(ctx, streams.MCPExecutionContext{
		ExecutionID: "creator-execution", TaskID: sourceResult.Task.ID, SessionID: "creator-session",
	})
	principalCtx, err = principalResolver.ScopePrincipal(principalCtx, sourceResult.Task.ID, "creator-session")
	require.NoError(t, err)
