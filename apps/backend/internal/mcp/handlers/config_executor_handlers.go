...
func (h *Handlers) handleCreateExecutor(ctx context.Context, msg *ws.Message) (*ws.Message, error) {
	var req struct {
	...
	if req.Type == "" {
		return ws.NewError(msg.ID, msg.Action, ws.ErrorCodeValidation, "type is required", nil)
	}
	if req.Status == "" {
		req.Status = models.ExecutorStatusActive
	}

	executor, err := h.taskSvc.CreateExecutor(ctx, &service.CreateExecutorRequest{
		Name:      req.Name,
		Type:      req.Type,
		Status:    req.Status,
	...
	if err != nil {
		...
		return ws.NewError(msg.ID, msg.Action, ws.ErrorCodeInternalError, "Failed to create executor: "+err.Error(), nil)
	}
	return ws.NewResponse(msg.ID, msg.Action, dto.FromExecutor(executor))
}

func (h *Handlers) handleUpdateExecutor(ctx context.Context, msg *ws.Message) (*ws.Message, error) {
	var req struct {
	...
	executor, err := h.taskSvc.UpdateExecutor(ctx, req.ExecutorID, &service.UpdateExecutorRequest{
		Name:      req.Name,
		Status:    req.Status,
		Resumable: req.Resumable,
		Config:    req.Config,
	...
	if err != nil {
		...
		return ws.NewError(msg.ID, msg.Action, ws.ErrorCodeInternalError, "Failed to update executor: "+err.Error(), nil)
	}
	return ws.NewResponse(msg.ID, msg.Action, dto.FromExecutor(executor))
}

func (h *Handlers) handleDeleteExecutor(ctx context.Context, msg *ws.Message) (*ws.Message, error) {
	return h.handleDeleteByField(ctx, msg, "executor_id", "failed to delete executor", "Failed to delete executor",
		func(ctx context.Context, id string) error { return h.taskSvc.DeleteExecutor(ctx, id) })
}
