...
drained:
	...
	require.Len(t, tools, 40, "final registry should contain the complete task tool set")
	assert.Contains(t, tools, "get_task_mr_automation_kandev")
	assert.NotContains(t, tools, "get_task_pr_automation_kandev")
	assert.Contains(t, tools, "request_fresh_ci_run_kandev")
}
...
func TestServerModeTask_ToolCount(t *testing.T) {
	...
	// + 2 task-dependency (add/remove) + 1 scoped CI run + 3 task change-link
	// operations + 1 rich-output = 43.
