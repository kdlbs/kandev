...
func TestServerModeTask_ToolCount(t *testing.T) {
	...
	// 1 publish_review_findings + 1 list_review_findings + 1 resolve_review_finding
	// + 1 related-tasks + 1 diagnostic bundle
	// + 2 task-dependency (add/remove) + 3 task change-link operations + 1 rich-output = 44.
	...
	assert.Contains(t, tools, "list_review_findings_kandev", "review findings read access must be registered in task mode")
	assert.Contains(t, tools, "resolve_review_finding_kandev", "review finding resolution must be registered in task mode")
