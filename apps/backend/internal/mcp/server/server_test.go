...
func TestServerSurfaceAutomationHasFixedCoordinatorCatalog(t *testing.T) {
	...
	want := []string{
		...
		"settle_stale_session_kandev", "get_message_delivery_kandev", "retry_message_delivery_kandev",
	}
...
func TestServerModeTask_ToolCount(t *testing.T) {
	...
	// 1 neutral outcome tool + 1 rich-output + 1 exact stale-session recovery
	// + 2 durable delivery recovery tools. The exact count below guards the
	// complete task-mode catalog.
	...
	assert.Contains(t, tools, "settle_stale_session_kandev")
	assert.Contains(t, tools, "get_message_delivery_kandev")
	assert.Contains(t, tools, "retry_message_delivery_kandev")
