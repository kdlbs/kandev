...
func (s *Server) registerSessionWakeTools() {
	...
	s.mcpServer.AddTool(mcp.NewTool("upsert_session_wake_kandev", mcp.WithDescription("Create, update, or reactivate one recurring wake identified by marker for this session."), mcp.WithString("marker", mcp.Required()), mcp.WithString("prompt", mcp.Required()), mcp.WithString("cron_expression", mcp.Required()), mcp.WithString("expires_at", mcp.Required()), mcp.WithString("timezone")), s.wrapHandler("upsert_session_wake_kandev", s.sessionWakeHandler(ws.ActionMCPUpsertSessionWake)))
