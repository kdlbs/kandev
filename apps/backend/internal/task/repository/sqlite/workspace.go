...
func (r *Repository) CreateWorkspace(ctx context.Context, workspace *models.Workspace) error {
	...
	_, err := r.db.ExecContext(ctx, r.db.Rebind(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`), workspace.ID, workspace.Name, workspace.Description, workspace.OwnerID, workspace.DefaultExecutorID, workspace.DefaultEnvironmentID, workspace.DefaultAgentProfileID, workspace.DefaultConfigAgentProfileID, workspace.TaskPrefix, workspace.TaskSequence, workspace.OrchestrateWorkflowID, workspace.CreatedAt, workspace.UpdatedAt)
...
func (r *Repository) GetWorkspace(ctx context.Context, id string) (*models.Workspace, error) {
	...
	err := r.ro.QueryRowContext(ctx, r.ro.Rebind(`
		SELECT id, name, description, owner_id, default_executor_id, default_environment_id, default_agent_profile_id, default_config_agent_profile_id, task_prefix, task_sequence, orchestrate_workflow_id, created_at, updated_at
	...
	`), id).Scan(
		...
		&workspace.OrchestrateWorkflowID,
		&workspace.CreatedAt,
		&workspace.UpdatedAt,
	)
