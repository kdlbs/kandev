...
// workspace_id, target_step_id, and turn_id use compatible-value checks
// because real producers fill them progressively rather than knowing them
// up front. A blank value may be filled or omitted by a later write, but two
// different non-blank values are an identity conflict. step_complete is the
// concrete case:
...
// a value task_id already determines. Requiring exact equality on any of
// these three fields would make that second write match zero rows and fail
// as ErrOperationIdentityConflict, permanently blocking the primary
...
func (r *Repository) recordWorkflowRouteOperationTx(
	...
	result, err := tx.ExecContext(ctx, r.db.Rebind(`
		...
		ON CONFLICT(id) DO UPDATE SET
			...
			target_step_id = CASE WHEN workflow_route_operations.target_step_id = '' THEN excluded.target_step_id ELSE workflow_route_operations.target_step_id END,
		...
		WHERE workflow_route_operations.task_id = excluded.task_id
			AND (workflow_route_operations.workspace_id = '' OR excluded.workspace_id = '' OR workflow_route_operations.workspace_id = excluded.workspace_id)
			...
			AND (workflow_route_operations.target_step_id = '' OR excluded.target_step_id = '' OR workflow_route_operations.target_step_id = excluded.target_step_id)
			AND COALESCE(workflow_route_operations.session_id, '') = COALESCE(excluded.session_id, '')
			AND (workflow_route_operations.turn_id IS NULL OR excluded.turn_id IS NULL OR workflow_route_operations.turn_id = excluded.turn_id)
