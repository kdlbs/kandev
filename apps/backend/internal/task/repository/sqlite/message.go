...
func (r *Repository) insertMessageRow(
	...
	_, err := execer.ExecContext(ctx, r.db.Rebind(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`), message.ID, message.TaskSessionID, message.TaskID, turnID, message.AuthorType, message.AuthorID, message.Content, requestsInput, messageType, metadataJSON, message.CreatedAt, message.UpdatedAt, message.PromptIndex, message.PayloadDigest, message.PayloadSize)
	return err
}
...
func (r *Repository) GetMessage(ctx context.Context, id string) (*models.Message, error) {
	...
	`), id).Scan(&message.ID, &message.TaskSessionID, &message.TaskID, &turnID, &message.AuthorType, &message.AuthorID, &message.Content, &requestsInput, &messageType, &metadataJSON, &message.CreatedAt, &message.UpdatedAt, &message.PayloadDigest, &message.PayloadSize)
	if err != nil {
		return nil, err
	}
