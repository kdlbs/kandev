...
type legacyEnv struct {
	...
	containerID, sandboxID, authSecretID, bootstrapSecretID, taskDirName  string
...
func (c *worktreeCutover) loadLegacyEnvs(tx *sqlx.Tx, columns map[string]bool) error {
	...
	rows, err := tx.Query(fmt.Sprintf(`
		SELECT id, task_id, %s, executor_type, executor_id,
			...
			COALESCE(agentctl_auth_secret_id, ''),
			COALESCE(agentctl_bootstrap_secret_id, ''),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&env.id, &env.taskID, &env.repositoryID, &env.executorType,
			...
			&env.containerID, &env.sandboxID, &env.authSecretID, &env.bootstrapSecretID,
