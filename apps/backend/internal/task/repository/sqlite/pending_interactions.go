...
// interaction. Rows are ordered oldest-first by (created_at, id).
...
func buildPendingInteractionQuery(
	...
	// current_turn duplicates the ranking in pendingActionsBySessionQuery on
	// purpose: both must agree, and a shared builder would have to thread the
	// scoping subquery's bind arguments through the projection's placeholder
	// list. The pinning test (TestPendingInteractionsAgreeWithPendingAction)
	// fails if the two ever disagree on a session.
	query := fmt.Sprintf(`
