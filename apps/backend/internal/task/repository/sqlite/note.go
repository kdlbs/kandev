...
const taskNoteSelectCols = `id, task_id, content, updated_by, created_at, updated_at`

// GetTaskNote retrieves a task note by task ID.
func (r *Repository) GetTaskNote(ctx context.Context, taskID string) (*models.TaskNote, error) {
...
func (r *Repository) UpsertTaskNote(ctx context.Context, note *models.TaskNote) error {
	...
	`), note.ID, note.TaskID, note.Content, note.UpdatedBy, note.CreatedAt, note.UpdatedAt); err != nil {
		return fmt.Errorf("failed to upsert task note: %w", err)
	}
	return nil
}

// DeleteTaskNote deletes a task note by task ID.
func (r *Repository) DeleteTaskNote(ctx context.Context, taskID string) error {
	result, err := r.db.ExecContext(ctx, r.db.Rebind(`DELETE FROM task_notes WHERE task_id = ?`), taskID)
	if err != nil {
