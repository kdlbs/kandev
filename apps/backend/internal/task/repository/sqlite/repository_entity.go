...
func (r *Repository) CreateRepository(ctx context.Context, repository *models.Repository) error {
	...
	_, err := r.db.ExecContext(ctx, r.db.Rebind(`
		INSERT INTO repositories (
			...
			provider_name, default_branch, worktree_branch_prefix, pull_before_worktree, setup_script, cleanup_script, dev_script, copy_files, startup_prompt, created_at, updated_at, deleted_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`), repository.ID, repository.WorkspaceID, repository.Name, repository.SourceType, repository.LocalPath, repository.Provider,
		...
		dialect.BoolToInt(repository.PullBeforeWorktree), repository.SetupScript, repository.CleanupScript, repository.DevScript, repository.CopyFiles, repository.StartupPrompt, repository.CreatedAt, repository.UpdatedAt, repository.DeletedAt)
...
func (r *Repository) GetRepository(ctx context.Context, id string) (*models.Repository, error) {
	...
	`), id).Scan(
		...
		&repository.DefaultBranch, &repository.WorktreeBranchPrefix, &repository.PullBeforeWorktree, &repository.SetupScript, &repository.CleanupScript, &repository.DevScript, &repository.CopyFiles, &repository.StartupPrompt, &repository.CreatedAt, &repository.UpdatedAt, &repository.DeletedAt,
	)
...
func (r *Repository) UpdateRepository(ctx context.Context, repository *models.Repository) error {
	...
	result, err := r.db.ExecContext(ctx, r.db.Rebind(`
		UPDATE repositories SET
			...
			provider_name = ?, default_branch = ?, worktree_branch_prefix = ?, pull_before_worktree = ?, setup_script = ?, cleanup_script = ?, dev_script = ?, copy_files = ?, startup_prompt = ?, updated_at = ?
