...
func (r *Repository) GetExecutorRunningByResumeToken(ctx context.Context, resumeToken string) (*models.ExecutorRunning, error) {
	if resumeToken == "" {
		return nil, fmt.Errorf("resume_token is required")
	}
	running := &models.ExecutorRunning{}
	var resumable int
	var lastSeen sql.NullTime
