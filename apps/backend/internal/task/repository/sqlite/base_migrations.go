...
// ensureWorkspaceCoordinatorGrantSchema keeps the Coordinator designation
// relation workspace-bound on both fresh and upgraded installations.
func (r *Repository) ensureWorkspaceCoordinatorGrantSchema() error {
	...
	return r.recreateTableNamed(
		...
		[]string{
			...
			JOIN tasks task ON task.id = coordinator_grant.coordinator_task_id
...
func (r *Repository) migrateLegacyPostgresWorkspaceCoordinatorGrantSchema() error {
	...
	_, err := r.db.Exec(`
		...
		WHERE coordinator_grant.workspace_id = ''
			...
			OR NOT EXISTS (SELECT 1 FROM tasks task
		...
		ALTER TABLE workspace_coordinator_grants
			...
			REFERENCES tasks(workspace_id, id) ON DELETE CASCADE;
		ALTER TABLE workspace_coordinator_grants
			ADD CONSTRAINT workspace_coordinator_grants_workspace_id_nonempty CHECK (workspace_id <> '');
		ALTER TABLE workspace_coordinator_grants
			ADD CONSTRAINT workspace_coordinator_grants_coordinator_task_id_nonempty CHECK (coordinator_task_id <> '');
	`)
	if err != nil {
