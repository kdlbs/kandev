	...
	for _, stepDef := range template.Steps {
		...
		if _, err := tx.ExecContext(ctx, r.db.Rebind(`
			INSERT INTO workflow_steps (
				...
				auto_archive_after_hours, agent_profile_id, profile_session_policy, stage_type,
		...
		`), idMap[stepDef.ID], workflowID, stepDef.Name, stepDef.Position, stepDef.Color, stepDef.Prompt, string(events),
			...
			stepDef.AutoArchiveAfterHours, stepDef.AgentProfileID, models.NormalizeWorkflowProfileSessionPolicy(string(stepDef.ProfileSessionPolicy)), normalizeBootstrapStageType(stepDef.StageType),
