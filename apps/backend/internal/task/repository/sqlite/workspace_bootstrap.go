...
func (r *Repository) insertTemplateSteps(ctx context.Context, tx *sqlx.Tx, workflowID string, template *wfmodels.WorkflowTemplate) error {
	...
	for _, stepDef := range template.Steps {
		...
		sessionTarget, err := json.Marshal(wfmodels.RemapWorkflowSessionTarget(stepDef.SessionTarget, idMap))
		if err != nil {
			return fmt.Errorf("marshal Kanban session target %q: %w", stepDef.Name, err)
		}
