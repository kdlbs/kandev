...
func (h *TaskHandlers) registerHTTP(router *gin.Engine) {
	...
	api.GET("/task-sessions/:id/fork-candidates", h.httpListConversationForkCandidates)
	api.POST("/task-sessions/:id/conversation-forks", h.httpCreateConversationForkDraft)
	api.GET("/conversation-forks/:id", h.httpGetConversationForkDraft)
	api.GET("/conversation-forks/:id/content", h.httpGetConversationForkContent)
	api.POST("/conversation-forks/:id/estimate", h.httpEstimateConversationForkDraft)
	api.DELETE("/conversation-forks/:id", h.httpDiscardConversationForkDraft)
