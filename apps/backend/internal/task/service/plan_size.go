...
// depending on mode (AC-TASKS-PLAN-APPEND-007.1/007.4). In replace mode
// (validatePlanWrite) it runs once, before plan storage or the per-task write
// lock, against the submitted content. In append mode (upsertPlan) it runs a
// second time, inside the per-task write lock, against the composed content
// (stored content plus fragment) once composition has produced it — not the
// submitted fragment alone, which could be small enough to admit an
// oversized document.
