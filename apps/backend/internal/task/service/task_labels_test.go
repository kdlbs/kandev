...
func TestValidateTaskLabels(t *testing.T) {
	tests := []struct {
		...
		{name: "null", encoded: "null", wantErr: "labels must be a JSON array"},
		{name: "plain string scalar", encoded: `"hello"`, wantErr: "labels must be a JSON array"},
		{name: "integer scalar", encoded: "42", wantErr: "labels must be a JSON array"},
		{name: "boolean scalar", encoded: "true", wantErr: "labels must be a JSON array"},
		{name: "non-array JSON", encoded: `{"key":"val"}`, wantErr: "labels must be a JSON array"},
