...
// string means replace (AC-TASKS-PLAN-APPEND-001.1), the default every
// existing caller relies on. Any other value — including one differing only
// in letter case, such as "Append" or "APPEND" — is rejected
// (AC-TASKS-PLAN-APPEND-001.3): it must not be interpreted as replace,
