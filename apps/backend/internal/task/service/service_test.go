...
// slowExecutionStopper records the time spent inside StopExecution so tests
// can assert the caller waited (or did not wait) for cleanup.
type slowExecutionStopper struct {
	delay     time.Duration
	mu        sync.Mutex
	stopCalls int
}

func (s *slowExecutionStopper) StopTask(_ context.Context, _, _ string, _ bool) error {
	return nil
}

func (s *slowExecutionStopper) StopSession(_ context.Context, _, _ string, _ bool) error {
	s.mu.Lock()
	s.stopCalls++
	s.mu.Unlock()
	time.Sleep(s.delay)
...
func (s *slowExecutionStopper) StopExecution(_ context.Context, _, _ string, _ bool) error {
	s.mu.Lock()
	s.stopCalls++
	s.mu.Unlock()
	time.Sleep(s.delay)
...
func (s *slowExecutionStopper) calls() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.stopCalls
}

// TestService_DeleteTaskAndWait verifies the synchronous delete path blocks
// until the execution stopper has run, while the async DeleteTask path returns
// before the stopper completes.
func TestService_DeleteTaskAndWait(t *testing.T) {
	svc, _, repo := createTestService(t)
	ctx := context.Background()

	_ = repo.CreateWorkspace(ctx, &models.Workspace{ID: "ws-1", Name: "Workspace"})
	_ = repo.CreateWorkflow(ctx, &models.Workflow{ID: "wf-1", WorkspaceID: "ws-1", Name: "Workflow"})
	_ = repo.CreateTask(ctx, &models.Task{ID: "task-sync", WorkspaceID: "ws-1", WorkflowID: "wf-1", WorkflowStepID: "step-1", Title: "Sync"})
	_ = repo.CreateTaskSession(ctx, &models.TaskSession{ID: "session-sync", TaskID: "task-sync", AgentProfileID: "profile-1", AgentExecutionID: "exec-sync", State: models.TaskSessionStateRunning})

	_ = repo.CreateTask(ctx, &models.Task{ID: "task-async", WorkspaceID: "ws-1", WorkflowID: "wf-1", WorkflowStepID: "step-1", Title: "Async"})
	_ = repo.CreateTaskSession(ctx, &models.TaskSession{ID: "session-async", TaskID: "task-async", AgentProfileID: "profile-1", AgentExecutionID: "exec-async", State: models.TaskSessionStateRunning})

	stopper := &slowExecutionStopper{delay: 100 * time.Millisecond}
	svc.SetExecutionStopper(stopper)

	// Sync: must block until StopExecution has run for the active session.
	syncStart := time.Now()
	if err := svc.DeleteTaskAndWait(ctx, "task-sync"); err != nil {
		t.Fatalf("DeleteTaskAndWait failed: %v", err)
	}
	syncElapsed := time.Since(syncStart)
	if syncElapsed < stopper.delay {
		t.Errorf("DeleteTaskAndWait returned too early: %v < %v", syncElapsed, stopper.delay)
	}
	if got := stopper.calls(); got != 1 {
		t.Errorf("expected 1 stop call after DeleteTaskAndWait, got %d", got)
	}

	// Async: must return before the stopper finishes.
	asyncStart := time.Now()
	if err := svc.DeleteTask(ctx, "task-async"); err != nil {
		t.Fatalf("DeleteTask failed: %v", err)
	}
	asyncElapsed := time.Since(asyncStart)
	if asyncElapsed >= stopper.delay {
		t.Errorf("DeleteTask blocked unexpectedly: %v >= %v", asyncElapsed, stopper.delay)
	}
}
