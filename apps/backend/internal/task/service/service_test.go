...
// TestService_CreateTask_RepositoryIDFallsBackToDefaultBranch verifies that
// when a task is created via repository_id without an explicit base_branch,
// the resulting task_repository row is anchored to the repository's
// default_branch instead of an empty value. This mirrors the github_url and
// local_path fast paths and is what subtasks rely on after the MCP layer
// stops inheriting the parent's branch.
func TestService_CreateTask_RepositoryIDFallsBackToDefaultBranch(t *testing.T) {
	svc, _, repo := createTestService(t)
	ctx := context.Background()
	...
	_ = repo.CreateRepository(ctx, &models.Repository{
		ID: "repo-1", WorkspaceID: "ws-1", Name: "Repo", DefaultBranch: "release",
	})
	...
	task, err := svc.CreateTask(ctx, &CreateTaskRequest{
		...
		Title:          "child",
		Repositories: []TaskRepositoryInput{
			{RepositoryID: "repo-1"}, // no BaseBranch — must fall back
		},
	})
	if err != nil {
	...
	if got := task.Repositories[0].BaseBranch; got != "release" {
		t.Fatalf("expected base_branch to fall back to repository default %q, got %q", "release", got)
	}
}

// TestService_CreateTask_RepositoryIDExplicitBaseBranchWins verifies that an
// explicit base_branch is preserved even when the repository has its own
// default_branch (no silent override).
func TestService_CreateTask_RepositoryIDExplicitBaseBranchWins(t *testing.T) {
	svc, _, repo := createTestService(t)
	ctx := context.Background()
	...
	task, err := svc.CreateTask(ctx, &CreateTaskRequest{
		...
		Title:          "child",
		Repositories: []TaskRepositoryInput{
			{RepositoryID: "repo-1", BaseBranch: "feature/x"},
		},
	})
	if err != nil {
		t.Fatalf("CreateTask failed: %v", err)
	}
	if got := task.Repositories[0].BaseBranch; got != "feature/x" {
		t.Fatalf("expected explicit base_branch to be preserved, got %q", got)
	}
}
