...
// always fails with errOrphanReapUnsupportedPlatform first
// (AC-TASKS-ORPHAN-REAP-007.4), so runOrphanReapPhase never reaches signal
// sending. This exists only so the package builds on Windows.
