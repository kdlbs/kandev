...
// Webhook is a proxied external webhook endpoint the plugin declares. Access
// defaults to authenticated, so a plugin must explicitly declare public access
// for third-party callbacks or an SSO entry point that validates callers in its
// own handler.
type Webhook struct {
