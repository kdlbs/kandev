...
func TestNewAnalysisStateIncludesDatabaseSources(t *testing.T) {
	...
	if len(storageAnalysisSources) != 9 {
		t.Fatalf("source list length = %d, want 9", len(storageAnalysisSources))
	}
	if state.Progress.TotalSources != len(storageAnalysisSources) {
		t.Fatalf("total sources = %d, want 9", state.Progress.TotalSources)
	}
}
