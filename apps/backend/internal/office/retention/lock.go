...
// lock for one sweep (AC-OFFICE-RUN-HISTORY-RETENTION-002.12). Its shape
// resolves three findings the operator accepted as Build's to decide
// (F25, F26, F27 in the task plan):
//
//   - F27 (single connection budget): every statement the sweep issues —
//     count, delete, census — runs through this one dedicated connection,
...
//     deadlock.
//   - F25 (exclusivity window): because every sweep statement runs on the
//     lock connection, there is no window where work proceeds on a
//     different connection after the session died — if the session ends,
//     the very next statement on it fails immediately instead of
//     continuing to run against the pool while another backend has
//     already re-acquired the lock. The between-tables alive() check the
//     design specifies is kept anyway, as a cheap early exit before
//     starting a table's work rather than the only guard against loss of
//     exclusivity.
//   - F26 (release safety): release() unlocks and closes on an independent
...
// whole sweep's duration (F27/F25 above).
...
func (s *sweepSession) release() {
	...
	if err != nil || !unlocked {
		...
		// by a later, unrelated caller (F26).
