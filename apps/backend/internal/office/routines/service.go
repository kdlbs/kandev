...
// expression names strictly after now, in the trigger's timezone, rejecting
// a computed time the expression does not actually name. That check is
// what catches NextCronTime's silent fallback to `after + 24h` for an
// expression with no slot in its search horizon.
func (s *RoutineService) computeSuppressionCursor(trigger *RoutineTrigger, now time.Time) (time.Time, error) {
	...
	matches, err := shared.MatchesCronExpression(trigger.CronExpression, trigger.Timezone, next)
	if err != nil {
		return time.Time{}, err
	}
	if !matches {
		return time.Time{}, fmt.Errorf(
			"computed slot %s does not match cron expression %q", next, trigger.CronExpression)
	}
	return next, nil
}
