...
// seedRoutineRunSession reserves, binds, and starts a run session exactly as
// tasklessTestLauncher.StartRunSession does, for tests that seed a
// routine-scoped run directly via SQL instead of going through QueueRun.
func seedRoutineRunSession(
	t *testing.T, ctx context.Context, svc *service.Service, sessionID, runID, agentProfileID string, at time.Time,
) {
	t.Helper()
	session := &models.RunSession{
		ID: sessionID, WorkspaceID: "ws-1", AgentProfileID: agentProfileID,
		RunID: runID, Attempt: 1, State: models.RunSessionStatePreparing,
		CreatedAt: at, Version: 1,
	}
	reserved, err := svc.RepoForTest().ReserveRunSession(ctx, session)
	if err != nil || !reserved {
		t.Fatalf("reserve run session %s: reserved=%v err=%v", sessionID, reserved, err)
	}
	execID := "execution-" + sessionID
	if _, err := svc.RepoForTest().BindRunSessionExecution(
		ctx, sessionID, execID, agentProfileID, "test-adapter", "test-model", "acp-"+sessionID,
	); err != nil {
		t.Fatalf("bind run session %s: %v", sessionID, err)
	}
	if _, err := svc.RepoForTest().MarkRunSessionStarted(
		ctx, sessionID, execID, agentProfileID, "test-adapter", "test-model", "acp-"+sessionID,
	); err != nil {
		t.Fatalf("start run session %s: %v", sessionID, err)
	}
}
...
// The real recovery mechanism is
// internal/backendapp/office_run_session_launcher.go's
// officeRunSessionLauncher.ReconcileRunSessions, which for an orphaned
// claimed run session calls repo.FinishRunSession(..., Interrupted, ...)
// then repo.RequeueClaimedRun(...). That function lives in internal/backendapp
// — outside this package's scope — and a repo-wide grep confirms no test
// anywhere exercises it directly. This test instead drives its exact two
// repo-level effects here, then proves the guarantee that matters to this
// work order: neither call touches continuation summaries, and the run
// remains claimable and completes normally afterward, attributing a fresh
// summary write to the recovered run without disturbing the untouched one.
func TestContinuationSummary_InterruptedAttemptRecoveredAtStartup_PreservesPriorSummary(t *testing.T) {
	...
	if _, err := svc.QueueRun(ctx, agent.ID, service.RunReasonRoutineTrigger, `{}`, "interrupt-first"); err != nil {
		t.Fatalf("queue first: %v", err)
	}
	runs, err := svc.ListRuns(ctx, agent.WorkspaceID)
	if err != nil || len(runs) != 1 {
	...
	// Claim and start a second attempt exactly as a real launch would, but
	// never complete it — this is the interrupted attempt.
	if _, err := svc.QueueRun(ctx, agent.ID, service.RunReasonRoutineTrigger, `{}`, "interrupt-second"); err != nil {
		t.Fatalf("queue second: %v", err)
	}
	claimed, err := svc.ClaimNextRun(ctx)
	if err != nil || claimed == nil {
		t.Fatalf("claim second: %v (run=%v)", err, claimed)
	}
	seedRoutineRunSession(t, ctx, svc, "run-session-interrupted", claimed.ID, agent.ID, time.Now().UTC())

	if _, err := svc.RepoForTest().FinishRunSession(
		ctx, "run-session-interrupted", models.RunSessionStateInterrupted, "no live execution found at startup",
	); err != nil {
		t.Fatalf("finish interrupted session: %v", err)
	}
	if _, err := svc.RepoForTest().RequeueClaimedRun(ctx, claimed.ID); err != nil {
		t.Fatalf("requeue interrupted run: %v", err)
	}

	// The recovery calls themselves must not have touched the summary.
	afterRecovery, err := svc.GetContinuationSummaryForTest(ctx, agent.ID, "agent:"+agent.ID)
	...
	if len(launcher.calls) != 2 {
		t.Fatalf("relaunch calls after requeue = %d, want 2", len(launcher.calls))
	}
	sessionID2, attempt2 := lastLaunchedSession(t, ctx, svc, launcher)
	if err := eb.Publish(ctx, events.AgentCompleted, bus.NewEvent(events.AgentCompleted, "test", lifecycle.AgentEventPayload{
		AgentExecutionID: "execution-" + sessionID2, AgentID: "test-adapter", AgentProfileID: agent.ID,
		RunID: claimed.ID, RunSessionID: sessionID2, RunAttempt: attempt2, OwnerKind: lifecycle.ExecutionOwnerRun,
		WorkspaceID: agent.WorkspaceID, Status: "COMPLETED",
	})); err != nil {
		t.Fatalf("publish relaunch completion: %v", err)
	}
	final, err := svc.GetContinuationSummaryForTest(ctx, agent.ID, "agent:"+agent.ID)
	if err != nil || final == nil {
		t.Fatalf("read summary after relaunch completion: %v (row=%v)", err, final)
	}
	if final.UpdatedByRunID != claimed.ID {
		t.Fatalf("summary after relaunch completion attributed to %q, want the recovered run %q",
			final.UpdatedByRunID, claimed.ID)
	}
}
