...
func (si *SchedulerIntegration) assembleAgentPrompt(
	...
	nativeConversation, _ := si.svc.repo.IsNativeConversation(ctx, taskID)
	if nativeConversation {
		pc.CommentBody = clipConversationText(pc.CommentBody, 6000)
	}
...
func (si *SchedulerIntegration) launchAgent(
	...
	if taskID == "" {
		si.logger.Error("cannot launch taskless run",
			zap.String(conversationRunIDKey, runID),
			zap.String(participantTypeAgent, agent.Name),
			zap.String(conversationReasonKey, run.Reason),
	...
	if starter, ok := si.svc.taskStarter.(TaskStarterWithEnv); ok {
		err = starter.StartTaskWithEnv(ctx, taskID, launch.ProfileID, launch.ExecutorID, launch.ExecutorProfileID, "",
			launch.Prompt, "", false, nil, launch.Env)
	} else {
		err = si.svc.taskStarter.StartTask(ctx, taskID, launch.ProfileID, launch.ExecutorID, launch.ExecutorProfileID, "",
	...
	if err != nil {
		...
		si.svc.AppendRunEvent(ctx, runID, "error", "error", map[string]interface{}{
			runEventPhaseKey:            "adapter.invoke",
			conversationErrorMessageKey: err.Error(),
		})
		si.releaseCheckoutIfNeeded(ctx, run)
		_ = si.svc.HandleRunFailure(ctx, run, err)
		return false
...
func (si *SchedulerIntegration) failTasklessRun(
	...
	si.svc.AppendRunEvent(ctx, run.ID, "error", "error", map[string]interface{}{
		runEventPhaseKey:            "scheduler.launch",
		conversationErrorMessageKey: msg,
	})
	si.releaseCheckoutIfNeeded(ctx, run)
...
func (si *SchedulerIntegration) tryRoutingDispatch(
	...
	if err != nil {
		si.logger.Error("routing dispatch failed",
			zap.String(conversationRunIDKey, run.ID), zap.Error(err))
		si.svc.AppendRunEvent(ctx, run.ID, "error", "error", map[string]interface{}{
			runEventPhaseKey:            "routing.dispatch",
			conversationErrorMessageKey: err.Error(),
		})
		si.releaseCheckoutIfNeeded(ctx, run)
		_ = si.svc.HandleRunFailure(ctx, run, err)
...
func (si *SchedulerIntegration) checkBudget(
	...
	if !allowed {
		si.logger.Info("run skipped (budget exceeded)",
			zap.String(conversationRunIDKey, run.ID), zap.String(conversationReasonKey, reason))
		si.releaseCheckoutIfNeeded(ctx, run)
		si.svc.clearAgentWorking(ctx, agent.ID, run.ID)
...
func (si *SchedulerIntegration) extractTaskID(payload string) string {
	return ParseRunPayload(payload)[conversationTaskIDKey]
}
...
func (si *SchedulerIntegration) buildPromptContext(
	...
	if reason == RunReasonWorkspaceTaskCallback {
		enrichWorkspaceTaskCallback(pc, payload)
	}
