...
func (r *Repository) EnsureAgentConversation(ctx context.Context, agent *models.AgentInstance) (*models.Channel, error) {
	...
	_, err = tx.ExecContext(ctx, tx.Rebind(`INSERT INTO office_channels
...
func (r *Repository) IsNativeConversation(ctx context.Context, taskID string) (bool, error) {
	var count int
	err := r.ro.GetContext(ctx, &count, r.ro.Rebind(`SELECT COUNT(*) FROM office_channels WHERE task_id = ? AND platform = 'web'`), taskID)
	return count > 0, err
}
