...
// TestIncrementWakeDeliverySeq_Monotonic is R5-F1's regression test for the
// delivery-sequence primitive wakeOperationID hashes: the same parent's
// sequence must never repeat across calls, including the very first call
// for a parent with no prior row, and a second parent's sequence is
// independent.
func TestIncrementWakeDeliverySeq_Monotonic(t *testing.T) {
	repo := newSearchTestRepo(t)
	ctx := context.Background()

	first, err := repo.IncrementWakeDeliverySeq(ctx, "parent-1")
	if err != nil {
	...
	second, err := repo.IncrementWakeDeliverySeq(ctx, "parent-1")
	if err != nil {
	...
	otherFirst, err := repo.IncrementWakeDeliverySeq(ctx, "parent-2")
	if err != nil {
...
// TestIncrementWakeDeliverySeq_DoesNotCreateOrClobberReceipt verifies the
// counter lives entirely outside parent_child_wake_receipts: a bump for a
// parent with no receipt yet must not fabricate one (GetWakeReceipt callers,
// including recordReceipt's still-open-second defer path and
// TestParentWakeReconciler_SkipsWithoutEngineDispatcher, read "no row" as
// "nothing recorded for this parent"), and a bump for a parent that already
// has a delivered receipt must not touch that receipt's fields — those are
// owned by recordReceipt's own UpsertWakeReceiptTx call, which runs later
// and only once the generation second has closed.
func TestIncrementWakeDeliverySeq_DoesNotCreateOrClobberReceipt(t *testing.T) {
	repo := newSearchTestRepo(t)
	ctx := context.Background()

	if _, err := repo.IncrementWakeDeliverySeq(ctx, "parent-no-receipt"); err != nil {
		t.Fatalf("increment: %v", err)
	}
	receipt, err := repo.GetWakeReceipt(ctx, "parent-no-receipt")
	if err != nil {
		t.Fatalf("get wake receipt: %v", err)
	}
	if receipt != nil {
		t.Fatalf("increment fabricated a receipt row: %#v", receipt)
	}
	...
	if err := repo.UpsertWakeReceiptTx(
		ctx, tx, "parent-1", "child-0:COMPLETED", "", "op-1", "2026-01-01 00:00:00", deliveredAt,
	); err != nil {
		t.Fatalf("upsert wake receipt: %v", err)
	}
	if err := tx.Commit(); err != nil {
	...
	if _, err := repo.IncrementWakeDeliverySeq(ctx, "parent-1"); err != nil {
		t.Fatalf("increment: %v", err)
	}

	after, err := repo.GetWakeReceipt(ctx, "parent-1")
	if err != nil {
		t.Fatalf("get wake receipt: %v", err)
	}
	if after == nil {
		t.Fatal("receipt disappeared after increment")
	}
	if after.ChildSetKey != "child-0:COMPLETED" || after.ChildGeneration != "2026-01-01 00:00:00" ||
		after.DeliveryOperationID != "op-1" {
		t.Fatalf("increment clobbered delivery fields: %#v", after)
	}
}
