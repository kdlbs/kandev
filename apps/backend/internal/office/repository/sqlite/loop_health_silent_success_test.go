...
func TestListSilentSuccesses_ReturnsFinishedProcessedSessionlessRun(t *testing.T) {
	...
	rows, total, err := repo.ListSilentSuccesses(context.Background(), "ws-a", windowStart, 50)
	if err != nil {
		t.Fatalf("list: %v", err)
	}
...
func TestListSilentSuccesses_LaunchedRunIsNotSilent(t *testing.T) {
	...
	windowStart := now.Add(-24 * time.Hour)
	_, total, err := repo.ListSilentSuccesses(context.Background(), "ws-a", windowStart, 50)
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if total != 0 {
