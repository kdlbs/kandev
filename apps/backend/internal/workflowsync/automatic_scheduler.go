...
func newAutomaticScheduler(ctx context.Context, workers int, run func(context.Context, automaticJob), idle func()) *automaticScheduler {
	if workers < 1 {
		workers = 1
	}
...
func (s *automaticScheduler) worker(ctx context.Context, run func(context.Context, automaticJob), idle func()) {
	defer s.wg.Done()
	for {
		s.mu.Lock()
