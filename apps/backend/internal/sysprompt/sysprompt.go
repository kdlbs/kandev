...
const coordinatorTaskControlSection = " Optional: session_id, delivery_mode. " +
	"For an autopilot child question, also pass reply_to_question_id with the question ID. " +
	"Use delivery_mode=\"queued\" or omit it for information that can wait. " +
	"Use delivery_mode=\"interrupt\" for urgent replacement work on a running direct child or a granted target; " +
	"if unconfirmed, it remains queued. " +
	...
	"Only the target task's direct parent or a task explicitly granted coordinator authority may call it. Required params: task_id. " +
