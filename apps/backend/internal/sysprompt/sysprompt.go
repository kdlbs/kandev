...
const coordinatorTaskControlSection = " Optional: session_id, delivery_mode. " +
	...
	"Use delivery_mode=\"interrupt\" for urgent replacement work on a running direct child or an authorized target; " +
	"if unconfirmed, it remains queued. " +
	"For halts, use stop_task_kandev.\n" +
	"- stop_task_kandev: Halt all live sessions observed for a direct child, with no prompt and no replacement turn. " +
	"Only the target task's direct parent or an authorized coordinator may call it. Required params: task_id. " +
