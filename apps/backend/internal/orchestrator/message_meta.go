...
// WithSenderSessionName records the sender session's user-supplied name (a
// snapshot at send time) so the receiving chat's sender badge can show which
// session sent the message, not just which task.
func (m *UserMessageMeta) WithSenderSessionName(name string) *UserMessageMeta {
	m.SenderSessionName = name
	return m
}
...
func (m *UserMessageMeta) ToMap() map[string]interface{} {
	...
	if m.SenderTaskID != "" {
		...
		if m.SenderSessionName != "" {
			meta["sender_session_name"] = m.SenderSessionName
		}
	}
