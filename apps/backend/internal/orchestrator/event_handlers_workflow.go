...
func (s *Service) handleTaskMovedNoSession(ctx context.Context, data watcher.TaskMovedEventData) {
	...
	_, err = s.StartTask(ctx, task.ID, agentProfileID, "", executorProfileID, 0, task.Description, data.ToStepID, planMode, nil, false)
	if err != nil {
...
func (s *Service) autoStartStepPrompt(
	...
	if session.State == models.TaskSessionStateCreated {
		s.logger.Info("auto-start: session is CREATED, launching agent via StartCreatedSession",
			...
			zap.String("session_id", sessionID),
			zap.String("step_name", stepName))
		_, err := s.StartCreatedSession(ctx, taskID, sessionID, session.AgentProfileID, prompt, true, planMode, nil, false)
		return err
	}
