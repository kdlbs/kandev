...
func TestResolveAuthSecrets(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantEnv: map[string]string{
				"GITHUB_TOKEN": "existing-token", // Not overwritten
			},
		},
		{
...
func TestResolveGHCLIToken(t *testing.T) {
	...
	tests := []struct {
		...
		metadata    map[string]interface{}
		existingEnv map[string]string
		// We can't easily test the actual gh CLI extraction, but we can test the logic
		expectSkip bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			initialToken := req.Env["GITHUB_TOKEN"]

			executor.resolveGHCLIToken(req, tt.metadata)

			if tt.expectSkip {
				// Token should not have been modified
				if req.Env["GITHUB_TOKEN"] != initialToken {
					t.Errorf("expected GITHUB_TOKEN to remain %q, got %q", initialToken, req.Env["GITHUB_TOKEN"])
				}
			}
		})
	}
...
func TestInjectGitHubToken(t *testing.T) {
	...
	// Set up mock secret store with GITHUB_TOKEN
	executor.secretStore = &mockSecretStore{
		secrets: map[string]string{
			"secret-1": "ghp_globaltoken",
		},
	...
	t.Run("injects token when not set", func(t *testing.T) {
		...
		if req.Env["GITHUB_TOKEN"] != "ghp_globaltoken" {
			t.Errorf("expected GITHUB_TOKEN to be set, got %q", req.Env["GITHUB_TOKEN"])
		}
		if req.Env["GH_TOKEN"] != "ghp_globaltoken" {
			t.Errorf("expected GH_TOKEN to be set, got %q", req.Env["GH_TOKEN"])
		}
	})
	...
	t.Run("skips if GITHUB_TOKEN already set", func(t *testing.T) {
		...
		if req.Env["GITHUB_TOKEN"] != "existing-token" {
			t.Errorf("expected GITHUB_TOKEN to remain unchanged, got %q", req.Env["GITHUB_TOKEN"])
		}
	})
	...
	t.Run("skips if GH_TOKEN already set", func(t *testing.T) {
		...
		// GITHUB_TOKEN should not be injected since GH_TOKEN is set
		if req.Env["GITHUB_TOKEN"] != "" {
			t.Errorf("expected GITHUB_TOKEN to not be injected when GH_TOKEN is set, got %q", req.Env["GITHUB_TOKEN"])
		}
	})
}
...
func TestInjectGitHubTokenFromCLI(t *testing.T) {
	...
	t.Run("skips if GH_TOKEN already set", func(t *testing.T) {
		...
		// Should not override existing GH_TOKEN
		if req.Env["GH_TOKEN"] != "existing-gh-token" {
			t.Errorf("expected GH_TOKEN to remain unchanged, got %q", req.Env["GH_TOKEN"])
		}
	})

	// Note: We can't easily test the actual gh CLI extraction without mocking exec.Command.
	// The function will gracefully handle the case where gh is not installed or not authenticated.
}
