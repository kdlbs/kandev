...
func (s *Service) executeQueuedMessageWithReservation(
	...
	lifecyclePrompt := isLifecycleAutomationOrigin(queuedMsg.Metadata["origin"]) || queuedMsg.IsWorkflowControl()
	...
	if lifecyclePrompt && !s.lifecycleQueuedDispatchIsCurrent(promptCtx, queuedMsg) {
		...
		} else if err := s.messageQueue.AcknowledgeQueued(promptCtx, callerSessionID, queuedMsg.ID); err != nil {
