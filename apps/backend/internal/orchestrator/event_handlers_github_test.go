...
func TestListTasksNeedingPRWatch(t *testing.T) {
	...
	t.Run("dedupes multiple sessions on the same task/repo/branch", func(t *testing.T) {
		...
		if err := testRepo.CreateTaskEnvironmentRepo(ctx, &models.TaskEnvironmentRepo{
			ID: "wt-1", TaskEnvironmentID: "env-t1", WorktreeID: "wtree-1", RepositoryID: "repo1", WorktreeBranch: "feature-a", CreatedAt: now,
		}); err != nil {
...
func TestResolveBranchForRepository(t *testing.T) {
	...
	t.Run("falls back to an active session worktree branch when checkout_branch is unset", func(t *testing.T) {
		...
		if err := testRepo.CreateTaskEnvironmentRepo(ctx, &models.TaskEnvironmentRepo{
			ID: "wt-1", TaskEnvironmentID: "env-s1", WorktreeID: "wtree-1", RepositoryID: "repo1", WorktreeBranch: "session-branch", CreatedAt: now,
		}); err != nil {
			t.Fatalf("create worktree: %v", err)
		}
