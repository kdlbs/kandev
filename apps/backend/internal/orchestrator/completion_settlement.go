...
func (s *Service) reconcileCompletionIntent(ctx context.Context, store completionIntentReconciliationStore, intent *models.CompletionIntent) {
	...
	lock.Lock()
	defer lock.Unlock()
	s.reconcileCompletionIntentLocked(ctx, store, intent, nil)
}
...
func (s *Service) reconcileCompletionIntentWithExecutionCheck(
	...
	if replaced {
		s.finishCompletionIntent(ctx, store, intent, models.CompletionIntentStateSuperseded, now, "successor_execution", auditEvent)
		return true
	}
	return false
}
...
func (s *Service) reconcileCompletionIntentLocked(
	...
	if turn != nil && turn.ID != intent.TurnID {
		s.finishCompletionIntent(ctx, store, intent, models.CompletionIntentStateSuperseded, now, "successor_turn", auditEvent)
		return
	}
	if s.reconcileCompletionIntentWithExecutionCheck(ctx, store, intent, now, auditEvent) {
		return
	}
	if turn != nil {
	...
	if task == nil || task.WorkflowStepID != intent.WorkflowStepID {
		s.settleMovedCompletionIntent(ctx, store, intent, now, auditEvent)
		return
	}
	...
	s.finishCompletionIntent(ctx, store, intent, models.CompletionIntentStateSettled, now, "quiet_grace", auditEvent)
}
...
func (s *Service) settleMovedCompletionIntent(
	...
	s.finishCompletionIntent(ctx, store, intent, models.CompletionIntentStateSuperseded, now, "task_moved", auditEvent)
}
...
func (s *Service) settleCompletionIntentForProviderTurn(ctx context.Context, sessionID, turnID string) {
	...
	s.finishCompletionIntent(ctx, store, intent, models.CompletionIntentStateSettled, now, "provider_terminal", nil)
}
