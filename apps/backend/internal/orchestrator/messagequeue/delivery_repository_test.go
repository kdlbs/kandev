...
// TestPurgeTaskDeletesOrphanedQueueEntryForCancelledOutboundDelivery covers
// purging the SENDER of an already-admitted cross-task delivery. The queue
// row it landed in is stamped with the TARGET's task_id (it is that task's
// queued work now), so the task_id-scoped delete in PurgeTaskInTransaction
// never touches it even though the delivery itself gets cancelled here.
// Left behind, the target session would still reserve and deliver that
// prompt on a later drain despite its receipt being cancelled.
func TestPurgeTaskDeletesOrphanedQueueEntryForCancelledOutboundDelivery(t *testing.T) {
	repo := newTestSQLiteRepo(t)
	ledger := repo.(DeliveryLedger)
	ctx := context.Background()
	now := time.Now().UTC()
	...
	delivery, _, err := ledger.CreateOrGetDelivery(ctx, Delivery{
		SenderTaskID: "sender-task", SenderSessionID: "sender-session", SourceTurnID: "sender-turn",
	...
	queued := &QueuedMessage{
		...
		Metadata:  map[string]interface{}{MetadataDeliveryID: delivery.ID},
	}
	require.NoError(t, repo.Insert(ctx, queued, 0))
	_, err = ledger.MarkDeliveryQueued(ctx, delivery.ID, "worker-a", queued.ID)
	require.NoError(t, err)

	// The sender task is purged while its outbound delivery is still sitting,
	// queued, in the target's session.
	_, err = repo.PurgeTask(ctx, "sender-task")
	require.NoError(t, err)
	...
	remaining, err := repo.ListBySession(ctx, "target-session")
	require.NoError(t, err)
	assert.Empty(t, remaining, "cancelled outbound delivery's orphaned queue entry must be removed")
}

// TestMarkDeliveryAmbiguousByQueueEntryHandlesMissingDeliveryRowWithoutPanic
// covers deliveryIDForQueueEntry's metadata_json fallback: it can return an
// ID for a message_deliveries row that does not exist (or was already
// purged), and MarkDeliveryAmbiguousByQueueEntry must report ErrEntryNotFound
// instead of dereferencing the nil *Delivery that GetDelivery returns for an
// unknown ID.
func TestMarkDeliveryAmbiguousByQueueEntryHandlesMissingDeliveryRowWithoutPanic(t *testing.T) {
	repo := newTestSQLiteRepo(t)
	ledger := repo.(DeliveryLedger)
	ctx := context.Background()
	...
	queued := &QueuedMessage{
		SessionID: "target-session",
		TaskID:    "target-task",
		Content:   "orphaned lifecycle row",
		QueuedBy:  QueuedByAgent,
		Metadata: map[string]interface{}{
			MetadataDeliveryID:       "delivery-that-does-not-exist",
			MetadataLifecycleDurable: true,
		},
	}
	require.NoError(t, repo.Insert(ctx, queued, 0))

	_, err := ledger.MarkDeliveryAmbiguousByQueueEntry(ctx, queued.ID, "acceptance uncertain")
