...
func (r *sqliteRepository) ensureQueueCapacityTx(
	...
	count, err := r.ordinaryQueueEntryCountTx(ctx, tx, sessionID)
	if err != nil {
		return err
	}
...
// ordinaryQueueEntryCountTx excludes workflow-control prompts. Those prompts
// are durable state-transition instructions with their own coalescing key and
// must remain admissible without consuming an operator's ordinary backlog
// capacity.
func (r *sqliteRepository) ordinaryQueueEntryCountTx(ctx context.Context, tx *sqlx.Tx, sessionID string) (int, error) {
