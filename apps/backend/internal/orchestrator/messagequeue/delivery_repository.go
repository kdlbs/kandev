...
// getContexter is satisfied by both *sqlx.DB and *sqlx.Tx, letting
// deliveryIDFromQueueEntry serve both the transactional and plain callers
// below without duplicating the metadata-fallback lookup.
type getContexter interface {
	GetContext(ctx context.Context, dest interface{}, query string, args ...interface{}) error
}

// deliveryIDFromQueueEntry resolves a queue entry to its owning delivery ID,
// falling back to the queued_messages.metadata_json delivery_id marker when
// no message_deliveries row references the entry directly (a durable
// lifecycle row can be queued before its receipt is linked back to it).
func deliveryIDFromQueueEntry(ctx context.Context, q getContexter, db *sqlx.DB, queueEntryID string) (string, error) {
	var deliveryID string
	err := q.GetContext(ctx, &deliveryID, db.Rebind(`SELECT id FROM message_deliveries WHERE queue_entry_id = ?`), queueEntryID)
	if err == nil {
	...
	err = q.GetContext(ctx, &metadataJSON, db.Rebind(`SELECT metadata_json FROM queued_messages WHERE id = ?`), queueEntryID)
	if errors.Is(err, sql.ErrNoRows) {
		return "", nil
	}
	if err != nil {
...
func (r *sqliteRepository) deliveryIDForQueueEntryTx(ctx context.Context, tx *sqlx.Tx, queueEntryID string) (string, error) {
	return deliveryIDFromQueueEntry(ctx, tx, r.db, queueEntryID)
}
...
func (r *sqliteRepository) MarkDeliveryAmbiguousByQueueEntry(ctx context.Context, queueEntryID, lastError string) (*Delivery, error) {
	...
	if affected == 0 && (delivery == nil || (delivery.State != DeliveryAmbiguous && delivery.State != DeliveryDelivered)) {
		return nil, ErrEntryNotFound
	}
	if affected == 1 {
		adminmetrics.RecordMessageDeliveryOutcome(string(DeliveryAmbiguous), 1)
...
func (r *sqliteRepository) deliveryIDForQueueEntry(ctx context.Context, queueEntryID string) (string, error) {
	return deliveryIDFromQueueEntry(ctx, r.db, r.db, queueEntryID)
}
