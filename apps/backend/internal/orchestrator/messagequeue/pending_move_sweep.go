...
func (r *sqliteRepository) ListPendingMoves(ctx context.Context) ([]PendingMoveRecord, error) {
	rows, err := r.ro.QueryxContext(ctx, `
		SELECT session_id, id, move_id, task_id, workflow_id, workflow_step_id, step_position, queued_at, actor, sender_session_id, expected_workflow_step_id, initiating_turn_id
		FROM pending_moves
	`)
	if err != nil {
	...
	for rows.Next() {
		var (
			sessionID, rowID, moveID, taskID, workflowID, workflowStepID string
			position                                                     int
			queuedAt                                                     time.Time
			actor, senderSessionID, expectedStepID, initiatingTurnID     string
		)
		if err := rows.Scan(&sessionID, &rowID, &moveID, &taskID, &workflowID, &workflowStepID,
			&position, &queuedAt, &actor, &senderSessionID, &expectedStepID, &initiatingTurnID); err != nil {
...
func (r *sqliteRepository) DeletePendingMoveIfMatch(
	...
	`), expected.SessionID, expected.Move.ID)
	if err != nil {
...
func (r *memoryRepository) DeletePendingMoveIfMatch(
	...
	if !ok || stored.ID != expected.Move.ID {
		return false, nil
	}
