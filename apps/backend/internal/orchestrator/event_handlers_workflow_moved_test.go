...
func TestProcessStepExitAndEnter(t *testing.T) {
	...
	t.Run("runs on_exit then on_enter", func(t *testing.T) {
		...
		svc := createTestService(repo, stepGetter, newMockTaskRepo())
		session, _ = repo.GetTaskSession(ctx, "s1")
		svc.processStepExitAndEnter(ctx, "t1", session, "step1", "step2", "test task", 0)
	...
	t.Run("handles missing from-step", func(t *testing.T) {
		...
		session, _ := repo.GetTaskSession(ctx, "s1")
		// Should not panic
		svc.processStepExitAndEnter(ctx, "t1", session, "nonexistent", "step2", "test task", 0)
	})
	...
	t.Run("handles missing to-step", func(t *testing.T) {
		...
		session, _ := repo.GetTaskSession(ctx, "s1")
		// Should not panic
		svc.processStepExitAndEnter(ctx, "t1", session, "step1", "nonexistent", "test task", 0)
	})
}
