...
func (s *Service) SettleStaleSession(ctx context.Context, request StaleSessionSettlementRequest) (StaleSessionSettlementResult, error) {
	...
	// Result is intentionally left blank here: finishCompletionIntent fills it
	// in to match whichever terminal state the reconciler actually reaches
	// (settled or superseded) and commits it atomically with that exact
	// transition. A crash between "the turn settled" and "the audit was
	// recorded" can therefore never happen for this caller — either both
	// durable facts land, or neither does.
	auditEvent := &models.SessionControlEvent{
		...
		EvidenceCode: "eligible_completion_intent",
	}
	...
	if err := s.reconcileCompletionIntentLocked(ctx, store, intent, auditEvent); err != nil {
		return StaleSessionSettlementResult{}, fmt.Errorf("commit stale session settlement: %w", err)
	}
...
func (s *Service) staleSettlementCandidate(ctx context.Context, store completionIntentReconciliationStore, request StaleSessionSettlementRequest) (*models.CompletionIntent, string) {
	...
	// Check the completion intent before session state so a retry after a
	// successful settlement returns already_settled instead of session_not_running.
	intent, intentErr := store.GetCompletionIntentForTurn(ctx, request.TargetSessionID, request.TargetTurnID)
	if intent != nil && (intent.State == models.CompletionIntentStateSettled || intent.State == models.CompletionIntentStateSuperseded) {
	...
	if intentErr != nil || !completionIntentIsEligible(intent, request.TargetTaskID) {
...
func (s *Service) rejectStaleSettlement(ctx context.Context, audit sessionControlEventStore, request StaleSessionSettlementRequest, evidence string) (StaleSessionSettlementResult, error) {
	...
	if evidence == "already_settled" {
		if err := audit.CreateSessionControlEvent(ctx, &models.SessionControlEvent{
			ActorTaskID: request.ActorTaskID, ActorSessionID: request.ActorSessionID,
			TargetTaskID: request.TargetTaskID, TargetSessionID: request.TargetSessionID,
			TargetTurnID: request.TargetTurnID, AuthorityBasis: request.AuthorityBasis,
			EvidenceCode: evidence, Result: "already_settled",
		}); err != nil {
			return StaleSessionSettlementResult{}, fmt.Errorf("record stale session already settled: %w", err)
		}
		return StaleSessionSettlementResult{Status: "already_settled"}, nil
	}
	if err := audit.CreateSessionControlEvent(ctx, &models.SessionControlEvent{
		ActorTaskID: request.ActorTaskID, ActorSessionID: request.ActorSessionID,
		TargetTaskID: request.TargetTaskID, TargetSessionID: request.TargetSessionID,
