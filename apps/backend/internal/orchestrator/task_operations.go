...
func (s *Service) startTask(ctx context.Context, taskID string, agentProfileID string, executorID string, executorProfileID string, priority string, prompt string, workflowStepID string, planMode, autoStart bool, attachments []v1.MessageAttachment, opts startTaskOptions) (*executor.TaskExecution, error) {
	...
	// Not gated on sessionCreated: a retried launch call can reach here with
	// the session row already created by a first attempt whose
	// persistSpawnSupervision failed afterward. persistSpawnSupervision is
	// idempotent against its own prior write, so retrying it here cannot
	// duplicate or corrupt the record, but skipping it on sessionCreated ==
	// false would permanently launch a spawned session without durable
