...
// handleCompletionSpecialCases reroutes terminal completions whose failure is
// not an agent failure: a backend-shutdown stop stays resumable, an
// uninitialized startup exit defers to its startup owner, and any remaining
// failure fills its attempt evidence. A handled return short-circuits
// MarkCompleted's ordinary failed/complete path.
