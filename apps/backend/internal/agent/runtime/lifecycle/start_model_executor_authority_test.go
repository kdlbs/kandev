...
func TestApplyStartModelPolicyExecutorAuthority(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "requested model absent fails closed",
			state:   modelState("executor-default"),
			policy:  StartModelPolicy{Model: "host-only-model"},
			wantErr: "BLOCKED_MODEL_IDENTITY",
		},
		{
			name:          "empty catalog attempts requested model",
			state:         &CachedModelState{},
			policy:        StartModelPolicy{Model: "host-only-model", FallbackModel: "fallback"},
			wantCalls:     []string{"host-only-model"},
			wantOutcome:   ModelSelectionOutcomeApplied,
			wantEffective: "host-only-model",
		},
		{
			name:    "advertised fallback does not replace unavailable request",
			state:   modelState("fallback"),
			policy:  StartModelPolicy{Model: "host-only-model", FallbackModel: "fallback"},
			wantErr: "BLOCKED_MODEL_IDENTITY",
		},
		{
			name:    "fallback method support cannot bypass unavailable request",
			state:   modelState("fallback"),
			policy:  StartModelPolicy{Model: "host-only-model", FallbackModel: "fallback"},
			wantErr: "BLOCKED_MODEL_IDENTITY",
		},
		{
			name:    "unadvertised fallback does not replace unavailable request",
			state:   modelState("executor-default"),
		...
		{
			name:          "auto fallback does not hide advertised apply error",
			state:         modelState("requested"),
