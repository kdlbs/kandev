...
func TestWorkspaceRestoreTerminalSessions(t *testing.T) {
	entryPoints := []struct {
		name string
			...
			t.Run(entryPoint.name+"/"+string(state), func(t *testing.T) {
				provider := &mockWorkspaceInfoProvider{infos: map[string]*WorkspaceInfo{
					terminalSessionID: {
						...
						TaskEnvironmentID: terminalEnvironmentID,
						WorkspacePath:     "/workspace/task", AgentID: "auggie",
					},
				}}
				...
				if err := entryPoint.call(mgr); err != nil {
					t.Fatalf("workspace restore error = %v", err)
				}
				...
				if execution, exists := mgr.executionStore.GetBySessionID(terminalSessionID); !exists {
					...
					t.Fatalf("workspace restore configured an agent command: %q", execution.AgentCommand)
				}
			})
		}
	}
