...
func (p *EventPublisher) PublishToolCall(execution *AgentExecution, toolCallID, title, status string, args map[string]interface{}) {
	...
	event := bus.NewEvent(events.ToolCallStarted, "agent-manager", payload)
	subject := events.BuildToolCallStartedSubject(sessionID)
...
func (p *EventPublisher) PublishToolCallComplete(execution *AgentExecution, event agentctl.AgentEvent) {
	...
	payload := ToolCallCompleteEventPayload{
	...
	busEvent := bus.NewEvent(events.ToolCallComplete, "agent-manager", payload)
	subject := events.BuildToolCallCompleteSubject(sessionID)
