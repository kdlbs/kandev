...
// ToolContext is the Go-native mirror of kandev.plugin.v1.ToolContext.
...
func (c *ToolContext) toProto() *pluginv1.ToolContext {
	if c == nil {
		return nil
	}
	return &pluginv1.ToolContext{
		TaskId:          c.TaskID,
...
func toolContextFromProto(p *pluginv1.ToolContext) *ToolContext {
	if p == nil {
		return nil
	}
	return &ToolContext{
		TaskID:          p.GetTaskId(),
		AgentInstanceID: p.GetAgentInstanceId(),
		SessionID:       p.GetSessionId(),
	}
}

// ToolRequest is the Go-native mirror of kandev.plugin.v1.ToolRequest,
// delivered to a Plugin's InvokeTool method.
...
func (r *ToolRequest) toProto() (*pluginv1.ToolRequest, error) {
	input, err := mapToStruct(r.Input)
	if err != nil {
		return nil, fmt.Errorf("pluginsdk: tool request input: %w", err)
	}
	return &pluginv1.ToolRequest{
...
func toolRequestFromProto(p *pluginv1.ToolRequest) (*ToolRequest, error) {
	input, err := structToMap(p.GetInput())
	if err != nil {
		return nil, fmt.Errorf("pluginsdk: tool request input: %w", err)
	}
	return &ToolRequest{
		...
		Input:      input,
		Context:    toolContextFromProto(p.GetContext()),
	}, nil
}

// ToolResponse is the Go-native mirror of kandev.plugin.v1.ToolResponse,
// returned by a Plugin's InvokeTool method.
type ToolResponse struct {
	Output map[string]any
	Error  string
}

func (r *ToolResponse) toProto() (*pluginv1.ToolResponse, error) {
	output, err := mapToStruct(r.Output)
	if err != nil {
		return nil, fmt.Errorf("pluginsdk: tool response output: %w", err)
	}
...
func toolResponseFromProto(p *pluginv1.ToolResponse) (*ToolResponse, error) {
	output, err := structToMap(p.GetOutput())
	if err != nil {
		return nil, fmt.Errorf("pluginsdk: tool response output: %w", err)
	}
	return &ToolResponse{
...
// delivered to a Plugin's HandleWebhook method. Unlike Event/ToolRequest,
// it has no Struct-typed fields so conversion cannot fail.
