...
test("editor dropdown opens the folder without configured editors and recovers from errors", async ({
  testPage,
  apiClient,
  seedData,
}) => {
  ...
  await testPage.route("**/api/v1/editors", (route) =>
    route.fulfill({ json: { editors: [], folder_opening_available: true } }),
  );
  ...
  await trigger.click();
  const folder = testPage.getByRole("menuitem", { name: "Open folder", exact: true });
  await expect(folder).toBeVisible();
  await testPage.screenshot({
    path: test.info().outputPath("desktop-folder-menu.png"),
    animations: "disabled",
  });
  await folder.click();
...
test("desktop folder picker opens only the selected repository", async ({
  ...
  await testPage.getByRole("menuitem", { name: "Open folder", exact: true }).focus();
  await testPage.keyboard.press("Enter");
  const picker = testPage.getByRole("dialog", { name: "Choose a folder" });
  await expect(picker).toBeVisible();
  await testPage.screenshot({
    path: test.info().outputPath("desktop-folder-picker.png"),
    animations: "disabled",
  });
  ...
  await trigger.click();
  await testPage.getByRole("menuitem", { name: "Open folder", exact: true }).click();
...
test("missing host folder opener disables the action before any picker opens", async ({
  ...
  await session.waitForLoad();
  await session.waitForChatIdle();
  await testPage.getByTestId("editors-menu-list").click();
  await expect(testPage.getByRole("menuitem", { name: "Open folder", exact: true })).toBeDisabled();
