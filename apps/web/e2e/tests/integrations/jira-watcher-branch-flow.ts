...
export async function assertJiraWatcherQualifiedBranchPersists(args: {
  ...
  await expect
    ...
    .toBeGreaterThan(0);
  await activate(page.getByRole("listbox").getByRole("option").first(), inputMode);
  ...
  await expect(branchDropdown.getByRole("option", { name: /^origin\/main origin/ })).toBeVisible();
  ...
  if (inputMode === "touch") {
    await activate(row.getByRole("button").first(), inputMode);
  } else {
