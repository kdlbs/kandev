...
test.describe("ssh executor — task launch", () => {
  test("reaches RUNNING with a durable task root nested below another checkout", async ({
    apiClient,
    seedData,
  }) => {
    ...
    execInContainer(seedData.sshTarget, [
      "sh",
      "-c",
      "mkdir -p /opt/jumprope-fullstack/.kandev && git init -q /opt/jumprope-fullstack && chown -R kandev:kandev /opt/jumprope-fullstack",
    ]);
