...
test.describe("Executor reuse", () => {
  ...
  test("fresh environment toggle not visible for standalone executor", async ({
    testPage,
    apiClient,
    seedData,
  }) => {
    ...
    // Create task with default standalone mock agent
    const task = await apiClient.createTaskWithAgent(
      seedData.workspaceId,
      "Toggle Hidden Task",
      seedData.agentProfileId,
    ...
    const card = kanban.taskCardByTitle("Toggle Hidden Task");
    await expect(card).toBeVisible({ timeout: 10_000 });
    await card.click();
    await expect(testPage).toHaveURL(/\/t\//, { timeout: 15_000 });
    ...
    // Fresh environment toggle should NOT be visible for standalone executor
    const freshToggle = session
      .newSessionDialog()
      .getByLabel("Fresh environment", { exact: false });
    await expect(freshToggle).not.toBeVisible();
  });
});
