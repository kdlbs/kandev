...
test.describe("mobile quick terminal tabs", () => {
  test("uses a safe full-height surface, touch-safe menu, and contained terminal scroll", async ({
    ...
    const xtermViewport = dialog.getByTestId("quick-terminal-terminal").locator(".xterm-viewport");
    await expect
      .poll(() => xtermViewport.evaluate((element) => element.scrollHeight >= element.clientHeight))
      .toBe(true);
