...
export default function UtilityAgentsSettingsPage() {
  ...
  const handleBuiltinModelChange = async (agent: UtilityAgent, value: string) => {
    if (value === USE_DEFAULT) {
      await updateUtilityAgent(agent.id, { agent_id: "", model: "", enabled: true });
      setAgents((prev) => prev.map((a) => (a.id === agent.id ? { ...a, agent_id: "", model: "" } : a)));
    } else {
      const [agentId, model] = value.split("|");
      await updateUtilityAgent(agent.id, { agent_id: agentId, model, enabled: true });
      setAgents((prev) => prev.map((a) => (a.id === agent.id ? { ...a, agent_id: agentId, model } : a)));
    }
  };

  const handleDeleteCustom = async (agent: UtilityAgent) => {
    await deleteUtilityAgent(agent.id);
    setAgents((prev) => prev.filter((a) => a.id !== agent.id));
  };

  const openEditDialog = (agent: UtilityAgent | null) => { setEditingAgent(agent); setDialogOpen(true); };
  const closeDialog = () => { setDialogOpen(false); setEditingAgent(null); fetchData(); };

  const allModels = useMemo(
    () =>
      inferenceAgents.flatMap((ia) =>
        ia.models.map((m) => ({
          value: `${ia.id}|${m.id}`,
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-muted-foreground">One-shot AI helpers for commits, PRs, and prompts.</p>
      </header>
      <Separator />
      <DefaultModelSection
      ...
      <PerActionOverridesSection
        ...
        allModels={allModels}
        defaultModel={defaultModel}
        onModelChange={handleBuiltinModelChange}
      ...
      />
      <UtilityAgentDialog open={dialogOpen} onOpenChange={setDialogOpen} agent={editingAgent} onSuccess={closeDialog} />
    </div>
  );
}
