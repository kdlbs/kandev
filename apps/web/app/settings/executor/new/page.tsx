...
function ExecutorFormCard({
  type, name, dockerHost, dockerTlsVerify, dockerCertPath, gitToken,
  onTypeChange, onNameChange, onDockerHostChange,
  onDockerTlsVerifyChange, onDockerCertPathChange, onGitTokenChange,
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Select value={type} onValueChange={(value) => onTypeChange(value as ExecutorType)}>
            <SelectTrigger id="executor-type"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          ...
          <Input id="executor-name" value={name} onChange={(event) => onNameChange(event.target.value)} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="docker-host">Docker host</Label>
          <Input id="docker-host" value={dockerHost} onChange={(event) => onDockerHostChange(event.target.value)} placeholder={isRemoteDocker ? 'tcp://remote:2376 or ssh://user@host' : 'unix:///var/run/docker.sock'} />
          <p className="text-xs text-muted-foreground">
            {isRemoteDocker ? 'The remote Docker host URL (tcp://, ssh://).' : 'Repositories will be mounted as volumes at runtime.'}
          </p>
        </div>
        {isRemoteDocker && (
          <RemoteDockerFields
            dockerTlsVerify={dockerTlsVerify} onDockerTlsVerifyChange={onDockerTlsVerifyChange}
            dockerCertPath={dockerCertPath} onDockerCertPathChange={onDockerCertPathChange}
            gitToken={gitToken} onGitTokenChange={onGitTokenChange}
          />
        )}
      </CardContent>
    </Card>
...
function buildExecutorConfig(type: ExecutorType, dockerHost: string, dockerTlsVerify: string, dockerCertPath: string, gitToken: string): Record<string, string> {
...
function ExecutorCreatePageContent() {
  ...
  const [name, setName] = useState(() => initialType === 'remote_docker' ? 'Remote Docker' : 'Local Docker');
  const [dockerHost, setDockerHost] = useState(() => initialType === 'remote_docker' ? 'tcp://' : 'unix:///var/run/docker.sock');
  ...
  const handleTypeChange = (value: ExecutorType) => {
    setType(value);
    if (value === 'local_docker') { setName('Local Docker'); setDockerHost('unix:///var/run/docker.sock'); }
    else if (value === 'remote_docker') { setName('Remote Docker'); setDockerHost('tcp://'); }
  };

  const handleCreate = async () => {
    setIsCreating(true);
    try {
      const config = buildExecutorConfig(type, dockerHost, dockerTlsVerify, dockerCertPath, gitToken);
      const payload = { name, type, status: 'active', config };
      const client = getWebSocketClient();
      const created = client ? await client.request<Executor>('executor.create', payload) : await createExecutorAction(payload);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Choose an executor type to run environments on your infrastructure.</p>
      </div>
      <Separator />
      <ExecutorFormCard
        type={type} name={name} dockerHost={dockerHost}
        dockerTlsVerify={dockerTlsVerify} dockerCertPath={dockerCertPath} gitToken={gitToken}
        onTypeChange={handleTypeChange} onNameChange={setName} onDockerHostChange={setDockerHost}
        onDockerTlsVerifyChange={setDockerTlsVerify} onDockerCertPathChange={setDockerCertPath} onGitTokenChange={setGitToken}
      />
      <div className="flex items-center justify-end gap-2">
        <Button variant="outline" onClick={() => router.push('/settings/executors')}>Cancel</Button>
        <Button onClick={handleCreate} disabled={isCreating}>{isCreating ? 'Creating...' : 'Create Executor'}</Button>
      </div>
    </div>
  );
}
