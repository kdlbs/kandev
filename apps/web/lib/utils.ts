...
/**
 * Extract organization/repository name from a repository URL or path.
...
export function extractRepoName(repositoryUrl: string | null | undefined): string | null {
  ...
  try {
    // Try to parse as GitHub URL (https://github.com/org/repo or git@github.com:org/repo)
    const githubHttpMatch = repositoryUrl.match(/github\.com[:/]([^/]+\/[^/]+?)(\.git)?$/);
    if (githubHttpMatch) {
      return githubHttpMatch[1];
    }

    // Try to extract from local path (e.g., /path/to/org/repo or ~/org/repo)
    const pathMatch = repositoryUrl.match(/([^/]+\/[^/]+)\/?$/);
    if (pathMatch) {
      return pathMatch[1];
    }
