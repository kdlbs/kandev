...
} from "@/lib/api/domains/office-tasks-api";
...
import type { OfficeTask, TaskFilterState } from "@/lib/state/slices/office/types";
...
export const officeQueryOptions = {
  ...
  /**
   * Paginated office tasks via keyset cursors.
   *
   * Accepts a pre-built `ListTasksParams` (sans cursor fields) so callers
   * with richer filter shapes than `TaskFilterState` (sort, include_system,
   * canonical-status conversion) can pass them through unchanged. The
   * params object is baked into the cache key so each distinct filter/sort
   * combination gets its own infinite cache entry.
   *
   * Cursor fields (`cursor`, `cursor_id`) are excluded from the key and
   * fed via `pageParam` so successive pages append to the same cache.
   */
  tasksPaginated: (wsId: string, params?: Omit<ListTasksParams, "cursor" | "cursor_id">) =>
    infiniteQueryOptions({
      queryKey: qk.office.tasksPaginated(wsId, params as Record<string, unknown> | undefined),
      queryFn: ({ pageParam }) =>
        listTasks(wsId, {
          ...(params ?? {}),
      ...
      initialPageParam: undefined as { cursor?: string; id?: string } | undefined,
      getNextPageParam: (lastPage) =>
        lastPage.next_cursor
          ? { cursor: lastPage.next_cursor, id: lastPage.next_id ?? undefined }
          : undefined,
      enabled: !!wsId,
      staleTime: 30_000,
    }),
...
/**
 * Flattens an `useInfiniteQuery` result for `tasksPaginated` to a flat
 * OfficeTask[] suitable for list rendering. Returns [] when the cache is
 * empty so consumers don't need to guard on `undefined`.
 */
export function flattenTasksPaginated(
  data: InfiniteData<ListTasksResponse> | undefined,
): OfficeTask[] {
  if (!data) return [];
