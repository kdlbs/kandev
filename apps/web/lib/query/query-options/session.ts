...
export function taskWalkthroughQueryOptions(taskId: string | null | undefined, enabled = true) {
  return queryOptions({
    queryKey: qk.taskWalkthrough.detail(taskId ?? ""),
    queryFn: () => (taskId ? getTaskWalkthrough(taskId) : Promise.resolve(null)),
    enabled: Boolean(taskId && enabled),
    staleTime: 60_000,
  });
}
