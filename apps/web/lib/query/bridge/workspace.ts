...
function registerRepositoryHandlers(
  ...
  // Workspace repositories: invalidate the per-workspace repos cache when a
  // repo is created / updated / deleted so dialogs that re-open within the
  // 30s staleTime still see fresh data (e.g. URL-added repos appearing in the
  // workspace dropdown on subsequent task-create dialog opens).
  const invalidateRepos = (workspaceId: string | undefined) => {
    if (!workspaceId) return;
    void queryClient.invalidateQueries({ queryKey: qk.workspaces.repos(workspaceId) });
  };
  return [
    ws.on(
      "repository.created",
      wrapBridgeHandler(queryClient, "repository.created", (message) => {
        invalidateRepos(message.payload.workspace_id);
      }),
    ),
    ws.on(
      "repository.updated",
      wrapBridgeHandler(queryClient, "repository.updated", (message) => {
        invalidateRepos(message.payload.workspace_id);
      }),
    ),
    ws.on(
      "repository.deleted",
      wrapBridgeHandler(queryClient, "repository.deleted", (message) => {
        invalidateRepos(message.payload.workspace_id);
      }),
    ),
  ];
...
export function registerWorkspaceBridge(ws: WebSocketClient, queryClient: QueryClient): () => void {
  const unsubCreated = ws.on(
    "workspace.created",
    wrapBridgeHandler(queryClient, "workspace.created", (message) => {
  ...
  const unsubUpdated = ws.on(
    "workspace.updated",
    wrapBridgeHandler(queryClient, "workspace.updated", (message) => {
      const payload = message.payload;
  ...
  const unsubDeleted = ws.on(
    "workspace.deleted",
    wrapBridgeHandler(queryClient, "workspace.deleted", (message) => {
  ...
  const repoUnsubs = registerRepositoryHandlers(ws, queryClient);

  return () => {
    unsubCreated();
    unsubUpdated();
    unsubDeleted();
    for (const fn of repoUnsubs) fn();
  };
}
