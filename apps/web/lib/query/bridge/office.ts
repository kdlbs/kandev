 ...
 *
 * The TanStack Query bridge for office WS events: it is now the sole
 * handler for the office domain (the Zustand mirror + its WS handler have
 * been removed), writing directly into the TQ cache via
...
function registerTaskMutationHandlers(d: TaskHandlerDeps): Array<() => void> {
  const unsubUpdated = d.ws.on(
    "office.task.updated",
    wrapBridgeHandler(d.qc, "office.task.updated", (message) => {
  ...
  const unsubCreated = d.ws.on(
    "office.task.created",
    wrapBridgeHandler(d.qc, "office.task.created", (message) => {
      const p = message.payload;
      if (!d.isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? d.getWsId();
      if (!wsId) return;
  ...
  const unsubMoved = d.ws.on(
    "office.task.moved",
    wrapBridgeHandler(d.qc, "office.task.moved", (message) => {
      const p = message.payload;
      if (!d.isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? d.getWsId();
      if (!wsId) return;
  ...
  const unsubStatusChanged = d.ws.on(
    "office.task.status_changed",
    wrapBridgeHandler(d.qc, "office.task.status_changed", (message) => {
      const p = message.payload;
      if (!d.isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? d.getWsId();
      if (!wsId) return;
...
function registerTaskNotificationHandlers(d: TaskHandlerDeps): Array<() => void> {
  const unsubDecision = d.ws.on(
    "office.task.decision_recorded",
    wrapBridgeHandler(d.qc, "office.task.decision_recorded", (message) => {
      const p = message.payload;
      if (!d.isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? d.getWsId();
      if (!wsId) return;
  ...
  const unsubReviewRequested = d.ws.on(
    "office.task.review_requested",
    wrapBridgeHandler(d.qc, "office.task.review_requested", (message) => {
      const p = message.payload;
      if (!d.isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? d.getWsId();
      if (!wsId) return;
  ...
  const unsubComment = d.ws.on(
    "office.comment.created",
    wrapBridgeHandler(d.qc, "office.comment.created", (message) => {
      const p = message.payload;
      if (!d.isCurrentWorkspace(p)) return;
...
function registerTaskHandlers(
  ...
  // Invalidates both the single-page `tasks` key and the paginated
  // `tasksPaginated` key (partial-match across every filter combination
  // for the workspace) so the office tasks list page picks up new /
  // deleted / moved tasks. The 30s staleTime keeps refetch cost bounded.
  const invalidateTasks = (wsId: string) => {
    void qc.invalidateQueries({ queryKey: qk.office.tasks(wsId) });
    void qc.invalidateQueries({ queryKey: qk.office.tasksPaginated(wsId), exact: false });
  };
  ...
  const patchTasksKey = (wsId: string, taskId: string, fields: Partial<OfficeTask>) => {
    ...
    // The infinite cache holds InfiniteData<ListTasksResponse>, not a
    // flat OfficeTask[] — invalidate so it refetches with the new state.
    // Partial-match across every filter combination for the workspace.
    void qc.invalidateQueries({ queryKey: qk.office.tasksPaginated(wsId), exact: false });
  };
...
function registerAgentHandlers(
  ...
  const unsubCompleted = ws.on(
    ...
    wrapBridgeHandler(qc, "office.agent.completed", (message) => {
      const p = message.payload;
  ...
  const unsubFailed = ws.on(
    "office.agent.failed",
    wrapBridgeHandler(qc, "office.agent.failed", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubUpdated = ws.on(
    ...
    wrapBridgeHandler(qc, "office.agent.updated", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
...
function registerMiscHandlers(
  ...
  const unsubApprovalCreated = ws.on(
    "office.approval.created",
    wrapBridgeHandler(qc, "office.approval.created", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubApprovalResolved = ws.on(
    "office.approval.resolved",
    wrapBridgeHandler(qc, "office.approval.resolved", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubCostRecorded = ws.on(
    "office.cost.recorded",
    wrapBridgeHandler(qc, "office.cost.recorded", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubRunQueued = ws.on(
    "office.run.queued",
    wrapBridgeHandler(qc, "office.run.queued", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubRunProcessed = ws.on(
    "office.run.processed",
    wrapBridgeHandler(qc, "office.run.processed", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubRoutineTriggered = ws.on(
    "office.routine.triggered",
    wrapBridgeHandler(qc, "office.routine.triggered", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
...
function registerRoutingHandlers(
  ...
  const unsubHealthChanged = ws.on(
    "office.provider.health_changed",
    wrapBridgeHandler(qc, "office.provider.health_changed", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
  ...
  const unsubRouteAttempt = ws.on(
    "office.route_attempt.appended",
    wrapBridgeHandler(qc, "office.route_attempt.appended", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
  ...
  const unsubSettingsUpdated = ws.on(
    "office.routing.settings_updated",
    wrapBridgeHandler(qc, "office.routing.settings_updated", (message) => {
      const p = message.payload;
      if (!isCurrentWorkspace(p)) return;
      const wsId = (p.workspace_id as string | undefined) ?? getWsId();
      if (!wsId) return;
