...
function makeContext(): TestContext {
  ...
  const cleanup = registerSessionBridge(ws as any, qc, { isEphemeralSurface: () => false });
...
// ---------------------------------------------------------------------------
// Task plan revision tests
// ---------------------------------------------------------------------------

const REV_CREATED = "task.plan.revision.created";
...
describe("session bridge — task plan revisions", () => {
  let ctx: TestContext;
  beforeEach(() => {
    ctx = makeContext();
  });

  it("seeds the revisions cache on first task.plan.revision.created", () => {
    emitRevision(ctx, { task_id: "task-20", id: "rev-1", revision_number: 1 });

    const data = ctx.qc.getQueryData<TaskPlanRevisionsData>(
      qk.taskSession.plansRevisions("task-20"),
    );
    expect(data?.revisions).toHaveLength(1);
    expect(data?.revisions[0].id).toBe("rev-1");
    expect(data?.revisions[0].revision_number).toBe(1);
    expect(data?.revisions[0].revert_of_revision_id).toBeNull();
  });

  it("prepends new revisions and keeps the list newest-first", () => {
    emitRevision(ctx, { task_id: "task-21", id: "rev-1", revision_number: 1 });
    emitRevision(ctx, { task_id: "task-21", id: "rev-2", revision_number: 2 });
    emitRevision(ctx, { task_id: "task-21", id: "rev-3", revision_number: 3 });

    const data = ctx.qc.getQueryData<TaskPlanRevisionsData>(
      qk.taskSession.plansRevisions("task-21"),
    );
    expect(data?.revisions.map((r) => r.revision_number)).toEqual([3, 2, 1]);
  });

  it("merges fields when a revision id already exists (coalesced write)", () => {
    emitRevision(ctx, { task_id: "task-22", id: "rev-1", revision_number: 1, title: "First" });
    emitRevision(ctx, {
      task_id: "task-22",
      id: "rev-1",
      revision_number: 1,
    ...
    const data = ctx.qc.getQueryData<TaskPlanRevisionsData>(
      qk.taskSession.plansRevisions("task-22"),
    );
    expect(data?.revisions).toHaveLength(1);
    expect(data?.revisions[0].title).toBe("First (updated)");
    expect(data?.revisions[0].updated_at).toBe(T1);
  });

  it("preserves a backfilled revision list when a new revision arrives", () => {
    ctx.qc.setQueryData<TaskPlanRevisionsData>(qk.taskSession.plansRevisions("task-23"), {
      revisions: [
        {
          id: "rev-old",
          task_id: "task-23" as import("@/lib/types/http").TaskId,
          revision_number: 1,
          title: "Old",
          author_kind: "agent",
          author_name: "agent",
    ...
    emitRevision(ctx, { task_id: "task-23", id: "rev-new", revision_number: 2 });

    const data = ctx.qc.getQueryData<TaskPlanRevisionsData>(
      qk.taskSession.plansRevisions("task-23"),
    );
    expect(data?.revisions.map((r) => r.id)).toEqual(["rev-new", "rev-old"]);
  });

  it("carries revert_of_revision_id through from the payload", () => {
    emitRevision(ctx, {
      task_id: "task-24",
      id: "rev-revert",
      revision_number: 2,
      revert_of_revision_id: "rev-1",
    });

    const data = ctx.qc.getQueryData<TaskPlanRevisionsData>(
      qk.taskSession.plansRevisions("task-24"),
    );
    expect(data?.revisions[0].revert_of_revision_id).toBe("rev-1");
  });
});
