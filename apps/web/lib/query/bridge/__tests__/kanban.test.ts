...
describe("applyIfNewer", () => {
  ...
  it("returns prev when next has the same timestamp and no authoritative diff", () => {
    const prev = makeTask({ updatedAt: TS_NEW, title: "Prev" });
    const next = makeTask({ updatedAt: TS_NEW, title: "Next" });
    // Only the (non-authoritative) title differs → stale event, prev preserved.
    expect(applyIfNewer(prev, next)).toBe(prev);
  });

  it("folds a changed workflowStepId through on a same-timestamp event", () => {
    // Regression: a workflow transition (In Progress → Review on turn-complete)
    // can land in the same wall-clock second as the prior cached update, so a
    // `<=` tie reads as "stale" and the step change would be dropped, leaving
    // the stepper on the old step.
    const prev = makeTask({ updatedAt: TS_NEW, title: "Prev", workflowStepId: "in-progress" });
    const next = makeTask({ updatedAt: TS_NEW, title: "Next", workflowStepId: "review" });
    const result = applyIfNewer(prev, next);
    expect(result.workflowStepId).toBe("review");
    // Title (non-authoritative) is preserved from prev.
    expect(result.title).toBe("Prev");
  });

  it("folds a changed workflowStepId through on a strictly-older event", () => {
    const prev = makeTask({ updatedAt: TS_NEW, workflowStepId: "in-progress" });
    const next = makeTask({ updatedAt: TS_OLD, workflowStepId: "review" });
    expect(applyIfNewer(prev, next).workflowStepId).toBe("review");
  });

  it("does not let a stale event with an empty workflowStepId clear a real step", () => {
    const prev = makeTask({ updatedAt: TS_NEW, workflowStepId: "review" });
    const next = makeTask({ updatedAt: TS_OLD, workflowStepId: "" });
    expect(applyIfNewer(prev, next).workflowStepId).toBe("review");
  });

  it("folds a changed task state through on a same-timestamp event", () => {
    const prev = makeTask({ updatedAt: TS_NEW, state: "IN_PROGRESS" });
    const next = makeTask({ updatedAt: TS_NEW, state: "REVIEW" });
    expect(applyIfNewer(prev, next).state).toBe("REVIEW");
  });

  it("does not let a stale event with an undefined state clear a real state", () => {
    // Analog of the workflowStepId empty-value guard: a timestamp-stale event
    // that simply omits `state` (undefined) carries no information and must not
    // clobber the cached state — only a defined, different value is authoritative.
    const prev = makeTask({ updatedAt: TS_NEW, state: "REVIEW" });
    const next = makeTask({ updatedAt: TS_OLD, state: undefined });
    expect(applyIfNewer(prev, next).state).toBe("REVIEW");
  });
  ...
  it("folds a changed primarySessionId onto prev when next is stale by timestamp", () => {
    const prev = makeTask({ updatedAt: TS_NEW, title: "Prev", primarySessionId: "sess-1" });
    const next = makeTask({ updatedAt: TS_NEW, title: "Next", primarySessionId: "sess-2" });
    const result = applyIfNewer(prev, next);
    // Title (stale) is preserved; primarySessionId (authoritative) is applied.
    expect(result.title).toBe("Prev");
    expect(result.primarySessionId).toBe("sess-2");
  });

  it("does not clear primarySessionId when a stale next omits it", () => {
    const prev = makeTask({ updatedAt: TS_NEW, primarySessionId: "sess-1" });
    const next = makeTask({ updatedAt: TS_OLD, primarySessionId: undefined });
    expect(applyIfNewer(prev, next)).toBe(prev);
  });
});
...
describe("registerKanbanBridge — task.updated", () => {
  ...
  it("applies a workflow step transition even when the timestamp did not advance", () => {
    // Regression: a turn-complete workflow transition (In Progress → Review)
    // can broadcast a task.updated whose updated_at equals the cached value
    // (same wall-clock second). The `<=` guard read this as stale and dropped
    // the new step, stranding the top-bar stepper on the old step. The
    // authoritative workflow_step_id must fold through.
    ws.emit(EV_TASK_UPDATED, {
      task_id: "t1",
      workflow_id: WF1,
      workflow_step_id: "s2-review",
      title: "Task 1",
      position: 0,
      updated_at: TS_OLD, // same timestamp as the seeded task (TS_OLD)
    });
    const task = getMulti(qc)?.snapshots[WF1]?.tasks.find((t) => t.id === "t1");
    expect(task?.workflowStepId).toBe("s2-review");
  });
...
describe("registerKanbanBridge — task.updated primary-session changes", () => {
  let ws: FakeWs;
  let qc: QueryClient;
  ...
  it("applies a primary-session change even when the timestamp did not advance", () => {
    // Regression: SetSessionPrimary updates task_sessions.updated_at but NOT
    // the task row's updated_at, so the task.updated it broadcasts carries an
    // unchanged updated_at. The timestamp guard must still let the new
    // primary_session_id through, or the star never moves to the new primary.
    seedMulti(qc, {
      snapshots: {
        [WF1]: {
          workflowId: WF1,
          workflowName: "WF1",
          steps: [],
          tasks: [makeTask({ id: "t1", primarySessionId: "sess-1", updatedAt: TS_OLD })],
        },
      },
    });
    ...
    ws.emit(EV_TASK_UPDATED, {
      ...
      title: "Task 1",
      position: 0,
      primary_session_id: "sess-2",
      updated_at: TS_OLD, // same timestamp — stale by the guard
    });

    const task = getMulti(qc)?.snapshots[WF1]?.tasks.find((t) => t.id === "t1");
    expect(task?.primarySessionId).toBe("sess-2");
  });

  it("does not clear a known primary when a stale event omits primary_session_id", () => {
    seedMulti(qc, {
      snapshots: {
        [WF1]: {
          workflowId: WF1,
          workflowName: "WF1",
          steps: [],
          tasks: [makeTask({ id: "t1", primarySessionId: "sess-1", updatedAt: TS_NEW })],
        },
      },
    });
    ...
    ws.emit(EV_TASK_UPDATED, {
      ...
      title: "Stale",
      position: 0,
      updated_at: TS_OLD, // older → stale, and omits primary_session_id
    });

    const task = getMulti(qc)?.snapshots[WF1]?.tasks.find((t) => t.id === "t1");
    expect(task?.primarySessionId).toBe("sess-1");
    expect(task?.title).toBe("Task 1");
  });
});
