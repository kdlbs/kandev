...
const EVT_TASK_UPDATED = "office.task.updated";
...
describe("registerOfficeBridge — task handlers", () => {
  ...
  it("office.task.updated patches the task in cache", () => {
    ...
    expect(tasks?.[0]?.title).toBe(UPDATED_TITLE);
  ...
  it("office.task.moved invalidates the paginated tasks key (partial match)", () => {
    // Same useInfiniteQuery migration guard as task.updated: a status move
    // can't be patched into InfiniteData in place, so the bridge must
    // invalidate every filter combination of the paginated key.
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ queryKey: qk.office.tasksPaginated(WS_ID), exact: false }),
    );
  });
  ...
  it("office.task.status_changed invalidates the paginated tasks key (partial match)", () => {
    const spy = vi.spyOn(qc, "invalidateQueries");
    ws.emit("office.task.status_changed", {
      workspace_id: WS_ID,
      task_id: TASK_ID,
  ...
  it("office.task.updated invalidates the paginated tasks key (partial match)", () => {
    // The office tasks list reads via useInfiniteQuery(tasksPaginated); the
    // bridge can't patch InfiniteData<ListTasksResponse> in place, so it must
    // invalidate every filter combination of that key. Regression guard for
    // the useInfiniteQuery migration (PR #1130 review blocker).
    const spy = vi.spyOn(qc, "invalidateQueries");
    ws.emit(EVT_TASK_UPDATED, {
      workspace_id: WS_ID,
      task_id: TASK_ID,
  ...
  it("office.task.created invalidates the paginated tasks key", () => {
    const spy = vi.spyOn(qc, "invalidateQueries");
    ws.emit("office.task.created", { workspace_id: WS_ID, task_id: "task-2" });
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ queryKey: qk.office.tasksPaginated(WS_ID), exact: false }),
    );
  });
