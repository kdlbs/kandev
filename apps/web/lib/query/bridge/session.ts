...
export function registerSessionBridge(
  ...
  return registerBridgeHandlers(ws, queryClient, {
    ...
    "task.walkthrough.created": (message) => {
      upsertTaskWalkthrough(queryClient, message);
    },
    "task.walkthrough.updated": (message) => {
      upsertTaskWalkthrough(queryClient, message);
    },
    "task.walkthrough.deleted": (message) => {
      queryClient.setQueryData(qk.taskWalkthrough.detail(message.payload.task_id), null);
    },
  });
}
...
function upsertTaskWalkthrough(queryClient: QueryClient, message: WalkthroughEvent): void {
  const payload = message.payload;
  const walkthrough: TaskWalkthrough = {
    id: payload.id,
    task_id: payload.task_id,
    title: payload.title,
    steps: payload.steps,
  ...
  queryClient.setQueryData(qk.taskWalkthrough.detail(payload.task_id), walkthrough);
}
