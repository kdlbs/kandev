      ...
      if (msg.id !== undefined && msg.method !== undefined) {
        ...
        if (handler) {
          try {
            ...
            this.ws.send(JSON.stringify({ jsonrpc: '2.0', id: msg.id, error: { code: -32603, message: String(err) } }));
          }
        } else {
...
function toMonacoRange(r: LspRange): { startLineNumber: number; startColumn: number; endLineNumber: number; endColumn: number } {
  return {
...
class LSPClientManager {
  ...
  saveEnabledState(sessionId: string, language: string): void {
    try { localStorage.setItem(this.lspStorageKey(sessionId, language), '1'); } catch {}
  }

  /** Clear the saved LSP state (manual stop). */
  clearEnabledState(sessionId: string, language: string): void {
    try { localStorage.removeItem(this.lspStorageKey(sessionId, language)); } catch {}
  }

  /** Check if LSP was previously enabled for this session+language. */
  isEnabledInStorage(sessionId: string, language: string): boolean {
    try { return localStorage.getItem(this.lspStorageKey(sessionId, language)) === '1'; } catch { return false; }
  }
  ...
  connect(sessionId: string, lspLanguage: string, userConfigs?: Record<string, Record<string, unknown>>): () => void {
    const key = `${sessionId}:${lspLanguage}`;
  ...
  private async initializeLsp(key: string, ws: WebSocket, lspLanguage: string, workspacePath: string | null, userConfigs?: Record<string, Record<string, unknown>>) {
    ...
    try {
      ...
      rpc.onNotification('textDocument/publishDiagnostics', (params) => {
        this.handleDiagnostics(params as { uri: string; diagnostics: Array<{ range: LspRange; message: string; severity?: number; source?: string; code?: unknown }> });
      });
      ...
      conn.providerDisposables = this.registerProviders(rpc, lspLanguage, key, conn.serverCapabilities, semanticRefreshCallbacks);
  ...
  private ensureModelsExist(uris: string[], connectionKey: string): void {
    ...
    for (const fileUri of uris) {
      ...
      if (conn?.workspacePath) {
        ...
        Promise.all([
          import('@/lib/ws/connection'),
          import('@/lib/ws/workspace-files'),
  ...
  openDocument(sessionId: string, lspLanguage: string, documentUri: string, languageId: string, text: string): void {
    const key = `${sessionId}:${lspLanguage}`;
    const conn = this.connections.get(key);
  ...
  private handleDiagnostics(params: { uri: string; diagnostics: Array<{ range: LspRange; message: string; severity?: number; source?: string; code?: unknown }> }) {
    const monaco = getMonacoInstance();
    if (!monaco) return;
  ...
  stop(sessionId: string, lspLanguage: string): void {
    ...
    if (conn.rpc && conn.initialized) {
      try {
        conn.rpc.sendRequest('shutdown', null).then(() => {
