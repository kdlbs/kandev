...
import type { OrchestrateSlice, Project } from "./types";
...
describe("project store actions", () => {
  it("setProjects replaces the list", () => {
    ...
    expect(store.getState().orchestrate.projects).toHaveLength(2);
  });
  ...
  it("addProject appends to the list", () => {
    ...
    expect(store.getState().orchestrate.projects).toHaveLength(2);
    expect(store.getState().orchestrate.projects[1].name).toBe("Project 2");
  });
  ...
  it("updateProject patches an existing project", () => {
    ...
    const project = store.getState().orchestrate.projects[0];
  ...
  it("updateProject is no-op for missing id", () => {
    ...
    expect(store.getState().orchestrate.projects[0].name).toBe("Project 1");
  });
  ...
  it("setProjects overwrites existing projects", () => {
    ...
    expect(store.getState().orchestrate.projects).toHaveLength(1);
    expect(store.getState().orchestrate.projects[0].name).toBe("New");
  });
});
