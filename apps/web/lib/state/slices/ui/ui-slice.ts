  ...
  return {
    openQuickChat: (
      ...
      set((draft) => {
        ...
        if (!sessionId) {
          const emptyTab = draft.quickChat.sessions.find((session) => session.sessionId === "");
          if (emptyTab) {
            emptyTab.workspaceId = workspaceId;
            emptyTab.kind = kind;
          } else {
            draft.quickChat.sessions.push({ sessionId: "", workspaceId, kind });
          }
