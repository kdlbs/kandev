...
import { IconBrandBitbucket, IconPuzzle, IconTicket } from "@tabler/icons-react";
...
describe("plugin icons", () => {
  ...
  it("provides the curated Bitbucket brand icon", () => {
    expect(lookupPluginIcon("bitbucket")).toBe(IconBrandBitbucket);
    expect(resolvePluginIcon("bitbucket")).toBe(IconBrandBitbucket);
  });
