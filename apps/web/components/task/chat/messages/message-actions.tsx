...
export function MessageActions({
  ...
  const turn = useAppStore((state) => {
    const turnId = message.turn_id;
    if (!turnId || !message.session_id) return null;
    return state.turns.bySession[message.session_id]?.find((item) => item.id === turnId) ?? null;
  });
  const usageMultiplier = useAppStore((state) => {
    if (!message.session_id) return null;
