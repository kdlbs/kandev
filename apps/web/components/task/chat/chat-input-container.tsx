...
function FailedSessionBanner({ showDialog, onShowDialog, taskId, taskTitle, taskDescription }: {
  showDialog: boolean;
    ...
    <>
      <div className="rounded border border-border overflow-hidden">
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          ...
          <Button variant="outline" size="sm" className="shrink-0 gap-1.5 cursor-pointer" onClick={() => onShowDialog(true)}>
...
function hasPendingCommentFiles(pendingCommentsByFile: Record<string, DiffComment[]> | undefined): boolean {
...
type DerivedInputParams = {
  isStarting: boolean; isSending: boolean; isFailed: boolean;
  ...
  isQueued: boolean; queuedMessage: string | null | undefined;
  onCancelQueue: (() => void) | undefined;
  updateQueueContent: ((content: string) => Promise<void>) | undefined;
  todoItems: { text: string }[]; allItemsLength: number;
  isInputFocused: boolean; placeholder: string | undefined;
  isAgentBusy: boolean; hasAgentCommands: boolean;
};
...
function computeDerivedInputState(p: DerivedInputParams): DerivedInputState {
  ...
  const hasQueuedMessage = !!(p.isQueued && p.queuedMessage && p.onCancelQueue && p.updateQueueContent);
...
function ChatInputBody({
  containerRef, height, resizeHandleProps, isPanelFocused, isInputFocused,
  isAgentBusy, hasClarification, showRequestChangesTooltip, hasPendingComments,
  showFocusHint, hasContextZone, allItems, sessionId, hasQueuedMessage,
  queuedMessage, onCancelQueue, updateQueueContent, queuedMessageRef, onQueueEditComplete,
  hasTodos, todoItems, pendingClarification, onClarificationResolved,
  value, inputRef, handleChange, handleSubmitWithReset, inputPlaceholder, isDisabled,
  planModeEnabled, submitKey, setIsInputFocused, taskId, onAddContextFile,
  onToggleContextFile, planContextEnabled, handleAgentCommand, handleImagePaste,
  onPlanModeChange, taskTitle, taskDescription, isSending, onCancel, contextCount,
  contextPopoverOpen, setContextPopoverOpen, contextFiles,
}: ChatInputBodyProps) {
  return (
    <div
      ref={containerRef}
    ...
    >
      ...
      {hasContextZone && (
        <ContextZone
          items={allItems}
          sessionId={sessionId}
          queueSlot={hasQueuedMessage ? (
            <QueuedMessageIndicator ref={queuedMessageRef} content={queuedMessage!} onCancel={onCancelQueue!} onUpdate={updateQueueContent!} isVisible={true} onEditComplete={onQueueEditComplete} />
          ) : undefined}
          todoSlot={hasTodos ? <TodoSummary todos={todoItems} /> : undefined}
          clarificationSlot={hasClarification ? (
            <div className="overflow-auto"><ClarificationInputOverlay message={pendingClarification!} onResolved={onClarificationResolved!} /></div>
          ) : undefined}
        />
      )}
...
export const ChatInputContainer = forwardRef<ChatInputContainerHandle, ChatInputContainerProps>(
  function ChatInputContainer(
    {
      onSubmit, sessionId, taskId, taskTitle, taskDescription,
      planModeEnabled, onPlanModeChange, isAgentBusy, isStarting, isSending, onCancel,
      placeholder, pendingClarification, onClarificationResolved,
      showRequestChangesTooltip = false, onRequestChangesTooltipDismiss,
      pendingCommentsByFile, submitKey = 'cmd_enter', hasAgentCommands = false,
      isFailed = false, isQueued = false, contextItems = [], planContextEnabled = false,
      contextFiles = [], onToggleContextFile, onAddContextFile, todoItems = [],
      queuedMessage, onCancelQueue, updateQueueContent, queuedMessageRef, onQueueEditComplete,
      isPanelFocused,
    },
    ref
    ...
    const { value, inputRef, handleImagePaste, handleChange, handleSubmit, allItems } = useChatInputState({
    ...
    useImperativeHandle(ref, () => ({
      ...
      insertText: (text: string, from: number, to: number) => { inputRef.current?.insertText(text, from, to); },
    }), [inputRef]);
    ...
    const handleAgentCommand = useCallback((commandName: string) => { onSubmit(`/${commandName}`); }, [onSubmit]);
    const handleSubmitWithReset = useCallback(() => handleSubmit(resetHeight), [handleSubmit, resetHeight]);

    const { isDisabled, hasClarification, hasPendingComments, hasQueuedMessage, hasTodos, hasContextZone, showFocusHint, inputPlaceholder } = computeDerivedInputState({
      isStarting, isSending, isFailed,
      pendingClarification, onClarificationResolved,
      pendingCommentsByFile,
      isQueued, queuedMessage, onCancelQueue, updateQueueContent,
      todoItems, allItemsLength: allItems.length,
      isInputFocused, placeholder, isAgentBusy, hasAgentCommands,
    });

    if (isFailed) {
      return <FailedSessionBanner showDialog={showNewSessionDialog} onShowDialog={setShowNewSessionDialog} taskId={taskId} taskTitle={taskTitle} taskDescription={taskDescription} />;
    }
