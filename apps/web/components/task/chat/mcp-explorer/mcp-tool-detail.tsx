...
function ArgumentRows({ tool }: { tool: MCPToolSummary }) {
  ...
  if (state.kind === "none") {
    return <p className="text-sm text-muted-foreground">{t("task:mcpNoArguments")}</p>;
  }
  if (state.kind === "too_large") {
    return <p className="text-sm text-muted-foreground">{t("task:mcpSchemaTooLarge")}</p>;
  }
  return (
      ...
      {state.arguments.map((argument) => (
        <div key={argument.name} className="rounded-md border border-border/70 px-3 py-2">
          <div className="flex min-w-0 flex-wrap items-baseline gap-2">
            <span className="break-all font-mono text-sm">{argument.name}</span>
            <span className="text-xs text-muted-foreground">{argument.type}</span>
            <span className="text-xs text-muted-foreground">
              {argument.required ? t("task:mcpRequired") : t("task:mcpOptional")}
            </span>
          </div>
          {argument.description && (
            <p className="mt-1 break-words text-sm text-muted-foreground">{argument.description}</p>
          )}
        </div>
      ))}
      {state.showJSON && (
        <details className="text-sm">
          ...
          <pre className="max-w-full overflow-x-auto rounded-md bg-muted/50 p-3 text-xs">
...
export function McpToolDetail({ tool, onBack }: { tool: MCPToolSummary; onBack: () => void }) {
  const { t } = useTranslation();
  return (
    <div data-testid="mcp-tool-detail" className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 border-b border-border/70 pb-2">
        ...
        <div className="flex min-w-0 flex-wrap items-baseline gap-3">
          <h3 className="min-w-0 break-all font-mono text-base font-semibold">{tool.name}</h3>
