...
function useCommentsState(resolvedSessionId: string | null): CommentsState {
...
function useChatPanelState({ sessionId, onOpenFile, onOpenFileAtLine }: UseChatPanelStateOptions) {
  const {
    resolvedSessionId, session, task, taskId, taskDescription,
    isStarting, isWorking, isAgentBusy, isFailed,
  } = useSessionState(sessionId);
  const { planModeEnabled, activeDocument, handlePlanModeChange } = usePlanMode(resolvedSessionId, taskId);
  const {
    contextFiles, removeContextFile, unpinFile, clearEphemeral,
    handleToggleContextFile, handleAddContextFile,
  } = useContextFiles(resolvedSessionId);
  const { addPlan } = usePanelActions();
  const { messages, isLoading: messagesLoading } = useSessionMessages(resolvedSessionId);
  const {
    allMessages, groupedItems, permissionsByToolCallId, childrenByParentToolCallId,
    todoItems, agentMessageCount, pendingClarification,
  } = useProcessedMessages(messages, taskId, resolvedSessionId, taskDescription);
  ...
  const { queuedMessage, isQueued, cancel: cancelQueue, updateContent: updateQueueContent } =
  ...
  const contextItems = useMemo<ContextItem[]>(
    ...
    [
      planContextEnabled, contextFiles, resolvedSessionId, removeContextFile, unpinFile, addPlan,
      promptsMap, onOpenFile, comments.pendingCommentsByFile, comments.handleRemoveCommentFile,
      comments.handleRemoveComment, onOpenFileAtLine, comments.planComments,
      comments.clearSessionPlanComments, taskId,
    ],
  );
  ...
  return {
    ...
    allMessages, groupedItems, permissionsByToolCallId, childrenByParentToolCallId,
    todoItems, agentMessageCount, pendingClarification,
    sessionModel, activeModel, chatSubmitKey, agentCommands,
    queuedMessage, isQueued, cancelQueue, updateQueueContent,
    ...comments,
    contextItems, planContextEnabled, prompts,
  };
}
...
function ChatInputArea({
  ...
  const {
    resolvedSessionId, task, taskId, taskDescription, isStarting, isAgentBusy, isFailed,
    planModeEnabled, activeDocument, handlePlanModeChange,
    contextFiles, handleToggleContextFile, handleAddContextFile,
    pendingCommentsByFile, chatSubmitKey, agentCommands,
    isQueued, queuedMessage, updateQueueContent, contextItems, planContextEnabled, todoItems,
  } = panelState;
...
function useSubmitHandler(
  ...
  const {
    resolvedSessionId, sessionModel, activeModel, planContextEnabled, isAgentBusy, activeDocument,
    planComments, contextFiles, prompts, markCommentsSent, clearSessionPlanComments, clearEphemeral,
  } = panelState;
  ...
  const handleSubmit = useCallback(
    ...
    [
      ...
      planComments.length, clearSessionPlanComments, resolvedSessionId, clearEphemeral,
    ],
  );
...
export const TaskChatPanel = memo(function TaskChatPanel({
  ...
  const {
    resolvedSessionId, session, taskId, isWorking, messagesLoading, groupedItems,
    allMessages, permissionsByToolCallId, childrenByParentToolCallId, agentMessageCount, cancelQueue,
  } = panelState;
