...
vi.mock("@/lib/sidebar/apply-view", () => ({
  applyView: (tasks: unknown[]) => ({
    groups: [{ key: "all", label: "All tasks", tasks }],
    subTasksByParentId: new Map(),
  }),
  flattenVisibleTaskIds: (grouped: {
    groups: Array<{ tasks: Array<{ id: string }> }>;
    subTasksByParentId: Map<string, Array<{ id: string }>>;
  }) => grouped.groups.flatMap((group) => group.tasks.map((task) => task.id)),
  computeMixedWorkflowSelection: (
    displayTasks: Array<{ id: string; workflowId?: string }>,
    selectedIds: Set<string>,
  ) => {
    const wfIds = new Set<string>();
    const movableSelectedIds = new Set<string>();
    let hasWorkflowless = false;
    for (const task of displayTasks) {
      if (!selectedIds.has(task.id)) continue;
      if (task.workflowId) {
        wfIds.add(task.workflowId);
        movableSelectedIds.add(task.id);
      } else {
        hasWorkflowless = true;
      }
    }
    return { isMixedWorkflowSelection: hasWorkflowless || wfIds.size > 1, movableSelectedIds };
  },
  sortIdsByVisibleOrder: (ids: string[], visibleTaskIds: string[]) => {
    const order = new Map(visibleTaskIds.map((id, index) => [id, index]));
    return [...ids].sort(
      (a, b) =>
...
vi.mock("./task-switcher", () => ({
  TaskSwitcher: ({
    ...
    const tasks = grouped.groups.flatMap((group) => group.tasks);
    return (
