...
export function SwimlaneContainer({
  ...
  if (isLoading && Object.keys(snapshots).length === 0) {
    return (
  ...
  // Filter to only workflows with tasks (unless a specific workflow is selected)
    ...
    <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 space-y-3">
      {visibleWorkflows.map((wf) => {
        ...
        return (
          ...
          >
            {(() => {
              const viewEntry = getViewByStoredValue(viewMode) ?? getDefaultView();
