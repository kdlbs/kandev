...
export function MonacoCodeEditor({
  ...
  const state = useMonacoEditorComments({
    path, enableComments, sessionId, wrapperRef, onChange, onSave, contentRef,
  });

  const lsp = useMonacoEditorLsp({
    sessionId, worktreePath, language, path, contentRef, editorRef: state.editorRef,
  });
  ...
  useEditorViewZoneComments(state.editorInstance, [state.comments, state.formZoneRange, state.editingCommentId], (addZone) => {
    ...
    if (state.formZoneRange) {
      addZone(state.formZoneRange.endLine, 120, <div className="px-2 py-1" data-comment-zone><CommentForm onSubmit={(c) => state.handleCommentSubmitRef.current(c)} onCancel={() => { state.setFormZoneRange(null); state.clearGutterSelection(); }} /></div>);
    }
  });
