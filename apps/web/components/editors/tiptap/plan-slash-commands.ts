...
import {
  ...
  IconCode, IconQuote, IconTable, IconMinus,
...
const PLAN_SLASH_COMMANDS: PlanSlashCommand[] = [
  ...
  // Lists
  {
    ...
    icon: IconList, category: 'Lists',
    action: (editor, range) => {
      editor.chain().focus().deleteRange(range).toggleBulletList().run();
    },
  },
  {
    id: 'numberedList', label: 'Numbered List', description: 'Ordered list',
    icon: IconListNumbers, category: 'Lists',
    action: (editor, range) => {
      editor.chain().focus().deleteRange(range).toggleOrderedList().run();
    },
  },
  {
    id: 'taskList', label: 'Task List', description: 'Checklist items',
    icon: IconListCheck, category: 'Lists',
    action: (editor, range) => {
  ...
  // Blocks
  {
    id: 'codeBlock', label: 'Code Block', description: 'Fenced code',
    icon: IconCode, category: 'Blocks',
    action: (editor, range) => {
      editor.chain().focus().deleteRange(range).toggleCodeBlock().run();
    },
  },
  {
    id: 'blockquote', label: 'Blockquote', description: 'Quote text',
    icon: IconQuote, category: 'Blocks',
    action: (editor, range) => {
      editor.chain().focus().deleteRange(range).toggleBlockquote().run();
    },
  },
  {
    id: 'table', label: 'Table', description: '3x3 table',
    icon: IconTable, category: 'Blocks',
    action: (editor, range) => {
  ...
  {
    id: 'horizontalRule', label: 'Divider', description: 'Horizontal line',
    icon: IconMinus, category: 'Blocks',
    action: (editor, range) => {
