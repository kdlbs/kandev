...
import { IconDeviceFloppy, IconLoader2, IconTrash, IconTextWrap, IconTextWrapDisabled, IconMessagePlus, IconRefresh } from '@tabler/icons-react';
...
function CodeMirrorWrapButton({ wrapEnabled, onToggleWrap }: { wrapEnabled: boolean; onToggleWrap: () => void }) {
  return (
    <Tooltip>
...
function CodeMirrorToolbar({
  ...
  enableComments, sessionId, commentCount, hasRemoteUpdate,
  onToggleWrap, onSave, onReloadFromAgent, onDelete,
}: {
  ...
  commentCount: number; hasRemoteUpdate?: boolean; onToggleWrap: () => void; onSave: () => void;
  onReloadFromAgent?: () => void;
  onDelete?: () => void;
}) {
  return (
    <PanelHeaderBarSplit
      ...
      right={
        <div className="flex items-center gap-1">
          ...
          <CodeMirrorReloadButton hasRemoteUpdate={hasRemoteUpdate} onReloadFromAgent={onReloadFromAgent} />
...
export function CodeMirrorCodeEditor({
  path, content, originalContent, isDirty, hasRemoteUpdate = false, isSaving,
  ...
  const state = useCodeMirrorEditorState({
    path, content, originalContent, isDirty, isSaving,
  ...
  return (
    <div ref={wrapperRef} className="flex h-full flex-col rounded-lg">
      <CodeMirrorToolbar
        ...
        commentCount={state.comments.length} hasRemoteUpdate={hasRemoteUpdate}
        onToggleWrap={() => state.setWrapEnabled(!state.wrapEnabled)}
        onSave={onSave} onReloadFromAgent={onReloadFromAgent} onDelete={onDelete}
      />
