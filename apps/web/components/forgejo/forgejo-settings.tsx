...
function ForgejoConnection({ workspaceId }: { workspaceId: string }) {
  const { config, error: configError, save: saveConfig, refresh: refreshConfig, disconnect: disconnectConfig } = useForgejoConfig(workspaceId);
  ...
  const [repositories, setRepositories] = useState<ForgejoRepository[]>([]);
	...
	const [reviewWatches, setReviewWatches] = useState<ForgejoReviewWatch[]>([]);
	const [reviewOwner, setReviewOwner] = useState(""); const [reviewRepo, setReviewRepo] = useState(""); const [reviewWorkflow, setReviewWorkflow] = useState("");
	const [presets, setPresets] = useState<ForgejoActionPreset[]>([]); const [presetName, setPresetName] = useState(""); const [presetInstructions, setPresetInstructions] = useState("");
  ...
  const save = useCallback(async () => { const next = await saveConfig({ origin, token: token || undefined, webhook_secret: webhookSecret || undefined }); setSavedOrigin(next.origin); setOrigin(next.origin); setToken(""); setWebhookSecret(""); setMessage(`Saved connection for ${next.username}`); }, [origin, saveConfig, token, webhookSecret]);
  ...
  const loadQueue = async () => { setQueue(await listForgejoQueue(options)); };
	...
	const pollReviewWatch = async (watch: ForgejoReviewWatch) => { const result = await pollForgejoReviewWatch(watch.id, options); setMessage(`Review watch found ${result.pull_requests.length} open pull request${result.pull_requests.length === 1 ? "" : "s"}`); await loadReviewWatches(); };
	const loadPresets = async () => { const result = await listForgejoActionPresets(options); setPresets(result.presets); };
	const savePreset = async () => { await saveForgejoActionPreset({ kind: "review", name: presetName, instructions: presetInstructions }, options); await loadPresets(); setPresetName(""); setPresetInstructions(""); };
	const deletePreset = async (preset: ForgejoActionPreset) => { await deleteForgejoActionPreset(preset.id, options); await loadPresets(); };
  ...
  useSettingsSaveContributor({
    ...
    invalidReason: !validOrigin ? "Enter a valid HTTP or HTTPS Forgejo origin." : "Enter a Forgejo token before connecting.",
    save,
    discard: () => { setOrigin(savedOrigin); setToken(""); setWebhookSecret(""); },
  });
  ...
  return <SettingsCard>
    ...
    <CardContent className="space-y-3">
      ...
      <Input data-testid="forgejo-token-input" type="password" placeholder={config?.has_secret ? "Token saved — enter a new value to replace it" : "Forgejo personal access token"} value={token} onChange={(event) => setToken(event.target.value)} />
		<Input data-testid="forgejo-webhook-secret-input" type="password" placeholder={config?.has_webhook_secret ? "Webhook secret saved — enter a new value to replace it" : "Webhook secret, optional"} value={webhookSecret} onChange={(event) => setWebhookSecret(event.target.value)} />
		<p className="text-sm text-muted-foreground">Set the same secret on the Forgejo webhook. It signs incoming issue events; polling still catches anything the webhook misses.</p>
      ...
      {message || configError ? <p role="status" className="text-sm text-muted-foreground">{message || configError}</p> : null}
      {repositories.length ? <ul className="text-sm space-y-1">{repositories.map((repository) => <li key={repository.full_name}>{repository.full_name}</li>)}</ul> : null}
		...
		{config?.has_secret ? <div className="space-y-2 border-t pt-3"><p className="font-medium text-sm">Issue watches</p><p className="text-sm text-muted-foreground">A matching issue creates a task in the workflow and step you choose. Use repository ID and agent profile ID when this workspace has multiple repositories or agents. An inflight limit of 0 means unlimited active tasks.</p><div className="grid gap-2 sm:grid-cols-3"><Input placeholder="Owner" value={watchOwner} onChange={(event) => setWatchOwner(event.target.value)} /><Input placeholder="Repository" value={watchRepo} onChange={(event) => setWatchRepo(event.target.value)} /><Input placeholder="Labels, optional" value={watchLabels} onChange={(event) => setWatchLabels(event.target.value)} /><Input placeholder="Workflow ID" value={watchWorkflow} onChange={(event) => setWatchWorkflow(event.target.value)} /><Input placeholder="Workflow step ID, optional" value={watchStep} onChange={(event) => setWatchStep(event.target.value)} /><Input placeholder="Kandev repository ID, optional" value={watchRepository} onChange={(event) => setWatchRepository(event.target.value)} /><Input placeholder="Base branch, optional" value={watchBaseBranch} onChange={(event) => setWatchBaseBranch(event.target.value)} /><Input placeholder="Agent profile ID, optional" value={watchAgentProfile} onChange={(event) => setWatchAgentProfile(event.target.value)} /><Input placeholder="Executor profile ID, optional" value={watchExecutorProfile} onChange={(event) => setWatchExecutorProfile(event.target.value)} /><Input type="number" min="30" placeholder="Poll interval seconds" value={watchInterval} onChange={(event) => setWatchInterval(event.target.value)} /><Input type="number" min="0" placeholder="Inflight limit (0 unlimited)" value={watchInflightLimit} onChange={(event) => setWatchInflightLimit(event.target.value)} /><Input placeholder="Task instructions, optional" value={watchPrompt} onChange={(event) => setWatchPrompt(event.target.value)} /></div><div className="flex flex-wrap gap-2"><Button className="cursor-pointer" type="button" disabled={!watchOwner || !watchRepo || !watchWorkflow} onClick={() => void saveWatch()}>Save watch</Button><Button className="cursor-pointer" variant="outline" type="button" onClick={() => void loadWatches()}>Load watches</Button></div><ul className="space-y-1 text-sm">{watches.map((watch) => <li className="flex flex-wrap items-center gap-2" key={watch.id}><span>{watch.owner}/{watch.repo}{watch.labels ? ` · ${watch.labels}` : ""}{watch.workflow_id ? ` → ${watch.workflow_id}` : ""}</span><Button className="cursor-pointer" size="sm" variant="outline" type="button" onClick={() => void pollWatch(watch)}>Poll</Button><Button className="cursor-pointer" size="sm" variant="destructive" type="button" onClick={() => void deleteWatch(watch)}>Delete</Button></li>)}</ul></div> : null}
		{config?.has_secret ? <div className="space-y-2 border-t pt-3"><p className="font-medium text-sm">Review watches</p><p className="text-sm text-muted-foreground">Each open pull request in the selected Forgejo repository creates one Kandev task. Use this for a recurring review queue.</p><div className="grid gap-2 sm:grid-cols-3"><Input placeholder="Owner" value={reviewOwner} onChange={(event) => setReviewOwner(event.target.value)} /><Input placeholder="Repository" value={reviewRepo} onChange={(event) => setReviewRepo(event.target.value)} /><Input placeholder="Workflow ID" value={reviewWorkflow} onChange={(event) => setReviewWorkflow(event.target.value)} /></div><div className="flex flex-wrap gap-2"><Button className="cursor-pointer" type="button" disabled={!reviewOwner || !reviewRepo || !reviewWorkflow} onClick={() => void saveReviewWatch()}>Save review watch</Button><Button className="cursor-pointer" variant="outline" type="button" onClick={() => void loadReviewWatches()}>Load review watches</Button></div><ul className="space-y-1 text-sm">{reviewWatches.map((watch) => <li className="flex flex-wrap items-center gap-2" key={watch.id}><span>{watch.owner}/{watch.repo} → {watch.workflow_id}</span><Button className="cursor-pointer" size="sm" variant="outline" type="button" onClick={() => void pollReviewWatch(watch)}>Poll</Button><Button className="cursor-pointer" size="sm" variant="destructive" type="button" onClick={() => void deleteReviewWatch(watch)}>Delete</Button></li>)}</ul></div> : null}
		{config?.has_secret ? <div className="space-y-2 border-t pt-3"><p className="font-medium text-sm">Action presets</p><p className="text-sm text-muted-foreground">Save reusable review instructions, then copy them into a review action or watch when needed.</p><div className="grid gap-2 sm:grid-cols-2"><Input placeholder="Preset name" value={presetName} onChange={(event) => setPresetName(event.target.value)} /><Input placeholder="Instructions" value={presetInstructions} onChange={(event) => setPresetInstructions(event.target.value)} /></div><div className="flex flex-wrap gap-2"><Button className="cursor-pointer" type="button" disabled={!presetName} onClick={() => void savePreset()}>Save preset</Button><Button className="cursor-pointer" variant="outline" type="button" onClick={() => void loadPresets()}>Load presets</Button></div><ul className="space-y-1 text-sm">{presets.map((preset) => <li className="flex flex-wrap items-center gap-2" key={preset.id}><span>{preset.name}{preset.instructions ? ` · ${preset.instructions}` : ""}</span><Button className="cursor-pointer" size="sm" variant="destructive" type="button" onClick={() => void deletePreset(preset)}>Delete</Button></li>)}</ul></div> : null}
    </CardContent>
  </SettingsCard>;
}
