...
function DialogHeaderContent({
  isCreateMode, isEditMode, isTaskStarted, sessionRepoName, initialTitle, taskName,
  repositoryId, discoveredRepoPath, workspaceId, repositoriesLoading, discoverReposLoading,
  headerRepositoryOptions, onRepositoryChange, onTaskNameChange,
...
function DialogFormBody({
  open, isSessionMode, isCreateMode, isTaskStarted, isPassthroughProfile, initialDescription,
  hasDescription, branchOptions, branchesLoading, agentProfileOptions, executorOptions,
  agentProfiles, agentProfilesLoading, executorsLoading, isCreatingSession, workflows, snapshots,
  effectiveWorkflowId, fs, handleKeyDown, onBranchChange, onAgentProfileChange, onWorkflowChange,
    ...
    <div className="flex-1 space-y-4 overflow-y-auto pr-1">
      ...
      {isPassthroughProfile && hasDescription && (
        <p className="text-xs text-amber-500">
          Prompt will be ignored — passthrough sessions don&apos;t support sending a prompt on start.
        </p>
      )}
      {!isSessionMode && (
        <CreateEditSelectors
      ...
      <WorkflowSection
        isCreateMode={isCreateMode} isTaskStarted={isTaskStarted}
        ...
        effectiveWorkflowId={effectiveWorkflowId} onWorkflowChange={onWorkflowChange}
      />
      {isSessionMode && (
        <SessionSelectors
...
export function TaskCreateDialog({
  ...
  steps,
  editingTask, onSuccess, onCreateSession, initialValues, taskId = null,
  ...
  const { handleRepositoryChange, handleAgentProfileChange, handleTaskNameChange, handleBranchChange, handleWorkflowChange } =
    useDialogHandlers(fs, repositories);
  const { handleSubmit, handleUpdateWithoutAgent, handleCreateWithoutAgent, handleCancel } =
    useTaskSubmitHandlers({
      isSessionMode, isEditMode, isPassthroughProfile, taskName: fs.taskName,
      workspaceId, workflowId, effectiveWorkflowId, effectiveDefaultStepId,
      ...
      <DialogContent className="w-full h-full max-w-full max-h-full rounded-none sm:w-[900px] sm:h-auto sm:max-w-none sm:max-h-[85vh] sm:rounded-lg flex flex-col">
        <DialogHeader>
          <DialogHeaderContent
            ...
            taskName={fs.taskName} repositoryId={fs.repositoryId}
            discoveredRepoPath={fs.discoveredRepoPath} workspaceId={workspaceId}
            repositoriesLoading={repositoriesLoading} discoverReposLoading={fs.discoverReposLoading}
            headerRepositoryOptions={headerRepositoryOptions}
            onRepositoryChange={handleRepositoryChange} onTaskNameChange={handleTaskNameChange}
          />
        </DialogHeader>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 overflow-hidden">
          <DialogFormBody
            ...
            workflows={workflows} snapshots={snapshots}
            effectiveWorkflowId={effectiveWorkflowId ?? null} fs={fs}
            handleKeyDown={handleKeyDown} onBranchChange={handleBranchChange}
          ...
          <DialogFooter className="border-t border-border pt-3 flex-col gap-3 sm:flex-row sm:gap-2">
            <TaskCreateDialogFooter
              ...
              hasTitle={fs.hasTitle} hasDescription={fs.hasDescription}
              hasRepositorySelection={hasRepositorySelection} branch={fs.branch}
              agentProfileId={fs.agentProfileId} workspaceId={workspaceId}
