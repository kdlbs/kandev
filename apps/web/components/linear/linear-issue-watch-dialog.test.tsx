...
vi.mock("./linear-issue-watch-filter-fields", () => ({
  FilterFields: () => null,
  SelectField: ({ label }: { label: string }) => <div>{label}</div>,
}));
vi.mock("./linear-issue-watch-fields", () => ({
  SettingsFields: () => null,
}));
...
describe("LinearIssueWatchDialog", () => {
  it("uses Linear placeholders with saved-prompt references", () => {
    render(
    ...
    expect(promptEditor).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        testId: "linear-issue-watch-prompt-editor",
