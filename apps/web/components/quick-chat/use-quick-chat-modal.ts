...
export function useQuickChatModal(workspaceId: string) {
  ...
  const handleOpenChange = useCallback((open: boolean) => {
    if (!open) { store.closeQuickChat(); setShowAgentPicker(false); }
  }, [store]);
  ...
  const handleSelectAgent = useCallback(async (agentId: string) => {
    ...
    try {
      ...
      const response = await startQuickChat(workspaceId, { agent_profile_id: agentId, title: initialName });
      ...
      toast({ title: "Failed to start quick chat", description: error instanceof Error ? error.message : "Unknown error", variant: "error" });
    } finally {
      setIsCreating(false);
    }
  ...
  const handleCloseTab = useCallback((sessionId: string) => {
    if (sessionId === "") { store.closeQuickChatSession(sessionId); return; }
  ...
  const handleConfirmClose = useCallback(async () => {
    ...
    if (taskId) {
      try { await deleteQuickChatTask(taskId); }
      catch (error) {
        console.error("Failed to delete quick chat task:", error);
        toast({ title: "Failed to delete quick chat", description: error instanceof Error ? error.message : "Unknown error", variant: "error" });
      }
    }
  }, [sessionToClose, store, toast]);

  return {
    isOpen: store.isOpen, sessions: store.sessions, activeSessionId: store.activeSessionId,
    sessionToClose, activeSessionNeedsAgent: store.activeSessionId === "" || showAgentPicker,
    setActiveQuickChatSession: store.setActiveQuickChatSession, setSessionToClose,
    handleOpenChange, handleNewChat, handleSelectAgent, handleCloseTab, handleConfirmClose,
  };
}
