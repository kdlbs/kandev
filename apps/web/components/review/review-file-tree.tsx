...
function FileNode({
  ...
  return (
    ...
    >
      ...
      <FileStatusIcon
        status={file.status as "modified" | "added" | "deleted" | "untracked" | "renamed"}
      />
...
function TreeNode({
  ...
  if (node.isDir) {
    ...
    const dirReviewed = dirFiles.filter(
      (f) => reviewedFiles.has(f.path) && !staleFiles.has(f.path),
    ).length;
    return (
      <div>
        <button
