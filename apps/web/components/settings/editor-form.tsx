...
function EditorKindFields({ state, setField }: { state: EditorFormState; setField: <K extends keyof EditorFormState>(key: K, value: EditorFormState[K]) => void }) {
  if (state.kind === 'custom_command') {
    return (
      <div className="space-y-2">
        <Input value={state.command} onChange={(event) => setField('command', event.target.value)} placeholder="code --goto {file}:{line}" />
  ...
  if (state.kind === 'custom_remote_ssh') {
    return (
      <div className="space-y-2">
        <Input value={state.host} onChange={(event) => setField('host', event.target.value)} placeholder="ssh-host.example.com" />
        <Input value={state.user} onChange={(event) => setField('user', event.target.value)} placeholder="optional username" />
        <Input value={state.scheme} onChange={(event) => setField('scheme', event.target.value)} placeholder="optional scheme (vscode, cursor)" />
        <p className="text-xs text-muted-foreground">
          Opens a VS Code Remote SSH URL using the scheme (example:{' '}vscode://vscode-remote/ssh-remote+user@host:/path/file:line).
        </p>
      </div>
    );
  }
  if (state.kind === 'custom_hosted_url') {
    return <Input value={state.url} onChange={(event) => setField('url', event.target.value)} placeholder="https://code.example.com" />;
  }
  return null;
}

export function EditorForm({ title, initialState, onCancel, onSave, submitLabel, isSaving }: EditorFormProps) {
  ...
  return (
    <div className="rounded-lg border border-border/70 bg-background p-4 space-y-4">
      ...
      <Input value={state.name} onChange={(event) => setField('name', event.target.value)} placeholder="Editor name" />
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Button type="button" onClick={handleSave} disabled={isSaving || !isValid}>{submitLabel}</Button>
      </div>
    </div>
  );
}
