...
- **All `/api/v1/office/*` routes 404 (office onboarding/deletion tests, danger-zone, etc.)** — means feature flags didn't turn on. The e2e profile (`profiles.yaml` `e2e:` column) sets `KANDEV_FEATURES_OFFICE: "true"`, but `profiles.ApplyProfile` never overrides an already-set var, so an inherited `KANDEV_FEATURES_OFFICE=false` (the kandev runtime injects it when the suite is launched from inside a kandev task) would disable Office. `fixtures/backend.ts` guards against this: `sanitizeInheritedEnv` strips the whole `KANDEV_FEATURES_*` namespace from the inherited env so the profile always governs feature flags. If office routes 404 again, check that strip is intact — don't reach for a per-run `KANDEV_FEATURES_OFFICE=true` override.
