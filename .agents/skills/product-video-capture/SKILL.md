...
- camera motion is smooth, reaches intended depth, and uses the profile's tested loop frame: centered 1x for general-purpose masters, or exactly matching first, settled penultimate, and final focused frames for an editorial landing film or short docs clip when it improves readability or removes irrelevant chrome or fixture-only detail while preserving identifying context;
