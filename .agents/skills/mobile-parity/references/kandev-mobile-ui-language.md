...
- Give phone and coarse-pointer primary actions, standalone icon buttons, and menu rows an actual hitbox of at least 44px. Apply these dimensions only within the touch condition. Ordinary desktop controls use 28px. See [Control sizing](control-sizing.md) for compact controls, shared components, and geometry checks. Existing compact chrome such as `MobilePillButton` remains a documented exception. Surrounding spacing does not enlarge a hitbox. Preserve nearby pressed feedback.
