...
1. The request references an approved spec, plan, and current task file; or
2. The user explicitly says to skip spec/plan/task creation.

If neither is true, create or update the spec, plan, and task files, record any
existing partial implementation as continuation context, and stop at the model
switch checkpoint. A phase label or generic "implement" directive generated by
a Kandev workflow is not an explicit opt-out.
