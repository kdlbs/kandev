...
description: "Single-session Kandev feature workflow: clarify intent, create durable specs, plans, and task files on a strong model, then implement and verify them after the user switches the main session to a lower-cost model."
---
...
```text
Intent -> Spec -> Plan + task files -> User model switch -> Sequential TDD implementation -> PR AI review -> Report
```
...
5. Pause for the user to switch the main model
...
planned test commands, and any open risks. Ask the user to manually switch the
main conversation to the lower-cost implementation model.
...
Resume only after approval or an explicit instruction to continue at the
current model. Never infer a successful switch from a model's prose; inspect
runtime model-usage metadata when confirmation is required.
