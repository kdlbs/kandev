...
- Task-owned browser LSP continuity exposes `lsp_lease_active`, `lsp_lease_detached`, `lsp_lease_evicted_total`, and `lsp_lease_released_total`. Release reasons are a closed set; lease, task, session, execution, user, and language identifiers are never metric labels. The feature is restart-required and disabled in every shipped profile.
