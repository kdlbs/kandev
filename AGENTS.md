...
Jira and Linear are the model for integrations that already live natively in this monorepo (per-workspace credentials, 90s auth-health poller via `internal/integrations/healthpoll`, settings page with status banner). Full layout, file conventions, and Jira-vs-Linear divergence notes in `apps/backend/internal/integrations/AGENTS.md` and the `/add-integration` skill — load either when working on an existing native integration.

**New** third-party integrations default to the plugin model instead (see "Kandev plugins" below), not the native `internal/<name>` playbook. As of 2026-08-15 this is decided per `kdlbs/kandev-plugin-bitbucket` (PR #1) plus the generic host seams in `kdlbs/kandev` PR #2117 (authenticated manifest actions, `registerIntegrationSettings`, `registerTaskAction({placement:"link"})` + `host.openTaskLinkDialog`, dynamic composer `reference_sources`, `PluginOwnedTaskTrees` cascade delete) — the seams that previously forced every integration down the native path now exist generically, so a new connector no longer needs bespoke host work to ship. `yattdev/kandev-plugin-redmine` is the first issue-tracker built this way; only its spec/plan/task-files, contract E2E specs, and the `plugin-registry/plugins.yaml` pointer land in this repo, all implementation lives in the plugin repo. Load `create-kandev-plugin` when scaffolding a new integration; fall back to the native path only when the integration genuinely needs a host-only seam a plugin cannot get (e.g. `internal/integrations/healthpoll`'s free poller, or a true per-workspace secret namespace — plugin secrets are flat `plugin:<id>:secret:<key>` and must be workspace-scoped by the plugin itself).
