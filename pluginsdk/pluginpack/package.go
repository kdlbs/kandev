// Package pluginpack builds valid kandev plugin tar.gz packages. It is a
// dependency-light archive writer deliberately kept separate from the host's
// installer so plugin repositories can use it without importing Kandev
// backend internals.
package pluginpack
...
// checksumsFileName is part of the package format and is generated by this
// writer rather than accepted from callers.
...
// in the tar header — the host installer chmods declared runtime executables
// to 0755 regardless of the archived mode.
func WritePackage(w io.Writer, files map[string][]byte) error {
	if _, exists := files[checksumsFileName]; exists {
		return fmt.Errorf("pluginpack: files must not include %q; it is generated", checksumsFileName)
	}
	...
	if err := tw.Close(); err != nil {
		return fmt.Errorf("pluginpack: closing tar writer: %w", err)
	}
	if err := gz.Close(); err != nil {
		return fmt.Errorf("pluginpack: closing gzip writer: %w", err)
	}
	return nil
}
...
func writeEntry(tw *tar.Writer, name string, data []byte) error {
	...
	if err := tw.WriteHeader(hdr); err != nil {
		return fmt.Errorf("pluginpack: writing header for %s: %w", name, err)
	}
	if _, err := tw.Write(data); err != nil {
		return fmt.Errorf("pluginpack: writing data for %s: %w", name, err)
	}
	return nil
}
