...
// real temp-dir package layouts, verifying raw tar contents, checksums, and
// platform filtering without importing Kandev backend internals.
package main
...
func requireValidChecksums(t *testing.T, data []byte) {
	t.Helper()
	entries := tarEntries(t, data)
	checksums, ok := entries["checksums.txt"]
	require.True(t, ok, "archive must contain checksums.txt")
	listed := make(map[string]bool)
	for _, line := range strings.Split(strings.TrimSpace(string(checksums)), "\n") {
		fields := strings.SplitN(line, "  ", 2)
		require.Len(t, fields, 2, "invalid checksum line %q", line)
		name := fields[1]
		want, err := hex.DecodeString(fields[0])
		require.NoError(t, err)
		body, exists := entries[name]
		require.True(t, exists, "checksum lists missing archive entry %q", name)
		sum := sha256.Sum256(body)
		require.Equal(t, sum[:], want, "checksum mismatch for %q", name)
		listed[name] = true
	}
	for name := range entries {
		if name != "checksums.txt" {
			require.True(t, listed[name], "archive entry %q missing from checksums", name)
		}
	}
}
...
func TestPack_ProducesInstallableTarball(t *testing.T) {
	...
	requireValidChecksums(t, buf.Bytes())
}
...
func TestPackToFile_WritesTarballToDisk(t *testing.T) {
	...
	require.NoError(t, packToFile(dir, out, PackOptions{}))
...
func TestPack_VersionOverrideChangesArchiveOnly(t *testing.T) {
	dir := writeTestPackageDir(t)
	original, err := os.ReadFile(filepath.Join(dir, "manifest.yaml"))
	require.NoError(t, err)

	var buf bytes.Buffer
	require.NoError(t, Pack(dir, &buf, PackOptions{VersionOverride: "pr-123-456"}))

	entries := tarEntries(t, buf.Bytes())
	archived, err := manifest.Parse(entries["manifest.yaml"])
	require.NoError(t, err)
	require.Equal(t, "pr-123-456", archived.Version)
	requireValidChecksums(t, buf.Bytes())

	after, err := os.ReadFile(filepath.Join(dir, "manifest.yaml"))
	require.NoError(t, err)
	require.Equal(t, original, after, "version override must not modify source manifest")
}

func TestPack_InvalidVersionOverrideErrors(t *testing.T) {
	dir := writeTestPackageDir(t)
	var buf bytes.Buffer
	err := Pack(dir, &buf, PackOptions{VersionOverride: "../unsafe"})
	require.Error(t, err)
	require.Contains(t, err.Error(), "version override")
}
