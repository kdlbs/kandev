...
// segment: it is used as a filesystem directory name at install time and to
// resolve an already-installed plugin's data on disk, so an empty,
// "."/".."-only, or
...
// ValidateVersion checks whether version is safe to embed in a packaged
// plugin manifest. Package installers use it as a directory name, so the
// value must be a non-empty, single path segment without traversal or
// whitespace.
func ValidateVersion(version string) error {
	m := Manifest{Version: version}
	return errors.Join(m.validateVersion()...)
}
